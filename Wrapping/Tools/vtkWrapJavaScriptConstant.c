  ...
  FILE* fp, const char* indent, const char* attrib, const char* attribval, ValueInfo* val)
{
  ...
  if (!attrib)
  {
    snprintf(key, 512, "\"%s\"", val->Name);
  }
  else
  {
    snprintf(key, 512, "%s", attrib);
  }
  if (val->IsEnum)
  {
  ...
  FILE* fp, const char* module, const char* basename, const char* indent, NamespaceInfo* data)
{
  ...
  if (scope)
  {
    ...
    {
      /* convert C++ class names to a JavaScript-friendly format */
      // TODO: Determine
      jsscope = text;
    }
  }
  ...
  while (j < data->NumberOfConstants)
  {
    ...
    {
      fprintf(fp, "EMSCRIPTEN_BINDINGS(%s_%s_%d_%d_constants) {\n", module,
        (scope ? scope : basename), constBindingBlockId++, data->ItemType);
      vtkWrapJavaScript_AddConstant(fp, indent, scope, val);
      fprintf(fp, "}\n");
      j++;
      continue;
    }
    ...
    while (j < k)
    {
      ...
      if (val->Access == VTK_ACCESS_PUBLIC)
      {
        if (scopeType)
        {
          snprintf(registeredName, 512, "%s_%s_%s", scope, val->Class, val->Name);
        }
        else if (scopeValue)
        {
          snprintf(registeredName, 512, "%s_%s", scope, val->Name);
        }
        else
        {
          snprintf(registeredName, 512, "%s", val->Name);
        }
...
void vtkWrapJavaScript_AddConstant(FILE* fp, const char* indent, const char* scope, ValueInfo* val)
{
