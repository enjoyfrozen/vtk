...
static void vtkWrapJavaScript_CustomMethods(
  FILE* fp, const char* classname, ClassInfo* data, const char* indent);
...
static void vtkWrapJavaScript_WriteMemberFunctionCall(
  FILE* fp, FunctionInfo* functionInfo, const char* argBaseName);
...
#if 0
/* Some functions do not name their parameters, so come up with a sensible name. */
// TODO: Handle duplicate names. Ex: Func(vtkStdString&, vtkStdString&) should generate Func(stdString_0&, stdString_1&)
static char* vtkWrapJavaScript_GenerateParameterName(
  ValueInfo* parameter, const char* argBaseName, int idx);
#endif
  ...
  FILE* fp, FunctionInfo* functionInfo, const char* argBaseName)
{
  ...
  for (int i = 0; i < functionInfo->NumberOfParameters; ++i)
  {
    ...
    else if (vtkWrap_IsFunction(parameter))
    {
      ...
      else if (vtkWrap_IsInteger(cbReturnValue))
      {
      ...
      for (j = 1; j <= cb->NumberOfParameters; ++j)
      {
        ...
        else if (vtkWrap_IsInteger(cbParam))
        {
          sig[j] = 'i'; // TODO: 64 bit integers need 'j'
        }
...
/* -------------------------------------------------------------------- */
/* Some functions do not name their parameters, so come up with a sensible name.
 * Caller must free the returned string after use.
 */
static char* vtkWrapJavaScript_GenerateParameterName(
  ValueInfo* parameter, const char* argBaseName, int idx)
{
  ...
  {
    if (strncmp(parameter->Class, "vtk", 3) == 0)
    {
      name = (char*)malloc(strlen(parameter->Class) - 3);
      snprintf(name, strlen(parameter->Class) - 2, "%c%s", (char)tolower(parameter->Class[3]),
        &parameter->Class[4]);
      return name;
    }
    else
    {
      name = (char*)malloc(strlen(argBaseName) + 3);
      snprintf(name, strlen(argBaseName) + 3, "%s_%d", argBaseName, idx);
      return name;
    }
  }
  name = (char*)malloc(strlen(parameter->Name));
  snprintf(name, strlen(parameter->Name) + 1, "%s", parameter->Name);
  return name;
}
#endif
  ...
  for (int bucketId = 1; bucketId <= numBuckets; ++bucketId)
  {
    ...
    if (needsOverloadSelection)
    {
      ...
      for (int i = fnum; i < numberOfWrappedFunctions; ++i)
      {
        ...
        vtkWrapJavaScript_WriteMemberFunctionCall(fp, wrappedFunctions[i], "arg");
        if (vtkWrapJavaScript_NeedsToReturnMemoryView(wrappedFunctions[i]))
        {
          fprintf(fp, "))");
        }
  ...
  FileInfo* finfo, HierarchyInfo* hinfo, const char* indent)
{
  ...
  /* TODO: Custom methods like AddObserver, InvokeEvent, ObjectBaseMethods, CollectionMethods */
  vtkWrapJavaScript_CustomMethods(fp, classname, data, indent);
  ...
  for (fnum = 0; fnum < numberOfWrappedFunctions; ++fnum)
  {
    ...
    if (theFunc->Name)
    {
      vtkWrapJavaScript_GenerateOneMethod(fp, classname, data, wrappedFunctions,
        numberOfWrappedFunctions, unwrappableFunctions, numberOfUnwrappableFunctions, fnum, indent);
    }
  }
