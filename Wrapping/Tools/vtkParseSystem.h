...
/**
  ...
  It is used primarily by vtkParsePreprocess.
*/
/**
  This file contains routines to accelerate file existence checks.
  An important part of parsing C and C++ is to search the include
  directories for header files.  Rather that hitting the file system
  for every search, we cache results of previous searches.
*/
...
typedef struct _SystemInfo
{
  ...
  const char*** DirHashTable;  /* paths to all discovered directories */
} SystemInfo;
...
extern "C"
{
  ...
  system_filetype_t vtkParseSystem_FileExists(SystemInfo* info, const char* name);

  /**
   * Free the memory that the SystemInfo uses to cache the files.
   */
  VTKWRAPPINGTOOLS_EXPORT
  void vtkParseSystem_FreeCache(SystemInfo* info);
