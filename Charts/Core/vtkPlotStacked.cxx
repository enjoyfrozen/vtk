...
public:
  ...
  void Configure(vtkPlotStacked* stacked, vtkDataArray* x_array, vtkDataArray* y_array,
    vtkPlotStackedSegment* prev, double bds[4])
  {
  ...
  void FindBadPoints()
  {
    ...
    {
      vtkIdType p = 2 * i;
      if (vtkMath::IsInf(data[p]) || vtkMath::IsInf(data[p + 1]) || vtkMath::IsNan(data[p]) ||
        vtkMath::IsNan(data[p + 1]))
      {
  ...
  bool GetNearestPoint(const vtkVector2f& point, const vtkVector2f& tol, vtkVector2f* location)
  {
...
{
public:
  vtkPlotStackedPrivate(vtkPlotStacked* stacked)
    : Stacked(stacked)
  {
  }
  void Update()
  ...
  vtkPlotStackedSegment* AddSegment(
    vtkDataArray* x_array, vtkDataArray* y_array, vtkPlotStackedSegment* prev = nullptr)
  {
  ...
  void PaintSegments(
    vtkContext2D* painter, vtkColorSeries* colorSeries, vtkPen* pen, vtkBrush* brush)
  {
    int colorInSeries = 0;
