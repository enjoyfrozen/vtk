...
public:
  ...
  vtkVector<float, 2> CurrentSelection;
...
bool vtkChartParallelCoordinates::Paint(vtkContext2D* painter)
{
  ...
  vtkVector<float, 2>& range = this->Storage->CurrentSelection;
  if (range[0] != range[1])
  {
  ...
  for (int i = 0; i < this->Storage->AxesSelections.size(); ++i)
  {
    for (int j = 0; j < this->Storage->AxesSelections[i].size(); ++j)
    {
      float min = this->Storage->AxesSelections[i].at(j);
      float max = this->Storage->AxesSelections[i].at(++j);
      if (min != max)
      {
        this->PaintRect(painter, i, min, max);
      }
    }
  }
...
bool vtkChartParallelCoordinates::MouseButtonReleaseEvent(const vtkContextMouseEvent& mouse)
{
  if (mouse.GetButton() == this->Actions.Select())
  {
    if (this->Storage->CurrentAxis >= 0)
    {
      ...
      this->Storage->CurrentSelection = vtkVector2f(0, 0);
...
bool vtkChartParallelCoordinates::ResetAxeSelection(int axe)
{
  ...
  for (int i = 0; i < this->Storage->AxesSelections[axe].size(); i++)
  {
    float& min = this->Storage->AxesSelections[axe].at(i);
    float& max = this->Storage->AxesSelections[axe].at(++i);
    if (min != max)
    {
      min = max = 0.0f;
      resetRange = true;
...
void vtkChartParallelCoordinates::UpdateCurrentAxisSelection(int axisId)
{
  ...
  vtkVector<float, 2> currentSelection = this->Storage->CurrentSelection;
  float minCurrentSelection =
    (currentSelection[0] < currentSelection[1]) ? currentSelection[0] : currentSelection[1];
  float maxCurrentSelection =
    (currentSelection[1] > currentSelection[0]) ? currentSelection[1] : currentSelection[0];

  bool isANewRange = true;
  bool startAMerge = false;
  for (int i = 0; i < this->Storage->AxesSelections[axisId].size(); i++)
  {
    float& minRange = this->Storage->AxesSelections[axisId].at(i);
    float& maxRange = this->Storage->AxesSelections[axisId].at(++i);
    ...
    if (this->GetSelectionMode() == vtkContextScene::SELECTION_ADDITION)
    {
      ...
      {
        minRange = -1;
        if (startAMerge && (i + 1) >= this->Storage->AxesSelections[axisId].size())
        {
      ...
      if (minCurrentSelection > minRange && minCurrentSelection < maxRange)
      {
        ...
        {
          bool isLastRange = (i + 1) >= this->Storage->AxesSelections[axisId].size();
          if (isLastRange)
          {
            maxRange = maxCurrentSelection;
            isANewRange = false;
          ...
          {
            if (maxCurrentSelection < this->Storage->AxesSelections[axisId].at(i + 1))
            {
              maxRange = maxCurrentSelection;
            }
            else
