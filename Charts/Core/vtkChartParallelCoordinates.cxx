...
bool vtkChartParallelCoordinates::Paint(vtkContext2D* painter)
{
  ...
  auto range = this->Storage->CurrentSelection;
  if (range[0] != range[1])
  {
  ...
  {
    for (int j = 0; j < this->Storage->AxesSelections[axis].size(); j += 2)
    {
...
void vtkChartParallelCoordinates::UpdateCurrentAxisSelection(int axisId)
{
  ...
  auto currentSelection = this->Storage->CurrentSelection;
  ...
  for (int i = 0; i < this->Storage->AxesSelections[axisId].size(); i += 2)
  {
