...
vtkZSpaceInteractorStyle::vtkZSpaceInteractorStyle()
{
  ...
  // XXX Consider finding another way to do interactive picking has the cell
  // XXX picker shows terrible performances (not hardware-accelerated)
...
bool vtkZSpaceInteractorStyle::HardwareSelect(const double p0[3])
{
  ...
  if (this->HardwarePicker->PickProp(p0, rayTransform, this->PickingFieldAssociation, ren))
  {
    return true;
  }
  return false;
...
void vtkZSpaceInteractorStyle::EndPickCallback(vtkSelection* sel)
{
  ...
  // XXX Problem is currently, this mode doesn't seems to work with composite datasets.
...
std::string vtkZSpaceInteractorStyle::GetPickedText(vtkDataSet* ds, const vtkIdType& aid)
{
  ...
  if (this->GetPickingFieldAssociation() == vtkDataObject::FIELD_ASSOCIATION_CELLS)
  {
    ...
    // TODO : seems there is no utility in pure VTK to remplace
    // vtkSMCoreUtilities::GetStringForCellType(int cellType)
    ssPickedText << "Type: " << cell->GetCellType() << "\n";
  }
  else // or a point
  {
...
void vtkZSpaceInteractorStyle::UpdatePickActor()
{
  ...
  if (this->PickedInteractionProp)
  {
    ...
    // if (this->PickedInteractionProp->GetUserMatrix() != nullptr) // TODO : why this code ?
    // {
    //   this->PickActor->SetUserMatrix(this->PickedInteractionProp->GetUserMatrix());
    // }
    this->PickActor->SetPosition(this->PickedInteractionProp->GetPosition());
