...
public:
   ...
   */
  void Initialize(const vtkPixelExtent& winExt, const std::deque<vtkPixelExtent>& blockExts,
  ...
  const std::deque<vtkPixelExtent>& GetGuardExtents() const { return this->GuardExt; }
  ...
  const std::deque<vtkPixelExtent>& GetDisjointGuardExtents() const { return this->GuardExt; }
  ...
  const std::deque<vtkPixelExtent>& GetCompositeExtents() const { return this->CompositeExt; }

  /**
   * Get the whole dataset extent (all blocks).
   */
  const vtkPixelExtent& GetDataSetExtent() const { return this->DataSetExt; }

  /**
   * Get the whole window extent.
   */
  const vtkPixelExtent& GetWindowExtent() const { return this->WindowExt; }
   ...
   */
  virtual int Gather(void*, int, int, vtkTextureObject*&) { return -1; }

  /**
   * Move a single buffer from the LIC decomp to the geometry decomp
   * In parallel THIS IS A COLLECTIVE OPERATION
   */
  virtual int Scatter(void*, int, int, vtkTextureObject*&) { return -1; }
...
protected:
  ...
  int MakeDecompDisjoint(
    const std::deque<vtkPixelExtent>& in, std::deque<vtkPixelExtent>& out, float* vectors);
   ...
   */
  int AddGuardPixels(const std::deque<vtkPixelExtent>& exts, std::deque<vtkPixelExtent>& guardExts,
    std::deque<vtkPixelExtent>& disjointGuardExts, float* vectors);
