...
bool GetMetrics(
  vtkRenderWindow* renwin, vtkTextProperty* tprop, const char* str, vtkTextRenderer::Metrics& m)
{
 ...
 */
bool ComputeBottomLeft(
  vtkTextProperty* tprop, vtkTuple<int, 4> bbox, double pos[3], bool textAsPath, double blpos[3])
{
  ...
  if (gl2psGetFileFormat() == GL2PS_PDF && !textAsPath &&
...
{
  float colf[4] = { col[0] / 255.f, col[1] / 255.f, col[2] / 255.f, col[3] / 255.f };
  ...
  vtkTransformFeedback* tfc, vtkRenderer* ren, float col[4])
{
  ...
  assert("Sanity" && vertexSize == (dataEnd - data) / tfc->GetNumberOfVertices());
  ...
  while (data < dataEnd)
  {
    ...
    for (VarVector::const_iterator it = vars.begin(), itEnd = vars.end(); it != itEnd; ++it)
    {
    ...
    {
      case GL_POINTS:
        gl2psAddPolyPrimitive(GL2PS_POINT, 1, verts, 0, 0.f, 0.f, 0xffff, 1, pointSize, 0, 0, 0);
        break;
      ...
      case GL_LINES:
        ...
        if (curVert == 0)
        {
          gl2psAddPolyPrimitive(
            GL2PS_LINE, 2, verts, 0, 0.f, 0.f, this->LineStipple, 1, lineWidth, 0, 0, 0);
        }
        break;
      ...
      case GL_TRIANGLES:
        ...
        if (curVert == 0)
        {
          gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, verts, 0, 0.f, 0.f, 0xffff, 1, 1, 0, 0, 0);
        }
        break;
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::DrawString(const std::string& str, vtkTextProperty* tprop,
  double pos[3], double backgroundDepth, vtkRenderer* ren)
{
  if (str.empty())
  {
    return;
  ...
  if (tprop->GetBackgroundOpacity() > 0.)
  {
    ...
    if (tren->GetMetrics(tprop, str, metrics, dpi))
    {
      ...
      gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, bgVerts, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0);
      gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, bgVerts + 2, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0);
    }
  }
  ...
  if (!isMath && !this->TextAsPath)
  {
    ...
    if (::ComputeBottomLeft(tprop, m.BoundingBox, pos, this->TextAsPath, blpos))
    {
      ...
      gl2psTextOptColorBL(
        spaceStr.c_str(), fontname, fontSize, align, angle, rgba, blpos[0], blpos[1]);
    }
    else
    {
  ...
  else
  {
    ...
    unsigned char rgba[4] = { static_cast<unsigned char>(rgbd[0] * 255),
      static_cast<unsigned char>(rgbd[1] * 255), static_cast<unsigned char>(rgbd[2] * 255),
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::DrawPath(vtkPath* path, double rasterPos[3], double windowPos[2],
  unsigned char rgba[4], double scale[2], double rotateAngle, float strokeWidth, const char* label)
{
  ...
  switch (gl2psGetFileFormat())
  {
    case GL2PS_PS:
    case GL2PS_EPS:
      this->DrawPathPS(path, rasterPos, windowPos, rgba, scale, rotateAngle, strokeWidth, l);
      break;
    case GL2PS_SVG:
      this->DrawPathSVG(path, rasterPos, windowPos, rgba, scale, rotateAngle, strokeWidth, l);
      break;
    case GL2PS_PDF:
      this->DrawPathPDF(path, rasterPos, windowPos, rgba, scale, rotateAngle, strokeWidth, l);
      break;
    default:
      break;
  }
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::Draw3DPath(vtkPath* path, vtkMatrix4x4* actorMatrix,
...
//------------------------------------------------------------------------------
const char* vtkOpenGLGL2PSHelperImpl::TextPropertyToPSFontName(vtkTextProperty* tprop)
{
...
//------------------------------------------------------------------------------
int vtkOpenGLGL2PSHelperImpl::TextPropertyToGL2PSAlignment(vtkTextProperty* tprop)
{
  switch (tprop->GetJustification())
  {
    case VTK_TEXT_LEFT:
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::GetTransformParameters(vtkRenderer* ren, vtkMatrix4x4* actorMatrix,
  ...
  double point[3], vtkRenderer* ren, vtkMatrix4x4* actorMatrix)
{
  ...
  vtkOpenGLGL2PSHelperImpl::ProjectPoint(
    tmp, xform, vpOrigin, halfSize[0], halfSize[1], zFact[0], zFact[1]);
}
  ...
  vtkPoints* points, vtkRenderer* ren, vtkMatrix4x4* actorMatrix)
{
  ...
  for (vtkIdType i = 0; i < points->GetNumberOfPoints(); ++i)
  {
    ...
    point[3] = 1.0;
    vtkOpenGLGL2PSHelperImpl::ProjectPoint(
      point, xform, vpOrigin, halfSize[0], halfSize[1], zFact[0], zFact[1]);
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::DrawPathPS(vtkPath* path, double rasterPos[3], double windowPos[2],
  ...
  const std::string& label)
{
  ...
  out << static_cast<float>(rgba[0]) / 255.f << " " << static_cast<float>(rgba[1]) / 255.f << " "
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::DrawPathPDF(vtkPath* path, double rasterPos[3], double windowPos[2],
  unsigned char rgba[4], double scale[2], double rotateAngle, float strokeWidth,
  const std::string& /*label*/)
{
  vtkFloatArray* points = vtkArrayDownCast<vtkFloatArray>(path->GetPoints()->GetData());
  ...
  // color
  out << static_cast<float>(rgba[0]) / 255.f << " " << static_cast<float>(rgba[1]) / 255.f << " "
      << static_cast<float>(rgba[2]) / 255.f << (strokeWidth > 1e-5 ? " RG" : " rg") << endl;
  // opacity
  out << static_cast<float>(rgba[3]) / 255.f << (strokeWidth > 1e-5 ? " CA" : " ca") << endl;
  // translate
  ...
  out << cT << " " << sT << " " << -sT << " " << cT << " " << 0.f << " " << 0.f << " cm" << endl;
  // scale
  if (scale)
  {
    out << scale[0] << " " << 0.f << " " << 0.f << " " << scale[1] << " " << 0.f << " " << 0.f
        << " cm" << endl;
  }
...
//------------------------------------------------------------------------------
void vtkOpenGLGL2PSHelperImpl::DrawPathSVG(vtkPath* path, double rasterPos[3], double windowPos[2],
  unsigned char rgba[4], double scale[2], double rotateAngle, float strokeWidth,
  const std::string& label)
{
  vtkFloatArray* points = vtkArrayDownCast<vtkFloatArray>(path->GetPoints()->GetData());
  ...
  out << "<g transform=\"" << endl
      << "     translate(" << windowPos[0] << " " << windowHeight - windowPos[1] << ")" << endl;
  if (scale)
  {
  ...
  if (strokeWidth > 1e-5)
  {
    out << "   fill=\"none\"" << endl
        ...
        << "   stroke=\"rgb(" << std::setprecision(0) << static_cast<int>(rgba[0]) << ","
        << static_cast<int>(rgba[1]) << "," << static_cast<int>(rgba[2])
        << std::setprecision(floatPrec) << ")\"" << endl;
  }
  else
  {
    out << "   stroke=\"none\"" << endl
        << "   fill=\"rgb(" << std::setprecision(0) << static_cast<int>(rgba[0]) << ","
        << static_cast<int>(rgba[1]) << "," << static_cast<int>(rgba[2])
        << std::setprecision(floatPrec) << ")\"" << endl;
  }
