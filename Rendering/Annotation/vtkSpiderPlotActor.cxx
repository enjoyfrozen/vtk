...
struct vtkAxisRange
{
  ...
  vtkAxisRange(double min, double max)
    : Min(min)
    , Max(max)
  {
  }
...
public:
  ...
  vtkSpiderPlotActorConnection() { this->SetNumberOfInputPorts(1); }
};
...
int vtkSpiderPlotActor::PlaceAxes(vtkViewport* viewport, const int* vtkNotUsed(size))
{
    ...
    else // row
    {
      for (j = 0; j < numRows; j++)
      {
        for (i = 0; i < numComponents; i++)
        {
          ...
          if (::vtkSpiderPlotActorGetComponent(field, j, i, &v) == 0)
          {
  ...
  if (this->LabelVisibility)
  {
    ...
    for (i = 0; i < this->N; i++)
    {
      ...
      fontSize = this->LabelMappers[i]->SetConstrainedFontSize(viewport, tsize[0], tsize[1]);
      minFontSize = (fontSize < minFontSize ? fontSize : minFontSize);

      this->LabelActors[i] = vtkActor2D::New();
      this->LabelActors[i]->SetMapper(this->LabelMappers[i]);
      this->LabelActors[i]->GetPositionCoordinate()->SetCoordinateSystemToViewport();
