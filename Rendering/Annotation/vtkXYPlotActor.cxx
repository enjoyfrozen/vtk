...
public:
  ...
  vtkXYPlotActorConnections() { this->SetNumberOfInputPorts(1); }
};
...
vtkXYPlotActor::vtkXYPlotActor()
{
  ...
  this->AdjustTitlePositionMode = vtkXYPlotActor::AlignHCenter | vtkXYPlotActor::AlignTop |
    vtkXYPlotActor::AlignAxisHCenter | vtkXYPlotActor::AlignAxisVCenter;
}
...
//------------------------------------------------------------------------------
int vtkXYPlotActor::IsInputPresent(vtkAlgorithmOutput* in, const char* arrayName, int component)
{
...
//------------------------------------------------------------------------------
void vtkXYPlotActor::AddDataSetInput(vtkDataSet* ds, const char* arrayName, int component)
{
...
void vtkXYPlotActor::AddDataSetInputConnection(
...
//------------------------------------------------------------------------------
void vtkXYPlotActor::RemoveDataSetInput(vtkDataSet* ds, const char* arrayName, int component)
{
  int numConns = this->InputConnectionHolder->GetNumberOfInputConnections(0);
  for (int idx = 0; idx < numConns; idx++)
  {
    vtkAlgorithmOutput* aout = this->InputConnectionHolder->GetInputConnection(0, idx);
...
void vtkXYPlotActor::RemoveDataSetInputConnection(
  vtkAlgorithmOutput* in, const char* arrayName, int component)
{
...
void vtkXYPlotActor::AddDataObjectInputConnection(vtkAlgorithmOutput* aout)
{
  ...
  {
    vtkAlgorithmOutput* port = this->DataObjectInputConnectionHolder->GetInputConnection(0, i);
    if (port == aout)
    {
      return;
    }
...
int vtkXYPlotActor::RenderOpaqueGeometry(vtkViewport* viewport)
{
  ...
  if (numDS > 0)
  {
    ...
    {
      vtkAlgorithmOutput* port = this->InputConnectionHolder->GetInputConnection(0, i);
      vtkAlgorithm* alg = port->GetProducer();
      int portIdx = port->GetIndex();
      alg->Update(portIdx);
    ...
    (this->AxisTitleTextProperty && this->AxisTitleTextProperty->GetMTime() > this->BuildTime))
  {
    ...
    {
      vtkAxisActor2D::ComputeRange(range, xRange, this->NumberOfXLabels, numTicks, interval);
    }
    else
    {
      xRange[0] = range[0];
    ...
    {
      vtkAxisActor2D::ComputeRange(yrange, yRange, this->NumberOfYLabels, numTicks, interval);
    }
    else
    {
      yRange[0] = yrange[0];
...
void vtkXYPlotActor::ComputeXRange(double range[2], double* lengths)
{
  ...
  {
    vtkAlgorithmOutput* port = this->InputConnectionHolder->GetInputConnection(0, dsNum);
    vtkAlgorithm* alg = port->GetProducer();
...
//------------------------------------------------------------------------------
static inline int vtkXYPlotActorGetComponent(
...
//------------------------------------------------------------------------------
void vtkXYPlotActor::ComputeDORange(double xrange[2], double yrange[2], double* lengths)
{
  int i;
  vtkDataObject* dobj;
  vtkFieldData* field;
  ...
  {
    vtkAlgorithmOutput* port = this->DataObjectInputConnectionHolder->GetInputConnection(0, doNum);
    vtkAlgorithm* alg = port->GetProducer();
    int portIdx = port->GetIndex();
    dobj = alg->GetOutputDataObject(portIdx);
    ...
    if (this->XValues != VTK_XYPLOT_INDEX)
    {
      ...
      for (ptId = 0; ptId < num; ptId++)
      {
        ...
        if (this->DataObjectPlotMode == VTK_XYPLOT_ROW)
        {
          ...
          status = ::vtkXYPlotActorGetComponent(field, this->XComponent->GetValue(doNum), ptId, &x);
        }
        else // if ( this->DataObjectPlotMode == VTK_XYPLOT_COLUMN )
        {
          // x = field->GetComponent( ptId, this->XComponent->GetValue( doNum ) );
          status = ::vtkXYPlotActorGetComponent(field, ptId, this->XComponent->GetValue(doNum), &x);
        }
        if (!status)
        {
    ...
    for (ptId = 0; ptId < num; ptId++)
    {
      ...
      if (this->DataObjectPlotMode == VTK_XYPLOT_ROW)
      {
        ...
        status = ::vtkXYPlotActorGetComponent(field, this->YComponent->GetValue(doNum), ptId, &y);
      }
      else // if ( this->DataObjectPlotMode == VTK_XYPLOT_COLUMN )
      {
        // y = field->GetComponent( ptId, this->YComponent->GetValue( doNum ) );
        status = ::vtkXYPlotActorGetComponent(field, ptId, this->YComponent->GetValue(doNum), &y);
      }
      if (!status)
      {
...
//------------------------------------------------------------------------------
void vtkXYPlotActor::CreatePlotData(
  int* pos, int* pos2, double xRange[2], double yRange[2], double* lengths, int numDS, int numDO)
{
  ...
  if (numDS > 0)
  {
    for (int dsNum = 0; dsNum < numDS; dsNum++)
    {
      ...
      {
        vtkErrorMacro("Number of points: " << numPts << " exceeds number of scalar tuples: "
  ...
  else // plot data from data objects
  {
    ...
    for (doNum = 0; doNum < numDOs; doNum++)
    {
      ...
      numPts = (this->DataObjectPlotMode == VTK_XYPLOT_ROW ? numColumns : numRows);
      ...
      for (numLinePts = 0, length = 0.0, ptId = 0; ptId < numPts; ptId++)
      {
        ...
        if (this->DataObjectPlotMode == VTK_XYPLOT_ROW)
        {
          ...
          status1 =
            ::vtkXYPlotActorGetComponent(field, this->XComponent->GetValue(doNum), ptId, &x[0]);
          status2 =
            ::vtkXYPlotActorGetComponent(field, this->YComponent->GetValue(doNum), ptId, &xyz[1]);
        }
        else // if ( this->DataObjectPlotMode == VTK_XYPLOT_COLUMN )
        {
          ...
          status1 =
            ::vtkXYPlotActorGetComponent(field, ptId, this->XComponent->GetValue(doNum), &x[0]);
          ...
          status2 =
            ::vtkXYPlotActorGetComponent(field, ptId, this->YComponent->GetValue(doNum), &xyz[1]);
