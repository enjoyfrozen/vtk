...
void vtkOpenVRRenderWindow::StereoMidpoint()
{
  ...
  {
    this->RenderFramebuffer(this->FramebufferDescs[LEFT_EYE]);

    vr::Texture_t leftEyeTexture = {
      (void*)(long)this->FramebufferDescs[LEFT_EYE].ResolveColorTextureId, vr::TextureType_OpenGL,
      vr::ColorSpace_Gamma
    };
...
void vtkOpenVRRenderWindow::StereoRenderComplete()
{
  ...
  if (this->HMD && this->SwapBuffers) // picking does not swap and we don't show it
  {
    this->RenderFramebuffer(this->FramebufferDescs[RIGHT_EYE]);

    vr::Texture_t rightEyeTexture = {
      (void*)(long)this->FramebufferDescs[RIGHT_EYE].ResolveColorTextureId, vr::TextureType_OpenGL,
      vr::ColorSpace_Gamma
    };
...
const char* vtkOpenVRRenderWindow::GetWindowTitleFromAPI()
{
  ...
  strDisplay =
    this->GetTrackedDeviceString(this->HMD, vr::k_unTrackedDeviceIndex_Hmd, vr::Prop_SerialNumber_String);

  const char* result = "VTK -";
  strcat(result, strDriver.c_str());
  strcat(result, " ");
  strcat(result, strDisplay.c_str());
  return result;
}
