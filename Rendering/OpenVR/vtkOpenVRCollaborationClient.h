...
public:
  ...
  void SetLogCallback(
    std::function<void(std::string const& data, vtkLogger::Verbosity verbosity, void* cd)> cb,
    std::shared_ptr<void> clientData)
  {
    this->Callback = cb;
  ...
  void SetScaleCallback(std::function<double(void* cd)> cb, std::shared_ptr<void> clientData)
  {
    this->ScaleCallback = cb;
    this->ScaleCallbackClientData = clientData;
  }
...
protected:
  ...
  std::function<void(std::string const& data, vtkLogger::Verbosity, void* cd)> Callback;
  std::shared_ptr<void> CallbackClientData;

  std::function<double(void* cd)> ScaleCallback;
  std::shared_ptr<void> ScaleCallbackClientData;
