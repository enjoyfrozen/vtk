...
std::string BaseDeclarationFragment(vtkRenderer* vtkNotUsed(ren), vtkVolumeMapper* mapper,
  vtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs, int numberOfLights,
  int numberPositionalLights, int lightingComplexity, int noOfComponents, int independentComponents)
{
  ...
  std::string totalLights = std::to_string(numberOfLights);
  std::string positionalLights = std::to_string(numberPositionalLights);

  if (lightingComplexity == 2)
  {
    toShaderStr << "#define numberLights " << totalLights
                << "\n"
                   "#define numberPositionalLights "
                << positionalLights
                << "\n"
                   "vec4 g_fragWorldPos;\n"
                   "uniform vec3 in_lightAmbientColor[numberLights];\n"
                   "uniform vec3 in_lightDiffuseColor[numberLights];\n"
                   "uniform vec3 in_lightSpecularColor[numberLights];\n"
                   "uniform vec3 in_lightDirection[numberLights];\n";
    if (numberPositionalLights > 0)
    {
      toShaderStr << "uniform vec3 in_lightPosition[numberPositionalLights];\n"
                     "uniform vec3 in_lightAttenuation[numberPositionalLights];\n"
                     "uniform float in_lightConeAngle[numberPositionalLights];\n"
                     "uniform float in_lightExponent[numberPositionalLights];\n";
    }
  }
  else
  {
...
std::string ComputeLightingDeclaration(vtkRenderer* vtkNotUsed(ren), vtkVolumeMapper* mapper,
  vtkVolume* vol, int noOfComponents, int independentComponents, int numberOfLights,
  int numberPositionalLights, int lightingComplexity)
{
  ...
  if (shadeReqd)
  {
    ...
    else if (lightingComplexity == 2)
    {
      ...
      {
        shaderStr +=
          std::string("\n  for (int posNum = 0; posNum < numberPositionalLights; posNum++)\
          \n  {\
      ...
      shaderStr +=
        std::string("\n  for (int dirNum = numberPositionalLights; dirNum < numberLights; dirNum++)\
          \n  {\
