  ...
  vtkRenderer* vtkNotUsed(ren), vtkVolumeMapper* vtkNotUsed(mapper), vtkVolume* vtkNotUsed(vol))
{
  return std::string(
    ...
    "uniform int in_cprPolyLineNumberOfPoints;\n"
    "// positions of the centerline points in texture/normalized index space (xyz) + centerline "
    "distance (w)\n"
    "uniform sampler2D in_cprPolyLinePositions;\n"
    "// a texture of quaternions: Orientations of the centerline points in texture/index space "
    "(vec4)\n"
    "uniform sampler2D in_cprPolyLineOrientations;\n"
    "// size of the geometry used to display CPR input by the user: (x, y)\n"
    "uniform vec2 in_cprBoxSize;\n"
    "// size of the volume (image data), that is spacing * extent\n"
    "uniform vec3 in_cprVolumeSize;\n"
    "\n"
    ...
    "}\n"
    "\n"
    "vec4 sampleTextureAsArray(sampler2D tex, int index, int arraySize) {\n"
    "    float xCoord = (0.5 + float(index)) / float(arraySize);\n"
    "    return texture2D(tex, vec2(xCoord, 0.5));\n"
    "}\n"
    "\n"
    "vec3 custom3DTransform(vec3 p) {\n"
    "    // Use a binary search to find between which indices to interpolate position and "
    "orientation\n"
    "    float sampleDistance = p.z;\n"
    "    int minI = 0;\n"
    "    int maxI = in_cprPolyLineNumberOfPoints - 1;\n"
    "    while (minI + 1 < maxI) {\n"
    "        int midI = (minI + maxI) / 2;\n"
    "        // Sample cpr position texture\n"
    "        vec4 midPosition = sampleTextureAsArray(in_cprPolyLinePositions, midI, "
    "in_cprPolyLineNumberOfPoints);\n"
    "        float midDistance = midPosition.w;\n"
    "        if (midDistance < sampleDistance) {\n"
    "            minI = midI;\n"
    "        } else {\n"
    "            maxI = midI;\n"
    "        }\n"
    ...
    "    vec4 p0 = sampleTextureAsArray(in_cprPolyLinePositions, minI, "
    "in_cprPolyLineNumberOfPoints);\n"
    "    vec4 p1 = sampleTextureAsArray(in_cprPolyLinePositions, maxI, "
    "in_cprPolyLineNumberOfPoints);\n"
    "    float t = (sampleDistance - p0.w) / (p1.w - p0.w);\n"
    "    vec3 interpolatedPositionTC = mix(p0.xyz, p1.xyz, t);\n"
    "\n"
    "    // Spherical interpolation of orientation\n"
    "    vec4 q0 = sampleTextureAsArray(in_cprPolyLineOrientations, minI, "
    "in_cprPolyLineNumberOfPoints);\n"
    "    vec4 q1 = sampleTextureAsArray(in_cprPolyLineOrientations, maxI, "
    "in_cprPolyLineNumberOfPoints);\n"
    "    float qCosAngle = dot(q0, q1);\n"
    "    vec4 interpolatedOrientationTC;\n"
    "    if (qCosAngle > 0.999 || qCosAngle < -0.999) {\n"
    "        // Use LERP instead of SLERP when the two quaternions are close or opposite\n"
    "        interpolatedOrientationTC = normalize(mix(q0, q1, t));\n"
    "    } else {\n"
    "        float omega = acos(qCosAngle);\n"
    "        interpolatedOrientationTC = normalize(sin((1.0 - t) * omega) * q0 + sin(t * omega) * "
    "q1);\n"
    "    }\n"
    "\n"
    "    // Compute the new position to sample in the texture\n"
    "    // The position of the sample in the frame defined by interpolatedPositionTC and "
    "interpolatedOrientationTC\n"
    "    vec3 interpolatedDirectionTC = rotateByQuaternion(vec3((p.xy - vec2(0.5, 0.5)) * "
    "in_cprBoxSize, 0.0), interpolatedOrientationTC) /in_cprVolumeSize;\n"
    "    return interpolatedPositionTC + interpolatedDirectionTC;\n"
    "}\n"
    "\n"
    "// ----------------------------------------------------------------\n"
    "// Redefine texture() to use custom3DTransform() when sampling a 3D texture\n"
    "\n"
    "vec4 baseTextureFn(sampler3D tex, vec3 p) {\n"
    "  return texture3D(tex, p);\n"
    "}\n"
    "\n"
    "vec4 baseTextureFn(sampler2D tex, vec2 p) {\n"
    "  return texture2D(tex, p);\n"
    "}\n"
    "\n"
    "// TODO:\n"
    "//\n"
    "// In OpenGL ES, it is forbidden to override built-in functions\n"
    "// "
    "https://registry.khronos.org/OpenGL/specs/es/3.2/"
    "GLSL_ES_Specification_3.20.html#function-definitions\n"
    "// > A shader cannot redefine or overload built-in functions.\n"
    "//\n"
    "// But it is legal to refine it in OpenGL:\n"
    "// https://registry.khronos.org/OpenGL/specs/gl/GLSLangSpec.4.60.pdf\n"
    "// > A shader can redefine built-in functions. If a built-in function is redeclared in a "
    "shader (i.e., a\n"
    "// > prototype is visible) before a call to it, then the linker will only attempt to resolve "
    "that call within\n"
    "// > the set of shaders that are linked with it.\n"
    "\n"
    "vec4 texture(sampler3D tex, vec3 p) {\n"
    "  return baseTextureFn(tex, custom3DTransform(p));\n"
    "}\n"
    "\n"
    "vec4 texture(sampler2D tex, vec2 p) {\n"
    "  return baseTextureFn(tex, p);\n"
    "}\n"
    "// ----------------------------------------------------------------\n");
}
