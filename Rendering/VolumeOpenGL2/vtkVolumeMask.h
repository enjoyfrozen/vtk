...
public:
  ...
  //--------------------------------------------------------------------------
  vtkTimeStamp GetBuildTime() { return this->BuildTime; }
  ...
  //--------------------------------------------------------------------------
  void Update(vtkRenderer* ren, vtkImageData* input, int cellFlag, int textureExtent[6],
    ...
    if (obsolete)
    {
      ...
      if (this->Loaded)
      {
        ...
        this->Loaded = textureSize[0] * textureSize[1] * textureSize[2] *
            vtkAbstractArray::GetDataTypeSize(scalarType) * scalars->GetNumberOfComponents() <=
          maxMemoryInBytes;
        if (this->Loaded)
        {
  ...
  //--------------------------------------------------------------------------
  vtkIdType* GetLoadedExtent() { return this->LoadedExtent; }
