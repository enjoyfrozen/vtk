...
public:
  ...
  virtual void OnKeyPress() override
  {
    ...
    // Multiply Data
    if (key.compare("M") == 0) // "M" for "M"ore data
    {
      MultiplyData(2.0);
    }
    if (key.compare("m") == 0) // "m" for "l"ess data?
    {
      MultiplyData(0.5);
    }

    // Scale
    if (key.compare("B") == 0) // "B" for "B"igger transform
    {
      Scale(2.0);
    }
    if (key.compare("b") == 0) // "b" for "s"maller tranform?
    {
      Scale(0.5);
    }

    // Font Size
    if (key.compare("G") == 0) // "G" for bigger font size
    {
      FontSize(6);
    }
    if (key.compare("g") == 0) // "g" for smaller font size
    {
      FontSize(-6);
    }
    ...
    if (key.compare("I") == 0 || key.compare("i") == 0)
    {
      static int prefix = 0;
...
#define INSRC 0
  ...
  filteredLabelMapper->SetInputArrayToProcess(
    0, 0, 0, vtkDataObject::FIELD_ASSOCIATION_POINTS, LABEL_TYPES);
  filteredLabelMapper->SetInputConnection(filter->GetOutputPort());
