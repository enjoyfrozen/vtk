...
VOID CALLBACK vtkTDxWinDeviceTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime);

class vtkLessThanWindowHandle
{
public:
  bool operator()(const HWND& h1, const HWND& h2) const { return h1 < h2; }
};
...
static const std::map<HRESULT, std::string> resultCodeMap = { { S_OK, "S_OK" },
  { REGDB_E_CLASSNOTREG, "REGDB_E_CLASSNOTREG" },
  { CLASS_E_NOAGGREGATION, "CLASS_E_NOAGGREGATION" }, { E_NOINTERFACE, "E_NOINTERFACE" },
  { E_POINTER, "E_POINTER" }, { E_ABORT, "E_ABORT" }, { E_ACCESSDENIED, "E_ACCESSDENIED" },
  { E_FAIL, "E_FAIL" }, { E_HANDLE, "E_HANDLE" }, { E_INVALIDARG, "E_INVALIDARG" },
  { E_NOTIMPL, "E_NOTIMPL" }, { E_OUTOFMEMORY, "E_OUTOFMEMORY" },
...
{
  if (resultCodeMap.find(hr) != resultCodeMap.end())
    return resultCodeMap.at(hr).c_str();
  return "unknown";
}
...
void vtkTDxWinDevice::Initialize()
{
  ...
  if (alreadyConnected)
  {
    ...
    std::map<HWND, vtkTDxWinDevice*, vtkLessThanWindowHandle>::iterator it =
      vtkWindowHandleToDeviceObjectConnection.begin();
    ...
    vtkWindowHandleToDeviceObjectConnection.insert({ this->WindowHandle, this });
  ...
  else
  {
    ...
    if (!SUCCEEDED(hr))
    {
      vtkWarningMacro(<< "CoInitializeEx failed. hresult=0x" << hex << hr << dec
                      << HresultCodeToString(hr));
    ...
    if (status)
    {
      ...
      if (status)
      {
        ...
        vtkWindowHandleToDeviceObjectConnection.insert(
      ...
      {
        vtkWarningMacro(<< "Could not get the device interface. hresult=0x" << hex << hr << dec
                        << HresultCodeToString(hr));
      }
    }
    else
...
void vtkTDxWinDevice::StartListening()
{
  ...
  this->Private->TimerId =
    ::SetTimer(this->WindowHandle, VTK_IDT_TDX_TIMER, 25, vtkTDxWinDeviceTimerProc);
...
void vtkTDxWinDevice::StopListening()
{
  ...
  const UINT_PTR uIDEvent = this->WindowHandle != 0 ? VTK_IDT_TDX_TIMER : this->Private->TimerId;
  ...
  std::map<HWND, vtkTDxWinDevice*, vtkLessThanWindowHandle>::iterator it =
    vtkWindowHandleToDeviceObject.find(this->WindowHandle);
...
void vtkTDxWinDevice::Close()
{
  ...
  std::map<HWND, vtkTDxWinDevice*, vtkLessThanWindowHandle>::iterator it =
    vtkWindowHandleToDeviceObjectConnection.find(this->WindowHandle);
      ...
      for (i = 1; i <= nKeys; i++)
      {
        ...
        if (isPressed == VARIANT_TRUE)
        {
          if (!(o->KeyStates & mask))
          {
            ...
            o->Interactor->InvokeEvent(vtkCommand::TDxButtonPressEvent, &buttonInfo);
          }
        }
        else
        {
  ...
  if (o->Sensor != 0)
  {
    try
    {
      ...
      if (r->Angle > 0. || t->Length > 0.)
      {
        ...
        {
          timeFactor = static_cast<double>(currentTime - o->LastTimeStamp) / o->Sensor->Period;
        }
        ...
        o->Interactor->InvokeEvent(vtkCommand::TDxMotionEvent, &motionInfo);
      }
      else
      {
        o->LastTimeStamp = 0;
...
VOID CALLBACK vtkTDxWinDeviceTimerProc(
  HWND hwnd, UINT vtkNotUsed(uMsg), UINT_PTR vtkNotUsed(idEvent), DWORD vtkNotUsed(dwTime))
{
  std::map<HWND, vtkTDxWinDevice*, vtkLessThanWindowHandle>::iterator it =
    vtkWindowHandleToDeviceObject.find(hwnd);
