...
void vtkTDxUnixDevice::Initialize()
{
  ...
  int status =
    MagellanInit(static_cast<Display*>(this->DisplayId), static_cast<Window>(this->WindowId));
...
bool vtkTDxUnixDevice::ProcessEvent(const vtkTDxUnixDeviceXEvent* e)
{
  ...
  assert("pre: e_is_client_message" && static_cast<const XEvent*>(e)->type == ClientMessage);

  MagellanFloatEvent info;

  const XEvent* event = static_cast<const XEvent*>(e);

  int deviceEvent = MagellanTranslateEvent(static_cast<Display*>(this->DisplayId),
    const_cast<XEvent*>(event), &info, this->TranslationScale, this->RotationScale);
  ...
  switch (deviceEvent)
  {
    ...
    case MagellanInputButtonPressEvent:
      ...
      if (this->Interactor != 0)
      {
        this->Interactor->InvokeEvent(vtkCommand::TDxButtonPressEvent, &buttonInfo);
      }
      result = true;
      break;
    case MagellanInputButtonReleaseEvent:
      ...
      if (this->Interactor != 0)
      {
        this->Interactor->InvokeEvent(vtkCommand::TDxButtonReleaseEvent, &buttonInfo);
      }
      result = true;
      break;
    default:
...
void vtkTDxUnixDevice::SetSensitivity(double sensitivity)
{
  ...
  MagellanApplicationSensitivity(static_cast<Display*>(this->DisplayId), sensitivity);
}
