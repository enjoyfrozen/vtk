...
public:
  ...
  void Init(double values[VTK_VALUES_SIZE], double zView, bool exitFace)
  {
    this->Zview = zView;
    int i = 0;
...
public:
  ...
  vtkVertexEntry(int screenX, int screenY, double xWorld, double yWorld, double zWorld,
    double zView, double scalar, double invW)
    : ScreenX(screenX)
    , ScreenY(screenY)
    , Zview(zView)
    , InvW(invW)
  ...
  void Set(int screenX, int screenY, double xWorld, double yWorld, double zWorld, double zView,
    double scalar, double invW)
  {
  ...
  int GetScreenX() { return this->ScreenX; }
  int GetScreenY() { return this->ScreenY; }
...
public:
  ...
  virtual void SkipLines(int deltaY, int y) = 0;
...
{
public:
  vtkSimpleScreenEdge() { this->Case = VTK_CASE_UNDEFINED; }
  ...
  void Init(vtkVertexEntry* v0, vtkVertexEntry* v2, int dx20, int dy20, int onRight)
  {
...
//            assert("check: negative_equation" && this->Error<0);
  ...
  void SkipLines(int deltaY, int y) override
  {
    if (deltaY == 1)
    {
      this->NextLine(0);
...
class vtkDoubleScreenEdge : public vtkScreenEdge
{
public:
  void Init(
    vtkVertexEntry* v0, vtkVertexEntry* v1, vtkVertexEntry* v2, int dx10, int dy10, int onRight)
  {
  ...
  void NextLine(int y) override { this->Current->NextLine(y); }
  void SkipLines(int deltaY, int y) override { this->Current->SkipLines(deltaY, y); }
...
public:
  ...
  void Init(int x0, double invW0,
    double pValues0[VTK_VALUES_SIZE], // projected values
    double zView0, int x1, double invW1,
  ...
  vtkTypeBool IsAtEnd() { return this->X > this->X1; }
...
public:
  ...
  ~vtkPixelListEntryBlock() { delete[] this->Array; }
...
public:
  ...
  {
    if (this->FirstFreeElement == nullptr)
    {
  ...
  {
    assert("pre: e_exists" && e != nullptr);

    // the following line works even if this->FirstFreeElement==0
    e->SetNext(this->FirstFreeElement);
    this->FirstFreeElement = e;
  }
  void FreeSubList(vtkPixelListEntry* first, vtkPixelListEntry* last)
  {
    assert("pre: first_exists" && first != nullptr);
    assert("pre: last_exists" && last != nullptr);
...
public:
  ...
  void AddAndSort(vtkPixelListEntry* p)
  {
    ...
    else
    {
      ...
      {
        if (it->GetNext() == nullptr) // last element
        {
          it->SetNext(p);
          p->SetPrevious(it);
          p->SetNext(nullptr);
  ...
  {
    assert("pre: not_empty" && this->Size > 0);
    assert("pre: mm_exists" && mm != nullptr);
...
public:
  ...
  void AddAndSort(int i, vtkPixelListEntry* pixelEntry)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
    assert("pre: pixelEntry_exists" && pixelEntry != nullptr);
  ...
  void PopFront(int i, vtkPixelListEntryMemory* mm)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
    assert("pre: not_empty" && this->GetListSize(i) > 0);
    assert("pre: mm_exists" && mm != nullptr);
...
public:
  ...
  {
    assert("pre: ordered ids" && faceIds[0] < faceIds[1] && faceIds[1] < faceIds[2]);
...
public:
  ...
  void SetCellScalars(int cellScalars) { this->CellScalars = cellScalars; }
  ...
  void AddFace(vtkIdType faceIds[3], vtkDataArray* scalars, vtkIdType cellIdx,
    int orientationChanged, bool external)
  {
    // Ignore degenerate faces.
    if ((faceIds[0] == faceIds[1]) || (faceIds[1] == faceIds[2]))
      return;
...
void vtkUnstructuredGridVolumeZSweepMapper::Render(vtkRenderer* ren, vtkVolume* vol)
{
  ...
  this->Scalars = vtkUnstructuredGridVolumeZSweepMapper::GetScalars(this->GetInput(), this->ScalarMode, this->ArrayAccessMode,
...
void vtkUnstructuredGridVolumeZSweepMapper::BuildUseSets()
{
  ...
  if (this->CellScalars && this->GetMTime() > this->SavedTriangleListMTime.GetMTime())
  {
    needsUpdate = 1;
  }
  ...
  if (this->CellScalars)
  {
    this->UseSet->SetNumberOfComponents(this->Scalars->GetNumberOfComponents());
  }
  // for each cell
  ...
  for (cellIter->InitTraversal(); !cellIter->IsDoneWithTraversal(); cellIter->GoToNextCell())
  {
    ...
    if (faces > 0)
    {
      ...
      while (faceidx < faces)
      {
        ...
        input->GetCellNeighbors(cellIter->GetCellId(), face->GetPointIds(), cellNeighbors);
        bool external = (cellNeighbors->GetNumberOfIds() == 0);

        // Add face only if it is not already in the useset.
        this->UseSet->AddFace(
          orderedFaceIds, this->Scalars, cellIter->GetCellId(), orientationChanged, external);
...
// has changed.
int vtkUnstructuredGridVolumeZSweepMapper::ReorderTriangle(vtkIdType v[3], vtkIdType w[3])
{
  if (v[0] > v[1])
  {
    if (v[1] > v[2])
...
void vtkUnstructuredGridVolumeZSweepMapper::ProjectAndSortVertices(vtkRenderer* ren, vtkVolume* vol)
{
  ...
  while (pointId < numberOfPoints)
  {
    ...
    int xScreen = static_cast<int>(
      (outPoint[0] * invW + 1) * 0.5 * this->ImageViewportSize[0] - this->ImageOrigin[0]);
    int yScreen = static_cast<int>(
      (outPoint[1] * invW + 1) * 0.5 * this->ImageViewportSize[1] - this->ImageOrigin[1]);
    ...
    vertex->Set(xScreen, yScreen, outWorldPoint[0] / outWorldPoint[3],
      outWorldPoint[1] / outWorldPoint[3], outWorldPoint[2] / outWorldPoint[3], zView, scalar,
      invW);
...
void vtkUnstructuredGridVolumeZSweepMapper::CreateAndCleanPixelList()
{
  ...
  if (this->PixelListFrame != nullptr)
  {
...
void vtkUnstructuredGridVolumeZSweepMapper::RasterizeTriangle(
  vtkVertexEntry* ve0, vtkVertexEntry* ve1, vtkVertexEntry* ve2, bool externalFace)
{
  assert("pre: ve0_exists" && ve0 != nullptr);
  assert("pre: ve1_exists" && ve1 != nullptr);
  assert("pre: ve2_exists" && ve2 != nullptr);
  ...
  vtkScreenEdge* leftEdge = nullptr;
  vtkScreenEdge* rightEdge = nullptr;
  ...
  if (det == 0) // v0v1v2 aligned or v0=v1=v2
  {
    ...
    if (v0->GetScreenX() == v1->GetScreenX() && v0->GetScreenX() == v2->GetScreenX() &&
      v0->GetScreenY() == v1->GetScreenY() && v0->GetScreenY() == v2->GetScreenY())
    {
      x = v0->GetScreenX();
      int y = v0->GetScreenY();
      if (x >= 0 && x < this->ImageInUseSize[0] && y >= 0 && y < this->ImageInUseSize[1])
      {
        ...
        if (!this->MaxPixelListSizeReached)
        {
          this->MaxPixelListSizeReached =
            this->PixelListFrame->GetListSize(i) > this->MaxPixelListSize;
        }
      }
    }
    else // line
...
{
  assert("pre: left_exists" && left != nullptr);
  assert("pre: right_exists" && right != nullptr);

  vtkIdType i = y * this->ImageInUseSize[0];

  this->Span->Init(left->GetX(), left->GetInvW(), left->GetPValues(), left->GetZview(),
    right->GetX(), right->GetInvW(), right->GetPValues(), right->GetZview());
  ...
  while (!this->Span->IsAtEnd())
  {
    ...
    if (x >= 0 && x < this->ImageInUseSize[0]) // clipping
    {
      ...
      {
        this->MaxPixelListSizeReached =
          this->PixelListFrame->GetListSize(j) > this->MaxPixelListSize;
      }
    }
    this->Span->NextPixel();
  }
...
{
  assert("pre: v0_exists" && v0 != nullptr);
  assert("pre: v1_exists" && v1 != nullptr);
  ...
  {
    // write pixel
    if (x >= 0 && x < this->ImageInUseSize[0] && y >= 0 && y < this->ImageInUseSize[1]) // clipping
    {
      vtkIdType j = y * this->ImageInUseSize[0] + x; // mult==bad!!
      // Write the pixel
      vtkPixelListEntry* p0 = this->MemoryManager->AllocateEntry();
        ...
        while (!done)
        {
          ...
          else
          {
            if (this->ZBuffer != nullptr)
            {
              ...
              doIntegration = current->GetZview() < zBuffer && next->GetZview() < zBuffer;
            }
            else
            {
              doIntegration = 1;
          ...
          if (doIntegration)
          {
            if (current->GetZview() != next->GetZview())
            {
              ...
              length =
                sqrt(vtkMath::Distance2BetweenPoints(current->GetValues(), next->GetValues()));
              if (length != 0)
              //              if(length>=0.4)
              {
                ...
                if (this->CellScalars)
                {
                  ...
                  this->NearIntersections->SetValue(
                    0, current->GetValues()[VTK_VALUES_SCALAR_INDEX]);
                  this->FarIntersections->SetValue(
                    0, current->GetValues()[VTK_VALUES_SCALAR_INDEX]);
                }
                else
                {
                ...
                this->RealRayIntegrator->Integrate(this->IntersectionLengths,
                  this->FarIntersections, this->NearIntersections, color);
#else
                this->RealRayIntegrator->Integrate(this->IntersectionLengths,
