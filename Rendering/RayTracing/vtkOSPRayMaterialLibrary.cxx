...
//------------------------------------------------------------------------------
std::string vtkOSPRayMaterialLibrary::WriteString(bool writeImageInline)
{
  Json::Value root;
  root["family"] = "OSPRay";
  root["version"] = "0.0";
  ...
  while (it != this->Internal->NickNames.end())
  {
    ...
    if (this->Internal->TexturesFor.find(nickname) != this->Internal->TexturesFor.end())
    {
      ...
      {
        std::string vname = vit->first;
        TextureInfo& texInfo = vit->second;
...
{
  // When writing into a buffer, always pass through rawdata texture
  // to be coherent with previous behaviour
  std::string rstring = this->WriteString(true);
...
{
  std::string rstring = this->WriteString(false);
  ...
  const std::string& nickname, const std::string& varname)
{
  ...
  if (this->Internal->TexturesFor.find(nickname) != this->Internal->TexturesFor.end())
  {
    ...
    std::string realname = ::FindRealName(this->Internal->ImplNames[nickname], varname);
    return tsForNickname[realname].Texture;
  }
  return nullptr;
}

//------------------------------------------------------------------------------
void vtkOSPRayMaterialLibrary::GetTextureName(
  const std::string& nickname, const std::string& varname, std::string& textureName)
{
  NamedTextures tsForNickname;
  if (this->Internal->TexturesFor.find(nickname) != this->Internal->TexturesFor.end())
  {
    tsForNickname = this->Internal->TexturesFor[nickname];
    std::string realname = ::FindRealName(this->Internal->ImplNames[nickname], varname);
    textureName = tsForNickname[realname].Name;
  }
}

//------------------------------------------------------------------------------
void vtkOSPRayMaterialLibrary::GetTextureFilename(
  const std::string& nickname, const std::string& varname, std::string& filename)
{
  NamedTextures tsForNickname;
  if (this->Internal->TexturesFor.find(nickname) != this->Internal->TexturesFor.end())
  {
    tsForNickname = this->Internal->TexturesFor[nickname];
    std::string realname = ::FindRealName(this->Internal->ImplNames[nickname], varname);
    filename = tsForNickname[realname].Filename;
  }
}
