...
//------------------------------------------------------------------------------
vtkSmartPointer<vtkViewNode> vtkViewNode::GetParentOwned() const
{
...
{
  auto parent = this->GetFirstAncestorOfTypeOwned(type);
  // Extract the pointer. The caller doesn't know if it owns this or not, so it
  // cannot be passed back with a new reference without leaking in existing
  // code.
  vtkViewNode* parent_ptr = parent;
...
//------------------------------------------------------------------------------
vtkSmartPointer<vtkViewNode> vtkViewNode::GetFirstAncestorOfTypeOwned(const char* type) const
{
