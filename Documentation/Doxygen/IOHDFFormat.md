...
that could be easily loaded in VTK without the need for external
data description (such as XDMF).  We implement a reader for ImageData
and UnstructuredGrid while other dataset types may be added later. We
use the same extension for serial and parallel formats and for all
