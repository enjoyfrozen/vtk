 ...
 * point and cell ids. This is provided since it relative easy to generate these
...
public:
  ...
  //@{
  /**
   * When set to true (default), this filter will generate global point ids. In
   * input has global point ids, the filter may use them, if possible. It is not
   * possible to use input global  point ids when the boundary mode is set to
   * `SPLIT_BOUNDARY_CELLS`. In that case, new global point ids are generated if
   * requested regardless of whether the input has global point ids.
   */
  vtkSetMacro(GenerateGlobalPointIds, bool);
  vtkGetMacro(GenerateGlobalPointIds, bool);
  vtkBooleanMacro(GenerateGlobalPointIds, bool);
  //@}
...
private:
  ...
  bool MarkGhostCells(vtkPartitionedDataSet* pieces);
