  ...
  for (vtkIdType i = 0; i < pointIds->GetNumberOfIds() - 1; ++i)
  {
    ...
    switch (this->SamplingPattern)
    {
      ...
      case SAMPLE_LINE_AT_SEGMENT_CENTERS:
        ...
        {
          tmp = this->SampleLineAtEachCell(p1, p2, htgInput, tolerance);
        }
        break;
  ...
  vtkHyperTreeGrid * input, const double tolerance) const
{
  ...
  this->Internal->HyperTreeGridLocator->IntersectWithLine(p1.GetData(), p2.GetData(), 0.0, pointsFound, intersectedIds, cell);
  this->Internal->HyperTreeGridLocator->IntersectWithLine(p2.GetData(), p1.GetData(), 0.0, pointsFound, intersectedIds, cell);
  ...
  for (vtkIdType i = 0; i < intersectedIds->GetNumberOfIds(); ++i)
  {
    ...
    // Add intersected cell
    intersections[cellId].CellId = cellId;
    vtkMath::Subtract(pointsFound->GetPoint(i), p1.GetData(), ptBuffer.data());
    double thisT = vtkMath::Norm(ptBuffer.data()) / norm;
    if(i < static_cast<vtkIdType>(intersections.size()))
    {
      intersections[cellId].InT = thisT;
    }
    else
    {
      intersections[cellId].OutT = thisT;
    }
  }
