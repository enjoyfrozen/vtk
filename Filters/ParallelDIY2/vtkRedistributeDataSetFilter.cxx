...
namespace
{
static const char* POINT_OWNERSHIP_ARRAYNAME = " __RDSF_POINT_OWNERSHIP__";
...
vtkRedistributeDataSetFilter::vtkRedistributeDataSetFilter()
  ...
  , GenerateGlobalCellIds(false)
  , GenerateGlobalPointIds(false)
...
//----------------------------------------------------------------------------
int vtkRedistributeDataSetFilter::FillInputPortInformation(int port, vtkInformation* info)
{
...
std::vector<vtkBoundingBox> vtkRedistributeDataSetFilter::GenerateCuts(vtkDataObject* dobj)
{
  ...
  cellCenterFilter->SetInputData(dobj);
  cellCenterFilter->Update();

  std::vector<vtkSmartPointer<vtkPoints> > cellCenters;
  if (auto cd = vtkCompositeDataSet::SafeDownCast(cellCenterFilter->GetOutputDataObject(0)))
  {
    auto iter = cd->NewIterator();
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
    {
      if (auto ps = vtkPointSet::SafeDownCast(iter->GetCurrentDataObject()))
      {
        if (ps->GetPoints())
        {
          cellCenters.push_back(ps->GetPoints());
        }
      }
    }
    iter->Delete();
  ...
  {
    auto ps = vtkPointSet::SafeDownCast(cellCenterFilter->GetOutputDataObject(0));
    assert(ps != nullptr);
    if (ps->GetPoints())
    {
      cellCenters.push_back(ps->GetPoints());
  ...
  {
    cd->GetBounds(local_bounds);
  }
  else if (auto ds = vtkDataSet::SafeDownCast(dobj))
  {
    ds->GetBounds(local_bounds);
  }
  ...
  return vtkDIYKdTreeUtilities::GenerateCuts(
    cellCenters, std::max(1, num_partitions), controller, local_bounds);
}
  ...
  vtkDataObject* inputDO, vtkPartitionedDataSet* outputPDS, const std::vector<vtkBoundingBox>& cuts)
{
  ...
  if (this->GenerateGlobalPointIds)
  {
    // generate global point ids, if none are present.
    this->AssignGlobalPointIds(outputPDS, cuts);
  }
  ...
  if (!this->EnableDebugging)
  {
    ...
    for (unsigned int partId = 0, max = outputPDS->GetNumberOfPartitions(); partId < max; ++partId)
    {
      ...
      {
        dataset->GetPointData()->RemoveArray(POINT_OWNERSHIP_ARRAYNAME);
  ...
  vtkDataSet* inputDS, vtkPartitionedDataSet* outputPDS, const std::vector<vtkBoundingBox>& cuts)
{
  ...
  vtkLogF(INFO, "num-pieces %d", pieces->GetNumberOfPartitions());
...
//----------------------------------------------------------------------------
bool vtkRedistributeDataSetFilter::AssignGlobalPointIds(
  vtkPartitionedDataSet* pieces, const std::vector<vtkBoundingBox>& cuts)
{
  assert(this->GenerateGlobalPointIds == true);
  ...
  for (unsigned int partId = 0; partId < pieces->GetNumberOfPartitions(); ++partId)
  {
    ...
    if (dataset && dataset->GetNumberOfPoints() > 0 &&
      dataset->GetPointData()->GetGlobalIds() == nullptr)
    {
      missing_gids = 1;
      break;
    }
  ...
  // we need to generate global point ids since not all pieces (if any) have global point
  // ids.

  // add a point ownership array which helps decide which block assign global id
  // to a point uniquely.
  for (unsigned int partId = 0; partId < pieces->GetNumberOfPartitions(); ++partId)
  {
    vtkDataSet* dataset = pieces->GetPartition(partId);
    ...
    vtkNew<vtkIntArray> pt_ownership;
    pt_ownership->SetName(POINT_OWNERSHIP_ARRAYNAME);
    pt_ownership->SetNumberOfComponents(1);
    pt_ownership->SetNumberOfTuples(dataset->GetNumberOfPoints());
    dataset->GetPointData()->AddArray(pt_ownership);

    vtkSMPTools::For(0, dataset->GetNumberOfPoints(), [&](vtkIdType start, vtkIdType end) {
      for (vtkIdType ptId = start; ptId < end; ++ptId)
      {
        int ownerPart = VTK_INT_MAX;
        double pt[3];
        dataset->GetPoint(ptId, pt);
        for (int cc = 0; cc < cuts.size(); ++cc)
        {
          if (cuts[cc].ContainsPoint(pt))
          {
            ownerPart = std::min(ownerPart, cc);
          }
        }

        // save point ownership
        pt_ownership->SetTypedComponent(ptId, 0, ownerPart);
      }
    });
  }

  return vtkDIYKdTreeUtilities::GenerateGlobalPointIds(
    pieces, POINT_OWNERSHIP_ARRAYNAME, this->Controller);
}
...
//----------------------------------------------------------------------------
bool vtkRedistributeDataSetFilter::MarkGhostCells(vtkPartitionedDataSet* pieces)
{
  for (unsigned int partId = 0; partId < pieces->GetNumberOfPartitions(); ++partId)
  {
    vtkDataSet* dataset = pieces->GetPartition(partId);
...
void vtkRedistributeDataSetFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "GenerateGlobalPointIds: " << this->GenerateGlobalPointIds << endl;
