...
void vtkRedistributeDataSetFilter::SetExplicitCuts(const std::vector<vtkBoundingBox>& boxes)
{
  ...
  if (!native)
  {
    vtkErrorMacro("Strategy set is not native");
    return;
  }
  ...
  vtkIdType* mb_offset)
{
  ...
  for (auto& ds : input_partitions)
  {
    ...
    {
      if (curOutput->GetNumberOfPartitions() != info[*ptdOffset + inputPartId].NumberOfPartitions)
      {
  ...
  *ptdOffset += input_partitions.size();
  ...
  vtkPartitionedDataSet* outputPDS, const vtkPartitioningStrategy::PartitionInformation& info)
{
  ...
  if (parts->GetNumberOfPartitions() != info.NumberOfPartitions)
  {
