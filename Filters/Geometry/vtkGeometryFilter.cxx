...
public:
  ...
  template <typename T1, typename T2, typename TCellIdType>
  void InsertNextCell(T1 npts, const T2* pts, TCellIdType cellId)
  {
...
template <typename TInputIdType, typename TOffsetsValueType, typename TConnectivityValueType>
void ExtractCellGeometry(vtkUnstructuredGridBase* input, vtkIdType cellId, int cellType,
  TOffsetsValueType npts, const TConnectivityValueType* pts, int faceId,
...
struct ExtractUG : public ExtractCellBoundaries<TInputIdType>
{
  ...
  struct FaceOperator
  {
    ...
    void operator()(CellStateT& state, ExtractUG* This, vtkIdType beginHash, vtkIdType endHash)
    {
      ...
      const ConnectivityValueType* connectivityPtr = state.GetConnectivityRange().data();
      ...
      for (vtkIdType hash = beginHash; hash < endHash; ++hash)
      {
        ...
        for (localFaceId = 0; localFaceId < numberOfFaces; ++localFaceId)
        {
          ...
          if (!This->CellVis || This->CellVis[cellId])
          {
            ...
            const OffsetsValueType npts = offsets[cellId + 1] - offsets[cellId];
            const ConnectivityValueType* pts = connectivityPtr + offsets[cellId];
  ...
  void FaceOperator(vtkIdType faceBeginCellId, vtkIdType faceEndCellId)
  {
    ...
    for (vtkIdType faceCellId = faceBeginCellId; faceCellId < faceEndCellId; ++faceCellId)
    {
      ...
      if (!this->ForceSimpleVisibilityCheck || this->Input->IsCellVisible(cellId))
      {
        ...
        polys.template InsertNextCell<TInputIdType, TInputIdType>(4, face.data(), cellId);
      }
    }
  }
