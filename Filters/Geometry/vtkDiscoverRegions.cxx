...
struct VertexEdgeUse
{
  ...
  bool
    Orientation; // true => face uses positive edge sense for its positive normal side. false => otherwise
...
{
  os << " hood("
     << "bpt" << hood.BasePoint << " x " << hood.XAxis << " Bordants [";
  for (std::vector<VertexEdgeUse>::iterator it = hood.Bordants.begin(); it != hood.Bordants.end();
       ++it)
  {
...
{
  os << " hood("
     << "bpt " << hood.BasePoint << " dir " << hood.Direction << " x " << hood.XAxis << " y "
     << hood.YAxis << " Bordants [";
  for (std::vector<EdgeFaceUse>::iterator it = hood.Bordants.begin(); it != hood.Bordants.end();
       ++it)
  {
    os << " cell " << it->CellId << " or " << it->Orientation << " ang " << it->Angle;
  }
  ...
  vtkIdType /*edge*/, bool sense, Edgerhoods::iterator hood, vtkPoints* pts, double /*tol*/)
{
  ...
  if (hood->second.Bordants.empty())
  {
    ...
    vtkMath::Cross(
      hood->second.Direction.GetData(), hood->second.XAxis.GetData(), hood->second.YAxis.GetData());
...
int SpliceEdgeIntoVertex(vtkIdType edgeId, vtkIdType& idA, vtkIdType& idB, bool sense,
  vtkPoints* pts, Vertexhoods& hoods, double tol)
{
  ...
  else
  {
    ...
    double angle = atan2(yAxis.Dot(edgeDir), srch.first->second.XAxis.Dot(edgeDir));
...
static void BuildNeighborhoods(vtkPolyData* pd, H& hoods)
{
  ...
  for (cells->InitTraversal(); cells->GetNextCell(npts, conn); ++cellId)
  {
    ...
    if (distinctVertsCriterion < dim)
    {
      vtkGenericWarningMacro(<< "Cell " << cellId << " has only " << distinctVertsCriterion
                             << " distinct vertices");
    }
  }
}
...
struct RegionTracker
{
  ...
  void AddContainmentRelation(
    vtkIdType cellOnShell, bool cellSense, vtkIdType cellOnContainer, bool containerSense)
  {
  ...
  void CollapseContainmentRelationships()
  {
    ...
    for (vtkIdType i = 0; i < numRelations; ++i)
    {
      ...
      cout
        ...
        << "\n";
        */
      containedRegions[i] = this->Collapse[this->Sets.Find(
        this->ModelRegions->GetValue(3 * containedCellIds[i] + (containedSense[i] ? 2 : 1)))];
      containerRegions[i] = this->Collapse[this->Sets.Find(
        this->ModelRegions->GetValue(3 * containerCellIds[i] + (containerSense[i] ? 2 : 1)))];
    }
  }
  ...
  {
    vtkIdType entry[3];
    for (std::map<vtkIdType, vtkIdType>::iterator it = this->ModelMap.begin();
         it != this->ModelMap.end(); ++it)
    {
      this->ModelRegions->GetTypedTuple(it->second, entry);
      vtkIdType r0 = this->Sets.Find(entry[1]);
      vtkIdType r1 = this->Sets.Find(entry[2]);
      cout << "  Face " << it->first << " facet " << it->second << " bounds regions " << r0 << ","
           << r1 << "\n";
    }
  }
...
struct vtkDiscoverRegionsFacetFromCellArray
{
  ...
  vtkIdTypeArray* GetFaceGroupArray() { return this->Array; }
  bool AreOversubscribedFacetsPossible(vtkIdTypeArray* other) const { return other != this->Array; }
  T* Array;
};
...
void MergeRegions(vtkPolyData* pdIn, RegionTracker<N>& regions, const T& cell2facet)
{
  ...
  typename N::iterator it;
  for (it = regions.CellNeighborhoods.begin(); it != regions.CellNeighborhoods.end(); ++it)
  {
    ...
    for (vtkIdType f = 0; f < numFaces; ++f)
    {
      ...
      vtkIdType materialA = regions.ModelRegions->GetValue(
        modelRegionA * 3 + (it->second.Bordants[f].Orientation ? 2 : 1));
      vtkIdType materialB = regions.ModelRegions->GetValue(
        modelRegionB * 3 + (it->second.Bordants[(f + 1) % numFaces].Orientation ? 1 : 2));
      /*
      cout << "  merge "
...
struct RayHitRecord
{
  ...
  bool operator<(const RayHitRecord& other) const { return this->T < other.T; }
};
...
template <typename T, typename N>
bool IntersectWithRay(vtkPolyData* pdIn, RegionTracker<N>& regions, T& cell2facet, vec3d& basePoint,
  const vec3d& direction, RayHitRecords& hits, vtkIdType except = -1)
{
  bool allHitsOk = true;
  int dim = N::Dimension;
  hits.clear();
...
static bool ContainingShellFromHits(RayHitRecords& hits, vtkIdType& region, vtkIdType& hitCell,
  bool& hitSense, bool returnInwardShell)
{
  //cout << "--\n";
...
template <typename T, typename N>
void FindPointsInRegions(vtkPolyData* pdIn, RegionTracker<N>& regions, T& cell2facet,
...
template <typename T, typename N>
vtkIdType FindRegionContainingPoint(vtkPolyData* pdIn, vtkVector3d& pt, RegionTracker<N>& regions,
  T& cell2facet, vtkMinimalStandardRandomSequence* rnd)
{
...
template <typename T, typename N>
vtkIdType DiscoverNestings(vtkPolyData* pdIn, RegionTracker<N>& regions, T& cell2facet)
{
  ...
  for (shellIt = shells.begin(); shellIt != shells.end(); ++shellIt)
  {
    ...
    bool containerSense =
      false; // Whether the containerCell was hit on its positive (true) or negative (false) side.
    int numTries = 0;
    // Perturb the normal at least 5 times and repeat until
    // over half the container answers agree. Or we've tried 15
    // times, which is a local approximation to an infinite loop.
    while (numTries < 5 ||
      ((!containerCount.empty() && containerCount[container] / numTries < 0.5) && numTries < 15))
    {
      // Perturb the unit normal by at most a half-unit vector.
      // This prevents the normal from disappearing, or
      // pointing into the solid, or along a tangent to the solid.
...
struct ModelFacetSplitRec
{
  ...
  {
    return (this->OriginalFacet < other.OriginalFacet
        ? true
        : (this->OriginalFacet > other.OriginalFacet
              ? false
              : (this->Regions[0] < other.Regions[0]
                    ...
                    : (this->Regions[0] > other.Regions[0]
                          ? false
                          : (this->Regions[1] < other.Regions[1]
                                ? true
                                : (this->Regions[1] > other.Regions[1] ? false : false))))));
  }
};
...
public:
  ...
  {
    std::pair<ModelFacetSplitRec, vtkIdType> rec(
      ModelFacetSplitRec(orig, r0, r1), this->NextModelFacet);
...
void ReconcileModelFacets(
  vtkPolyData* pdIn, const char* modelFaceArrayName, RegionTracker<N>& regions, T& cell2facet)
{
  ...
  for (vtkIdType i = 0; i < numCellRegions; ++i)
  {
    ...
    vtkIdType revisedModelFace = modelFaceSplitter.SplitFaceByRegion(
      currentModelFace, regions.Sets.Find(regionIds[1]), regions.Sets.Find(regionIds[2]));
...
template <typename T, typename N>
void AssignHoles(vtkPolyData* pdIn, vtkPoints* holePoints, RegionTracker<N>& regions, T& cell2facet,
  vtkIntArray* isAHole)
{
  ...
  for (vtkIdType i = 0; i < numHolePts; ++i)
  {
    ...
    vtkIdType region = FindRegionContainingPoint(pdIn, h, regions, cell2facet, rnd.GetPointer());
  ...
  T& cell2facet)
{
  ...
  {
    AssignHoles(pdIn, holePtsIn, regions, cell2facet, holeRegions.GetPointer());
  }
  ...
  if (regionPtsIn)
  {
    ...
    for (vtkIdType i = 0; i < numRegionPts; ++i)
    {
      ...
      vtkIdType region =
        FindRegionContainingPoint(pdIn, regionPt, regions, cell2facet, rnd.GetPointer());
  ...
  if (regions.RegionPoints)
  {
    vtkIdTypeArray* regionIds =
      vtkIdTypeArray::SafeDownCast(regions.RegionPoints->GetPointData()->GetArray("Region"));
...
template <typename T, typename N>
void PrepareOutput(vtkPolyData* pdIn, RegionTracker<N>& regions, T& cell2facet,
  const char* faceGroupArrayName, vtkPolyData* pdOut, vtkTable* tabOut)
{
  ...
  if (regions.ReconciledModelRegions && regions.ReconciledModelRegions->GetNumberOfTuples() > 0)
  {
  ...
  for (int i = 0; i < 2; ++i)
  {
    cellRegions->SetComponentName(i, regions.ModelRegions->GetComponentName(i + 1));
  }
...
template <typename T, typename N>
int DiscoverRegions(vtkPolyData* pdIn, vtkPolyData* holesIn, vtkPolyData* regionPtsIn,
  ...
  vtkPolyData* pdOut, vtkTable* tabOut, T& cell2facet, RegionTracker<N>& regions)
{
  ...
  AssignRegionIDsHolesAndAttributes(pdIn, holePointsIn, regionPtsIn,
    regionGroupArrayName ? regionGroupArrayName : "", regions, cell2facet);
...
int vtkDiscoverRegions::FillInputPortInformation(int port, vtkInformation* info)
{
...
int vtkDiscoverRegions::FillOutputPortInformation(int port, vtkInformation* info)
{
  if (port == 1)
  {
  ...
  switch (dimension)
  {
    case 2:
    {
      ...
      if (mapArray)
      {
        ...
        status = DiscoverRegions(pdIn, holesIn, regionPtsIn, this->RegionGroupArrayName,
          this->FaceGroupArrayName, this->ModelFaceArrayName, pdOut, tabOut, cell2facet,
          regionTracker);
      }
      else
