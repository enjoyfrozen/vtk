...
bool EvaluateBasisFunctions(vtkCellGrid* grid, vtkDGCell* dgCell)
{
  ...
  for (vtkIdType ii = 0; ii < nn; ++ii)
  {
    ...
    test(nearly_eq(params[0], rval[0]) &&
        nearly_eq(params[1], rval[1]) &&
...
bool TestDeRhamBases(vtkCellGrid* grid, vtkDeRhamCell* drCell)
{
  ...
  {
    vtkDGOperation<vtkDGArraysInputAccessor, vtkDGArrayOutputAccessor> fieldEvaluator(drCell, divField, "Basis"_token);
  ...
  for (const auto& curlField : curlFields)
  {
    ...
    vtkDGOperation<vtkDGArraysInputAccessor, vtkDGArrayOutputAccessor> fieldEvaluator(drCell, curlField, "Basis"_token);
    inIt.Restart();
