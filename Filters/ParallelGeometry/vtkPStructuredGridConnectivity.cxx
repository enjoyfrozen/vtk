...
//------------------------------------------------------------------------------
void vtkPStructuredGridConnectivity::PrintSelf(std::ostream& os, vtkIndent indent)
{
  ...
  vtkPointData* pointData, vtkCellData* cellData, vtkPoints* gridNodes)
{
  ...
  this->Superclass::RegisterGrid(
    gridID, extents, nodesGhostArray, cellGhostArray, pointData, cellData, gridNodes);
...
void vtkPStructuredGridConnectivity::RegisterRemoteGrid(
  int gridID, int extents[6], int process)
{
  // Sanity check
  assert("pre: gridID out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->GridRanks.size())));

  // NOTE: remote grids only have their extents since that information is
  // required to determine neighboring.
  this->Superclass::RegisterGrid(gridID, extents, nullptr, nullptr, nullptr, nullptr, nullptr);
...
void vtkPStructuredGridConnectivity::ComputeNeighbors()
{
  ...
  assert("pre: Null multi-process controller" && (this->Controller != nullptr));
...
void vtkPStructuredGridConnectivity::CreateGhostLayers(int N)
{
  ...
  if (N == 0)
  {
    vtkWarningMacro("N=0 ghost layers requested! No ghost layers will be created!");
    this->Controller->Barrier();
    return;
  }
  ...
  int gridIdx, int nei, const vtkStructuredNeighbor& Neighbor)
{
  ...
  assert("pre: Neighbor grid ID is out-of-bounds!" && (Neighbor.NeighborID >= 0) &&
  ...
  vtkStructuredData::GetCellExtentFromPointExtent(GhostedGridExtent, GhostedGridCellExtent);
    ...
    for (int j = Neighbor.RcvExtent[2]; j <= Neighbor.RcvExtent[3]; ++j)
    {
      for (int k = Neighbor.RcvExtent[4]; k <= Neighbor.RcvExtent[5]; ++k)
      {
        ...
        if (this->HasPoints(gridIdx))
        {
          ...
          vtkIdType srcIdx =
            vtkStructuredData::ComputePointIdForExtent(const_cast<int*>(Neighbor.RcvExtent), ijk);
          ...
          this->CopyCoordinates(
            this->RemotePoints[gridIdx][nei], srcIdx, this->GhostedGridPoints[gridIdx], targetIdx);
        ...
        if (this->HasPointData(gridIdx))
        {
          ...
          this->CopyFieldData(this->RemotePointData[gridIdx][nei], srcIdx,
            this->GhostedGridPointData[gridIdx], targetIdx);
        } // END if has remote point data

        if (this->HasCellData(gridIdx) && this->IsNodeWithinExtent(i, j, k, RcvCellExtent))
        {
          ...
          // and not a point ID.
          vtkIdType sourceCellIdx = vtkStructuredData::ComputePointIdForExtent(RcvCellExtent, ijk);

          // Compute the target cell index. Note, since we are passing a cell
          // extent to ComputePointIdForExtent, the result will be a cell ID
          // and not a point ID.
          vtkIdType targetCellIdx = vtkStructuredData::ComputePointIdForExtent(
            GhostedGridCellExtent, ijk, this->DataDescription);

          // Transfer the cell data
          this->CopyFieldData(this->RemoteCellData[gridIdx][nei], sourceCellIdx,
...
void vtkPStructuredGridConnectivity::TransferGhostDataFromNeighbors(int gridID)
{
  ...
  assert(
    "pre: Neighbors are not properly allocated" && (this->NumberOfGrids == this->Neighbors.size()));
  ...
  for (int nei = 0; nei < NumNeis; ++nei)
  {
    ...
    {
      this->TransferRemoteNeighborData(gridID, nei, this->Neighbors[gridID][nei]);
    }
...
void vtkPStructuredGridConnectivity::PackGhostData()
{
  ...
  assert(
    "pre: RcvBuffers is not properly allocated!" && this->RcvBuffers.size() == this->NumberOfGrids);

  for (unsigned int idx = 0; idx < this->GridIds.size(); ++idx)
  {
    int gridIdx = this->GridIds[idx];
    assert("ERROR: grid index is out-of-bounds!" && (gridIdx >= 0) &&
      (gridIdx < static_cast<int>(this->NumberOfGrids)));

    int NumNeis = this->GetNumberOfNeighbors(gridIdx);
    this->SendBuffers[gridIdx].resize(NumNeis, nullptr);
    this->RcvBuffers[gridIdx].resize(NumNeis, nullptr);
    ...
    for (int nei = 0; nei < NumNeis; ++nei)
    {
      ...
      assert("ERROR: neighbor grid index is out-of-bounds" && (neiGridIdx >= 0) &&
      ...
      if (this->IsGridRemote(neiGridIdx))
      {
        ...
        this->SerializeGhostData(gridIdx, neiGridIdx, this->Neighbors[gridIdx][nei].SendExtent,
          this->SendBuffers[gridIdx][nei], this->SendBufferSizes[gridIdx][nei]);
...
//------------------------------------------------------------------------------
void vtkPStructuredGridConnectivity::SerializeBufferSizes(int*& sizesbuf, vtkIdType& N)
{
  assert("pre: sizes buffer must be nullptr" && (sizesbuf == nullptr));
  assert("pre: Number of sends should be at least 1" && (this->TotalNumberOfSends >= 1));
  ...
  N = 3 * k;
  sizesbuf = new int[N];
  assert("pre: Cannot allocate sizes buffer" && (sizesbuf != nullptr));
  ...
  int* buffersizes, vtkIdType N, int vtkNotUsed(processId))
{
  ...
  for (int i = 0; i < NumTuples; ++i)
  {
    ...
    if (this->IsGridLocal(rcvGrid))
    {
      ...
      assert("ERROR: rcver grid is out-of-bounds!" && (rcvGrid >= 0) &&
        (rcvGrid < static_cast<int>(this->NumberOfGrids)));
      assert("ERROR: neighbor index is out-of-bounds!" && (neiIndex >= 0) &&
...
void vtkPStructuredGridConnectivity::ExchangeBufferSizes()
{
  ...
  assert("ERROR: sizesbuffer is nullptr!" && (sizesbuffer != nullptr));
  ...
  assert("pre: receive buffer should not be nullptr" && (rcvbuffer != nullptr));
  ...
  for (int i = 0; i < numRanks; ++i)
  {
    if (i != this->Rank)
    {
      this->DeserializeBufferSizesForProcess(rcvbuffer + offSet[i], rcvcounts[i], i);
    } // END if remote rank
  }   // END for all ranks
...
void vtkPStructuredGridConnectivity::UnpackGhostData()
{
  ...
  for (unsigned int idx = 0; idx < this->GridIds.size(); ++idx)
  {
    ...
    for (int nei = 0; nei < NumNeis; ++nei)
    {
      ...
      if (this->IsGridRemote(neiGridIdx))
      {
        this->DeserializeGhostData(gridIdx, nei, neiGridIdx,
          this->Neighbors[gridIdx][nei].RcvExtent, this->RcvBuffers[gridIdx][nei],
...
void vtkPStructuredGridConnectivity::PostReceives()
{
  ...
  vtkMPIController* myMPIController = vtkMPIController::SafeDownCast(this->Controller);
  assert("pre: Cannot acquire MPI controller" && (myMPIController != nullptr));
  ...
  for (unsigned int idx = 0; idx < this->GridIds.size(); ++idx)
  {
    ...
    for (size_t nei = 0; nei < NumNeis; ++nei)
    {
      ...
      if (this->IsGridLocal(neiGridIdx))
      {
        // The neighboring grid is local, thus, the ghost data are transferred
      ...
      assert("pre: RcvBuffer must be nullptr!" && (this->RcvBuffers[gridIdx][nei] == nullptr));

      this->RcvBuffers[gridIdx][nei] = new unsigned char[this->RcvBufferSizes[gridIdx][nei]];
      assert("ERROR: Could not allocate RcvBuffer!" && this->RcvBuffers[gridIdx][nei]);
      assert("pre: RequestIndex is out-of-bounds!" && (rqstIdx >= 0) &&
        (rqstIdx < this->TotalNumberOfMsgs));
...
void vtkPStructuredGridConnectivity::PostSends()
{
  ...
  for (unsigned int idx = 0; idx < this->GridIds.size(); ++idx)
  {
    ...
    for (int nei = 0; nei < NumNeis; ++nei)
    {
      ...
      assert("pre: RcvBuffer must be nullptr!" && (this->SendBuffers[gridIdx][nei] != nullptr));
      assert("pre: RequestIndex is out-of-bounds!" && (rqstIdx >= 0) &&
        (rqstIdx < this->TotalNumberOfMsgs));
...
void vtkPStructuredGridConnectivity::CommunicateGhostData()
{
  ...
  assert("pre: RcvBufferSizes is not properly allocated" &&
    (this->RcvBufferSizes.size() == this->NumberOfGrids));
  assert("pre: Neighbors have not been computed!" && this->Neighbors.size() == this->NumberOfGrids);
  assert("pre: MPI requests array must be nullptr!" && (this->MPIRequests == nullptr));

  // STEP 1: Allocate the MPI requests array
  this->MPIRequests = new vtkMPICommunicator::Request[this->TotalNumberOfMsgs];
  assert("pre:Could not alloc MPI requests array" && (this->MPIRequests != nullptr));
...
void vtkPStructuredGridConnectivity::ExchangeGhostData()
{
  ...
  for (unsigned int i = 0; i < this->NumberOfGrids; ++i)
  {
    this->RemotePoints[i].resize(this->GetNumberOfNeighbors(i), nullptr);
    this->RemotePointData[i].resize(this->GetNumberOfNeighbors(i), nullptr);
    this->RemoteCellData[i].resize(this->GetNumberOfNeighbors(i), nullptr);
  }
  ...
  int gridIdx, int ext[6], vtkMultiProcessStream& bytestream)
{
  assert("pre: gridIdx is out-of-bounds" && (gridIdx >= 0) &&
    (gridIdx < static_cast<int>(this->NumberOfGrids)));
  assert(
    "pre: GridPoints is not properly allocated" && this->GridPoints.size() == this->NumberOfGrids);
  ...
  assert("Cannot allocate temporary pnts array" && (pnts != nullptr));
  ...
  for (int i = ext[0]; i <= ext[1]; ++i)
  {
    for (int j = ext[2]; j <= ext[3]; ++j)
    {
      ...
      {
        assert(
          "pre: IJK must be within grid extent!" && this->IsNodeWithinExtent(i, j, k, GridExtent));
        ...
        vtkIdType sourceIdx = vtkStructuredData::ComputePointIdForExtent(GridExtent, ijk);
        assert("pre: sourceIdx is out-of-bounds" && (sourceIdx >= 0) &&
          (sourceIdx < this->GridPoints[gridIdx]->GetNumberOfPoints()));

        this->GridPoints[gridIdx]->GetPoint(sourceIdx, x);

        // Compute the target index
        vtkIdType targetIdx = vtkStructuredData::ComputePointIdForExtent(ext, ijk, dataDescription);
        assert("pre: targetIdx is out-of-bounds" && (targetIdx >= 0) && (targetIdx < N));
  ...
  int gridIdx, int nei, int ext[6], vtkMultiProcessStream& bytestream)
{
  ...
  assert("pre:Neighbor list index is out-of-bounds" && (nei >= 0) &&
    (nei < this->GetNumberOfNeighbors(gridIdx)));
  ...
  assert("pre:Remote points for grid is not properly allocated!" &&
    (this->GetNumberOfNeighbors(gridIdx) == static_cast<int>(this->RemotePoints[gridIdx].size())));
  ...
  assert("pre: deserialize ghosts points array is nullptr" && (pnts != nullptr));
  assert("pre: points array is not of the expected size!" && (static_cast<int>(size) == 3 * N));
  ...
  for (int i = 0; i < N; ++i)
  {
    this->RemotePoints[gridIdx][nei]->SetPoint(i, pnts[i * 3], pnts[i * 3 + 1], pnts[i * 3 + 2]);
  }
...
{
  assert("pre: Cannot serialize a nullptr data array" && (dataArray != nullptr));
...
void vtkPStructuredGridConnectivity::DeserializeDataArray(vtkDataArray*& dataArray,
  int dataType, int numberOfTuples, int numberOfComponents,
  ...
  switch (dataType)
  {
    case VTK_FLOAT:
    {
      ...
      for (int i = 0; i < numberOfTuples; ++i)
      {
        ...
        {
          dataArrayPtr[i * numberOfComponents + j] = data[i * numberOfComponents + j];
...
{
  assert("pre: FieldData is nullptr" && (fieldData != nullptr));
  ...
  for (int array = 0; array < fieldData->GetNumberOfArrays(); ++array)
  {
    ...
    assert("pre: attempting to serialize a nullptr array!" && (myArray != nullptr));
    ...
    vtkDataArray* ghostArray = vtkDataArray::CreateDataArray(myArray->GetDataType());
    ...
    for (int i = ext[0]; i <= ext[1]; ++i)
    {
      for (int j = ext[2]; j <= ext[3]; ++j)
      {
        for (int k = ext[4]; k <= ext[5]; ++k)
        {
          ...
          // be a cell index if the incoming GridExtent and ext are cell extents.
          vtkIdType targetIdx = vtkStructuredData::ComputePointIdForExtent(ext, ijk);
  ...
  int* vtkNotUsed(ext), vtkFieldData* fieldData, vtkMultiProcessStream& bytestream)
{
  ...
  assert("ERROR: number of arrays must be greater or equal to 1" && (NumberOfArrays >= 1));
  ...
  for (int i = 0; i < NumberOfArrays; ++i)
  {
    ...
    this->DeserializeDataArray(dataArray, dataType, numTuples, numComponents, bytestream);
    assert("ERROR: data array should not be nullptr!" && (dataArray != nullptr));
  ...
  int gridIdx, int ext[6], vtkMultiProcessStream& bytestream)
{
  ...
  bytestream << 1;
  this->SerializeFieldData(GridExtent, ext, this->GridPointData[gridIdx], bytestream);
}
  ...
  int gridIdx, int ext[6], vtkMultiProcessStream& bytestream)
{
  ...
  bytestream << 1;
  this->SerializeFieldData(GridCellExtent, cellExtent, this->GridCellData[gridIdx], bytestream);
}
  ...
  int gridIdx, int nei, int ext[6], vtkMultiProcessStream& bytestream)
{
  ...
  this->DeserializeFieldData(cellext, this->RemoteCellData[gridIdx][nei], bytestream);
}

//------------------------------------------------------------------------------
void vtkPStructuredGridConnectivity::SerializeGhostData(int sendGridID, int rcvGrid,
  int sndext[6], unsigned char*& buffer, unsigned int& size)
{
  ...
  assert("pre: Receive grid should not be local" && !this->IsGridLocal(rcvGrid));
  assert("pre: sendGridID out-of-bounds!" &&
    (sendGridID >= 0 && sendGridID < static_cast<int>(this->NumberOfGrids)));
  assert("pre: rcvGridID is out-of-bounds!" && (rcvGrid >= 0) &&
    rcvGrid < static_cast<int>(this->NumberOfGrids));
  ...
  // Post-conditions
  assert("post: buffer should not be nullptr!" && (buffer != nullptr));
  ...
  int vtkNotUsed(neiGridIdx), int rcvext[6], unsigned char* buffer, unsigned int size)
{
  ...
  assert("pre: Serialized receiver grid must match this grid instance" && (rcvGrid == gridID));
...
void vtkPStructuredGridConnectivity::ExchangeGridExtents()
{
  ...
  assert("pre: buffer != nullptr" && (buffer != nullptr));
  assert("pre: N > 0" && (N > 0));
...
//------------------------------------------------------------------------------
void vtkPStructuredGridConnectivity::SerializeGridExtents(int*& sndbuffer, vtkIdType& N)
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
  ...
  int* rcvbuffer, vtkIdType& N, int processId)
{
  // Sanity checks
  assert("pre: Instance has not been initialized!" && this->Initialized);
  assert("pre: Process controller should not be nullptr!" && (this->Controller != nullptr));
  assert("pre: rcvbuffer should not be nullptr" && (rcvbuffer != nullptr));
  assert("pre: must be called for a remote process" && (processId != this->Rank));
