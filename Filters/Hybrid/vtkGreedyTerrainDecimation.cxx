...
public:
  vtkTerrainInfo()
    : TriangleId(VTK_VERTEX_NO_TRIANGLE)
  {
  }
...
void vtkGreedyTerrainDecimation::UpdateTriangles(vtkIdType ptId)
{
  ...
  for (vtkIdType i = 0; i < ncells; i++)
  {
    ...
    this->UpdateTriangle(
      cells[i], (*this->PointInfo)[pts[0]], (*this->PointInfo)[pts[1]], (*this->PointInfo)[pts[2]]);
  }
}
...
int vtkGreedyTerrainDecimation::InCircle(double x[3], double x1[3], double x2[3], double x3[3])
{
...
vtkIdType vtkGreedyTerrainDecimation::FindTriangle(double x[3], vtkIdType ptIds[3], vtkIdType tri,
...
// triangle id.
void vtkGreedyTerrainDecimation::CheckEdge(
  vtkIdType ptId, double x[3], vtkIdType p1, vtkIdType p2, vtkIdType tri, int depth)
{
  if (depth > 15)
  {
    return;
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  this->NumberOfTriangles =
    (this->NumberOfTriangles < this->MaximumNumberOfTriangles ? this->NumberOfTriangles
                                                              : this->MaximumNumberOfTriangles);
...
//
void vtkGreedyTerrainDecimation::UpdateTriangle(
  vtkIdType tri, int ij1[2], int ij2[2], int ij3[2], double h[3])
{
  ...
  int type = this->CharacterizeTriangle(
    ij1, ij2, ij3, min, max, midL, midR, mid, mid2, h, hMin, hMax, hMidL, hMidR);
...
//
int vtkGreedyTerrainDecimation::CharacterizeTriangle(int ij1[2], int ij2[2], int ij3[3], int*& min,
  int*& max, int*& midL, int*& midR, int*& mid, int mid2[2], double h[3], double& hMin,
  double& hMax, double& hL, double& hR)
{
