...
public:
  ...
  vtkGridTransformConnectionHolder() { this->SetNumberOfInputPorts(1); }
};
  ...
  int gridType, int gridExt[6], vtkIdType gridInc[3])
{
  ...
  vtkIdType increment = gridId[0] * gridInc[0] + gridId[1] * gridInc[1] + gridId[2] * gridInc[2];
...
template <class T>
inline void vtkNearestHelper(double displacement[3], double derivatives[3][3], T* gridPtr,
  int gridId[3], int gridId0[3], int gridId1[3], vtkIdType gridInc[3])
{
  ...
  double derivatives[3][3], void* gridPtr, int gridType, int gridExt[6], vtkIdType gridInc[3])
{
  ...
  {
    vtkNearestNeighborInterpolation(point, displacement, gridPtr, gridType, gridExt, gridInc);
    return;
  }
  ...
  switch (gridType)
  {
    vtkTemplateMacro(vtkNearestHelper(
      displacement, derivatives, static_cast<VTK_TT*>(gridPtr), gridId, gridId0, gridId1, gridInc));
  }
}
...
template <class T>
inline void vtkLinearHelper(double displacement[3], double derivatives[3][3], double fx, double fy,
  ...
  double derivatives[3][3], void* gridPtr, int gridType, int gridExt[6], vtkIdType gridInc[3])
{
  ...
  switch (gridType)
  {
    vtkTemplateMacro(vtkLinearHelper(displacement, derivatives, f[0], f[1], f[2],
      static_cast<VTK_TT*>(gridPtr), i000, i001, i010, i011, i100, i101, i110, i111));
  }
}
...
static void vtkSetTricubicInterpCoeffs(double F[4], int* l, int* m, double f, int interpMode)
{
  double fp1, fm1, fm2;
...
static void vtkSetTricubicDerivCoeffs(
...
template <class T>
inline void vtkCubicHelper(double displacement[3], double derivatives[3][3], double fx, double fy,
  double fz, T* gridPtr, int interpModeX, int interpModeY, int interpModeZ, vtkIdType factX[4],
  vtkIdType factY[4], vtkIdType factZ[4])
{
  ...
  double derivatives[3][3], void* gridPtr, int gridType, int gridExt[6], vtkIdType gridInc[3])
{
  ...
  switch (gridType)
  {
    vtkTemplateMacro(vtkCubicHelper(displacement, derivatives, f[0], f[1], f[2],
      static_cast<VTK_TT*>(gridPtr), interpModeX, interpModeY, interpModeZ, factX, factY, factZ));
  }
}
...
vtkMTimeType vtkGridTransform::GetMTime()
{
  ...
  {
    vtkAlgorithm* inputAlgorithm = this->ConnectionHolder->GetInputAlgorithm(0, 0);
  ...
  const double inPoint[3], double outPoint[3], double derivative[3][3])
{
  ...
  this->InterpolationFunction(ijkPoint, deltaP, nullptr, gridPtr, gridType, extent, increments);
  ...
  for (i = 0; i < n; i++)
  {
    ...
    this->InterpolationFunction(
      ijkPoint, deltaP, derivative, gridPtr, gridType, extent, increments);

    // convert displacement
    deltaP[0] = (inverse[0] - inPoint[0]) + deltaP[0] * scale + shift;
    deltaP[1] = (inverse[1] - inPoint[1]) + deltaP[1] * scale + shift;
    deltaP[2] = (inverse[2] - inPoint[2]) + deltaP[2] * scale + shift;
...
void vtkGridTransform::InternalUpdate()
{
  ...
  if (grid->GetScalarType() != VTK_CHAR && grid->GetScalarType() != VTK_UNSIGNED_CHAR &&
    grid->GetScalarType() != VTK_SHORT && grid->GetScalarType() != VTK_UNSIGNED_SHORT &&
    grid->GetScalarType() != VTK_FLOAT && grid->GetScalarType() != VTK_DOUBLE)
  {
