...
public:
  ...
  ~TemporalFractalOutputUtil() override
  {
    this->DataSets.clear();
    this->Levels.clear();
  }
  ...
  void CreateOutput(vtkMultiBlockDataSet* mbs)
  {
    for (size_t i = 0; i < this->DataSets.size(); i++)
    {
      ...
      vtkMultiBlockDataSet* block = vtkMultiBlockDataSet::SafeDownCast(mbs->GetBlock(level));
      if (!block)
      {
...
void vtkTemporalFractal::SetBlockInfo(vtkUniformGrid* grid, int level, int* ext, int onFace[6])
{
  // onFace[0]:xmin
  // onFace[1]:xmax
  // onFace[2]:ymin
...
int vtkTemporalFractal::RequestDataObject(
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  double timeStep = info->Get(vtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());
...
//------------------------------------------------------------------------------
int vtkTemporalFractal ::RequestOneTimeStep(vtkCompositeDataSet* output,
...
//------------------------------------------------------------------------------
int vtkTemporalFractal::LineTest2(
  float x0, float y0, float z0, float x1, float y1, float z1, double bds[6])
{
...
//------------------------------------------------------------------------------
int vtkTemporalFractal::LineTest(
  float x0, float y0, float z0, float x1, float y1, float z1, double bds[6], int level, int target)
{
  if (level >= target)
  {
    return 0;
...
//------------------------------------------------------------------------------
void vtkTemporalFractal::Traverse(int& blockId, int level, vtkDataObject* output, int x0, int x3,
  int y0, int y3, int z0, int z3, int onFace[6])
{
  double bds[6];
  ...
  { // 3D
    if (this->LineTest(
          -1.64662, 0.56383, 1.16369, -1.05088, 0.85595, 0.87104, bds, level, this->MaximumLevel) ||
      this->LineTest(
        -1.05088, 0.85595, 0.87104, -0.61430, 1.00347, 0.59553, bds, level, this->MaximumLevel))
...
void vtkTemporalFractal::AddFractalArray(vtkCompositeDataSet* output)
{
  ...
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
  {
    ...
    if (!this->GenerateRectilinearGrids)
    {
      ...
      fractalSource->SetOriginCX(origin[0] + (spacing[0] * 0.5), origin[1] + (spacing[1] * 0.5),
    ...
    {
      vtkRectilinearGrid* grid = vtkRectilinearGrid::SafeDownCast(iter->GetCurrentDataObject());
      assert("check: grid_exists" && grid != nullptr);
...
void vtkTemporalFractal::AddGhostLevelArray(vtkDataSet* grid, int dim[3], int onFace[6])
{
...
//------------------------------------------------------------------------------
void vtkTemporalFractal::CellExtentToBounds(int level, int ext[6], double bds[6])
{
  int spacingFactor = 1;
  double spacing[3];
...
//------------------------------------------------------------------------------
void vtkTemporalFractal::ExecuteRectilinearMandelbrot(vtkRectilinearGrid* grid, double* ptr)
{
  int a0 = 0;
  int a1 = 1;
  int a2 = 2;
...
//------------------------------------------------------------------------------
void vtkTemporalFractal::GetContinuousIncrements(
