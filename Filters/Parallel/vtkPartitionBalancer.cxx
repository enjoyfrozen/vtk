  ...
  vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  {
    numberOfNonNullPartitionsInInput += (inputPDS->GetPartitionAsDataObject(partitionId) != nullptr)
      ? 1 : 0;
  }

  if (!this->Controller)
  {
    outputPDS->SetNumberOfPartitions(numberOfNonNullPartitionsInInput);
    unsigned int outputPartitionId = 0;
    for (unsigned int partitionId = 0; partitionId < numberOfNonNullPartitionsInInput;
        ++partitionId)
    {
      if (vtkDataObject* dobj = inputPDS->GetPartitionAsDataObject(partitionId))
      {
        outputPDS->SetPartition(outputPartitionId++, dobj);
      }
    }
    return 1;
  }
