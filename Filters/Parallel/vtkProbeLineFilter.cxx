...
{
  double p12[3] = { this->Point2[0] - this->Point1[0], this->Point2[1] - this->Point1[1],
  ...
  double n1[3], n2[3];
  vtkMath::Cross(p12, v, n1);
  vtkMath::Normalize(n1);
  vtkMath::Cross(p12, n1, n2);
  ...
  for (std::size_t dsId = 0; dsId < inputs.size(); ++dsId)
  {
    ...
    // set, we can end up with a 2D plane output when we concatenate the 2 slices instead of a 1D
    // line.
