...
bool vtkPParticleTracerBase::UpdateParticleListFromOtherProcesses()
{
  ...
  for (size_t i = 0; i < received.size(); i++)
  {
    ...
    info.Current.TailPointId = info.Previous.TailPointId =
      static_cast<vtkIdType>(this->Tail.size());
...
bool vtkPParticleTracerBase::IsPointDataValid(vtkDataObject* input)
{
  ...
  if (this->Controller->GetLocalProcessId() == 0)
  {
    ...
    if (retVal == 1)
    {
      ...
      for (std::vector<std::string>::iterator it = arrayNames.begin(); it != arrayNames.end(); ++it)
      {
        stream << *it;
      }
    }
