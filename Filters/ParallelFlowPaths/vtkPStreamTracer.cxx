  ...
  vtkPolyData* traceOut, vtkAbstractInterpolatedVelocityField* func, PStreamTracerPoint* point)
{
  ...
  vtkAbstractInterpolatedVelocityField *simpleFunc = func->NewInstance();
  simpleFunc->CopyParameters(func);

  // Use Runge-Kutta2 it produces better results
  vtkNew<vtkRungeKutta2> integrator;
  integrator->SetFunctionSet(simpleFunc);

  memcpy(outPoint, lastPoint, sizeof(double) * 3);

  double timeStepTaken =
    this->SimpleIntegrate(nullptr, outPoint, this->LastUsedStepSize, integrator, simpleFunc);
