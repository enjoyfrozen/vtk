...
int vtkAMRInterpolatedVelocityField::SelfInitialize()
{
  ...
  if ( ds != nullptr )
  {
    vectors = ds->GetAttributesAsFieldData(this->VectorsType)->GetArray(this->VectorsSelection);
  }
  ...
  // is required. If no vectors specified, use the local dataset vectors.
  for (size_t cc=0; cc < datasets.size(); ++cc)
  {
    if ( ! vectors )
    {
      vectors = datasets[cc]->GetCellData()->GetArray(this->VectorsSelection);
    }
...
int vtkAMRInterpolatedVelocityField::FunctionValues(double* x, double* f)
{
  ...
  if (!ds || !this->FunctionValues(ds, x, f))
  {
...
void vtkAMRInterpolatedVelocityField::
CopyParameters(vtkAbstractInterpolatedVelocityField* from)
{
  this->Superclass::CopyParameters(from);

  vtkAMRInterpolatedVelocityField* obj =
    vtkAMRInterpolatedVelocityField::SafeDownCast(from);
  if (!obj)
  {
    return;
  }
