...
int TestStreaklineFilter()
{
  ...
  vtkCellArray* lines = out->GetLines();
  vtkNew<vtkIdList> polyLine;
  vtkFloatArray* particleAge =
    vtkArrayDownCast<vtkFloatArray>(out->GetPointData()->GetArray("ParticleAge"));
...
int TestParticleTracers(int, char*[])
{
  ...
  filter->Update();
  double data_time =
    filter->GetOutputDataObject(0)->GetInformation()->Get(vtkDataObject::DATA_TIME_STEP());
  int numRequestData = imageSource->GetNumRequestData();
