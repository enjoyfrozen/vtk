...
struct vtkFunctionCache
{
  ...
  vtkFunctionCache(vtkFindCellStrategy *strategy, vtkDataArray *vectors)
    : Strategy(strategy), Vectors(vectors) {}
};
...
//------------------------------------------------------------------------------
void vtkAbstractInterpolatedVelocityField::
Initialize(vtkCompositeDataSet* compDS, int initStrategy)
{
  ...
  for (size_t cc=0; cc < datasets.size(); ++cc)
  {
    ...
    this->FunctionCacheMap->insert(std::make_pair(datasets[cc], vtkFunctionCache(strategyClone,vectors)));
...
//------------------------------------------------------------------------------
int vtkAbstractInterpolatedVelocityField::
                FunctionValues(vtkDataSet* dataset, double* x, double* f)
{
  ...
  vtkAbstractInterpolatedVelocityField* from)
{
  ...
  // Copy the function cache, including strategies, from the prototype.
  // In a threaded situation, there must be separate strategies for
  // each interpolation field.
  ...
  for ( ; cIter != from->FunctionCacheMap->end(); ++cIter )
  {
    ...
    this->FunctionCacheMap->insert(std::make_pair(cIter->first, vtkFunctionCache(strategy,vectors)));
  }
}

//------------------------------------------------------------------------------
void vtkAbstractInterpolatedVelocityField::
AddToFunctionCache(vtkDataObject *ds, vtkFindCellStrategy *s, vtkDataArray *vectors)
{
