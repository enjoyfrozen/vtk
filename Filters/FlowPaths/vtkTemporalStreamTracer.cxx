  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  outInfo->Set(vtkStreamingDemandDrivenPipeline::TIME_STEPS(), &this->OutputTimeValues[0],
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
    ...
    !outInfo->Has(vtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP()))
  {
    ...
    vtkDebugMacro(<< "SetTimeStep       : requestedTimeValue " << requestedTimeValue
  ...
  else
  {
    ...
    vtkDebugMacro(<< "UPDATE_TIME_STEPS : requestedTimeValue " << requestedTimeValue
                  << " ActualTimeStep " << this->ActualTimeStep);
  }
...
int vtkTemporalStreamTracer::InitializeInterpolator()
{
  ...
  {
    vtkDebugMacro("Not enough inputs have been found. Can not execute."
      << numValidInputBlocks[0] << " " << numValidInputBlocks[1]);
    return VTK_ERROR;
  }
  if (numValidInputBlocks[0] != numValidInputBlocks[1])
  {
    vtkDebugMacro("The number of datasets is different between time steps "
      << numValidInputBlocks[0] << " " << numValidInputBlocks[1]);
    return VTK_ERROR;
  }
  //
...
int vtkTemporalStreamTracer::GenerateOutput(
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)

{
  ...
  this->UpdatePieceId = outInfo->Get(vtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
...
#define PASSES 2
  for (int pass = 0; pass < PASSES; pass++)
  {
    ...
    if (this->UpdateNumPieces > 1 && pass < (PASSES - 1))
    {
      ...
      vtkDebugMacro(<< "End of Pass " << pass << " with " << this->ParticleHistories.size() << " "
  ...
  double targettime, vtkInitialValueProblemSolver* integrator)
{
  ...
  {
    vtkDebugMacro(<< "Bad particle time : expected (" << this->CurrentTimeSteps[0] << "-"
...
/*
  ...
  if (this->Interpolator->GetLastValidCellId(0)!=-1) {
    ...
    {
      vtkDebugMacro(<< "SendParticleToAnotherProcess : Sending Particle " << particleId << " Time "
  << intersection[3]); this->AddParticleToMPISendList(intersection, velocity); vtkIdType nextPoint =
