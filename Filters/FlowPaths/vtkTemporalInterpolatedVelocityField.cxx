  ...
  const std::vector<vtkSmartPointer<vtkAbstractCellLinks>>& datasetLinks)
{
  ...
  for (size_t i = 0; i < datasets.size(); ++i)
  {
    ...
    if (vtkClosestPointStrategy::SafeDownCast(datasetInfo.Strategy))
    {
      ...
      {
        ugrid->SetLinks(links);
      }
      else if (auto polyData = vtkPolyData::SafeDownCast(datasetInfo.DataSet))
      {
        polyData->SetLinks(vtkCellLinks::SafeDownCast(links));
      }
    }
  }
}
  ...
  vtkCompositeDataSet* t0, vtkCompositeDataSet* t1)
{
  ...
  if (t0 == t1) // First time calling this method
  {
    ...
    this->CreateLocators(datasets[1], this->FindCellStrategy, this->Locators[1]);
    this->InitializeWithLocators(this->IVF[0], datasets[1], this->Locators[1], this->Links[1]);
    this->InitializeWithLocators(this->IVF[1], datasets[1], this->Locators[1], this->Links[1]);
  ...
  else // t0 != t1
  {
    ...
    switch (this->MeshOverTime)
    {
      case MeshOverTimeTypes::DIFFERENT:
        ...
        this->InitializeWithLocators(this->IVF[0], datasets[0], this->Locators[0], this->Links[0]);
        this->CreateLocators(datasets[1], strategy, this->Locators[1]);
        this->InitializeWithLocators(this->IVF[1], datasets[1], this->Locators[1], this->Links[1]);
        break;
      case MeshOverTimeTypes::STATIC:
        this->InitializeWithLocators(this->IVF[0], datasets[0], this->Locators[1], this->Links[1]);
        this->InitializeWithLocators(this->IVF[1], datasets[1], this->Locators[1], this->Links[1]);
        break;
      case MeshOverTimeTypes::LINEAR_TRANSFORMATION:
        this->Locators[0] = std::move(this->Locators[1]);
        this->InitializeWithLocators(this->IVF[0], datasets[0], this->Locators[0], this->Links[1]);
        if (vtkCellLocatorStrategy::SafeDownCast(strategy))
        {
  ...
  vtkTemporalInterpolatedVelocityField* from)
{
  ...
  std::copy_n(from->Vals1, 3, this->Vals1);
  std::copy_n(from->Vals2, 3, this->Vals2);
  std::copy_n(from->Times, 2, this->Times);
  std::copy_n(from->LastGoodVelocity, 3, this->LastGoodVelocity);
  this->CurrentWeight = from->CurrentWeight;
  this->OneMinusWeight = from->OneMinusWeight;
