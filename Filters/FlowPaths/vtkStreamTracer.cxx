...
int vtkStreamTracer::
CheckInputs(vtkAbstractInterpolatedVelocityField*& func, int* maxCellSize)
{
  if (!this->InputData)
  {
    return VTK_ERROR;
...
struct TracerOffset
{
  ...
  TracerOffset() : ThreadOutput(nullptr), ThreadPtId(-1), NumPts(0),
                   ...
                   StartingPtId(0), CellId(0), CellConnOffset(0) {}
};
...
struct TracerIntegrator
{
  ...
  TracerIntegrator(vtkStreamTracer *streamTracer, vtkCompositeDataSet *inputData,
                   bool matchingAttr, vtkDataSetAttributes *protoPD,
                   vtkDataArray *seedSource, vtkIdList *seedIds, vtkIntArray *intDirs, TracerOffsets &offsets,
                   ...
                   std::vector<void*> &customTerminationClientData,
                   std::vector<int> &customReasonForTermination) :
    StreamTracer(streamTracer), InputData(inputData), HasMatchingPointAttributes(matchingAttr),
    ProtoPD(protoPD), SeedSource(seedSource), SeedIds(seedIds), IntegrationDirections(intDirs), Offsets(offsets),
  ...
  void Initialize()
  {
    ...
    localOutput.Output->GetPointData()->
      InterpolateAllocate(this->ProtoPD, this->MaximumNumberOfSteps);
  }
  ...
  struct CompositeOverSeeds
  {
    ...
    CompositeOverSeeds(TracerOffsets &offsets, vtkPoints *outPoints, vtkIdType *caOffsetsPtr,
                       vtkIdType *caConnPtr, vtkPointData *outPD, vtkIdList *seedIds, int *seedIdsPtr,int *retValsPtr) :
      Offsets(offsets), OutPoints(outPoints), CAOffsets(caOffsetsPtr), CAConn(caConnPtr),
      OutPD(outPD), SeedIds(seedIds), OutSeedIds(seedIdsPtr), OutRetVals(retValsPtr)
    {}
  ...
  void Reduce()
  {
    ...
    CompositeOverSeeds comp(this->Offsets, outPoints,caOffsetsPtr,caConnPtr,
                            outputPD,this->SeedIds,seedIdsPtr,retValsPtr);
          ...
          std::vector<int> &customReasonForTermination)
{
  ...
  TracerIntegrator ti(this, this->InputData, this->HasMatchingPointAttributes, protoPD,
                      seedSource, seedIds, intDirs, offsets,
...
// than Integrate.
double vtkStreamTracer::
SimpleIntegrate(double seed[3], double lastPoint[3], double stepSize, vtkInitialValueProblemSolver* integrator,
...
void vtkStreamTracer::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Force Serial Execution: "
     << (this->ForceSerialExecution ? " On" : " Off") << endl;
}
