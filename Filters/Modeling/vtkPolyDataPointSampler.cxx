...
namespace
{
  ...
  struct GeneratePoints
  {
    ...
    GeneratePoints(vtkPolyDataPointSampler *self, double d,
                   vtkIdType numPts, vtkPoints *inPts, vtkPoints *newPts,
                   vtkPointData *inPD, vtkPointData *outPD,
                   vtkCellArray *inLines, vtkCellArray *inPolys,
                   vtkCellArray *inStrips) :
      Self(self), Distance(d), NumPts(numPts), InPts(inPts), InPD(inPD),
      InLines(inLines), InPolys(inPolys), InStrips(inStrips), OutPts(newPts),
      OutPD(outPD)
    {
  ...
  {
    RegularGeneration(vtkPolyDataPointSampler *self, double d,
                      vtkIdType numPts, vtkPoints *inPts, vtkPoints *newPts,
                      vtkPointData *inPD, vtkPointData *outPD,
                      vtkCellArray *inLines, vtkCellArray *inPolys,
                      vtkCellArray *inStrips) :
      GeneratePoints(self, d, numPts, inPts, newPts, inPD, outPD,
                     inLines, inPolys, inStrips)
    {
    }
    ...
    void SamplePolygon(vtkIdType npts, const vtkIdType* pts) override
    {
      ...
      else
      {
        ...
        for (vtkIdType i = 0; i < (npts - 2); ++i)
        {
          ...
          triPts[2] = pts[i + 2];
          if ( Self->GetGenerateEdgePoints() &&
               this->EdgeTable->IsEdge(triPts[0], triPts[2]) == -1)
          {
  ...
  struct RandomGeneration : public GeneratePoints
  {
    ...
    RandomGeneration(vtkPolyDataPointSampler *self, double d,
                     vtkIdType numPts, vtkPoints *inPts, vtkPoints *newPts,
                     vtkPointData *inPD, vtkPointData *outPD,
                     vtkCellArray *inLines, vtkCellArray *inPolys,
                     vtkCellArray *inStrips) :
    ...
    void SamplePoints() override
    {
      ...
      for (auto i=0; i < this->NumPts; ++i)
      {
        ranVal = this->RandomSeq->GetValue(); this->RandomSeq->Next();
        if ( ranVal <= frac )
        {//insert point
    ...
    void SampleEdge(vtkIdType p0, vtkIdType p1) override
    {
      ...
      if (len > this->Distance2)
      {
        ...
        for (auto i=0; i < npts; ++i)
        {
          t = this->RandomSeq->GetValue(); this->RandomSeq->Next();
          x[0] = x0[0] + t * (x1[0] - x0[0]);
          x[1] = x0[1] + t * (x1[1] - x0[1]);
          x[2] = x0[2] + t * (x1[2] - x0[2]);
          pId = this->OutPts->InsertNextPoint(x);
    ...
    void SampleTriangle(const vtkIdType* pts) override
    {
      ...
      if ( npts > 0 )
      {
        ...
        for (auto i=0; i < npts; ++i)
        {
          s = this->RandomSeq->GetValue(); this->RandomSeq->Next();
          t = this->RandomSeq->GetValue(); this->RandomSeq->Next();
          if ( (1.0 - s - t) >= 0.0 )
            ...
            vtkInformationVector* outputVector)
{
  ...
  {
    RegularGeneration gen(this, this->Distance, numPts, inPts, newPts,
                          inPD, outPD, inLines, inPolys, inStrips);
    abort = gen();
  }
  else //RANDOM_GENERATION
  {
    RandomGeneration gen(this, this->Distance, numPts, inPts, newPts,
                         inPD, outPD, inLines, inPolys, inStrips);
    abort = gen();
  }
