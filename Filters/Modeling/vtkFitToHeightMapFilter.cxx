...
struct FitPoints
{
  ...
  FitPoints(vtkIdType npts, TPoints *inPts, TPoints *outPts, TScalars *s,
            int dims[3], double o[3], double h[3]) :
    NPts(npts), InPoints(inPts), OutPoints(outPts), Scalars(s)
  {
    for (int i=0; i < 3; ++i)
    {
  ...
  static void Execute(vtkIdType numPts, TPoints *inPts, TPoints *points,
                      TScalars *s, int dims[3], double origin[3], double h[3])
  {
...
struct FitCells
{
  ...
  FitCells(int strat, vtkPolyData *mesh, double *cellHts,
           TScalars *s, int dims[3], double o[3], double h[3]) :
    Strategy(strat), Mesh(mesh), CellHeights(cellHts), Scalars(s)
  {
    for (int i=0; i < 3; ++i)
    {
      this->Dims[i] = static_cast<double>(dims[i]);
  ...
  static void Execute(int strategy, vtkPolyData *mesh,
                      double *cellHts, TScalars *s, int dims[3],
...
//----------------------------------------------------------------------------
int vtkFitToHeightMapFilter::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if ( inputBds[0] < imageBds[0] || inputBds[1] > imageBds[1] ||
       inputBds[2] < imageBds[2] || inputBds[3] > imageBds[3] )
  {
...
void vtkFitToHeightMapFilter::
SetHeightMapConnection(vtkAlgorithmOutput* algOutput)
{
...
//----------------------------------------------------------------------------
vtkImageData* vtkFitToHeightMapFilter::
GetHeightMap(vtkInformationVector *sourceInfo)
{
...
void vtkFitToHeightMapFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Use Height Map Offset: "
     << (this->UseHeightMapOffset ? "On\n" : "Off\n");
}
