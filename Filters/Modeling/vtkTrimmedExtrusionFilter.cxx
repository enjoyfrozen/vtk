...
struct ExtrudePoints
{
  ...
  ExtrudePoints(vtkIdType npts, T *inPts, T *points, unsigned char *hits,
    ...
    NPts(npts), InPoints(inPts), Points(points), Hits(hits), Locator(loc)
  {
  ...
  void operator() (vtkIdType ptId, vtkIdType endPtId)
  {
    ...
    for ( ; ptId < endPtId; ++ptId, xi+=3, x+=3, xo+=3, ++hits )
    {
      ...
      *hits = this->Locator->IntersectWithLine(p0,p1, this->Tol, t, xint,
                                               pc, subId, cellId, cell);
      if ( *hits > 0 )
      {
...
//-----------------------------------------------------------------------------
int vtkTrimmedExtrusionFilter::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  {
    vtkTemplateMacro(ExtrudePoints<VTK_TT>::Execute(numPts, (VTK_TT *)inPtr,
       (VTK_TT *)outPtr, hits, this->Locator, this->ExtrusionDirection, surfaceBds));
  }
...
//----------------------------------------------------------------------------
vtkIdType vtkTrimmedExtrusionFilter::
GetNeighborCount(vtkPolyData *input, vtkIdType inCellId,
...
void vtkTrimmedExtrusionFilter::
SetTrimSurfaceConnection(vtkAlgorithmOutput* algOutput)
{
...
void vtkTrimmedExtrusionFilter::
SetTrimSurfaceData(vtkPolyData *pd)
{
