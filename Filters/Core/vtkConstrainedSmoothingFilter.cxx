...
struct SmoothPoints
{
  ...
  void operator()(vtkIdType ptId, vtkIdType endPtId)
  {
    ...
    for (; ptId < endPtId; ++ptId)
    {
      ...
      {
        double xC[3]; xC[0]=xIn[0]; xC[1]=xIn[1]; xC[2]=xIn[2];
        double t, xInt[3];
        int plane;
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  else if (this->ConstraintStrategy == CONSTRAINT_DISTANCE || this->ConstraintStrategy == CONSTRAINT_BOX)
  {
...
void vtkConstrainedSmoothingFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Constraint Box: (" << this->ConstraintBox[0] << ", "
     << this->ConstraintBox[1] << ", " << this->ConstraintBox[2] << ")\n";
