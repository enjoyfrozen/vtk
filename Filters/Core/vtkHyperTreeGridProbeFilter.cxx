  ...
  vtkHyperTreeGrid* source, vtkDataSet* output, vtkIdType nanId)
{
  ...
  // Replace remaining invalid id values (not overriden durning reduction operation)
  // With "NaN index" (id pointing to the NaN value).
  // This id equals the total number of cells in the input source (from all nodes in distributed),
  // in order to not point to an existing cell id
  ...
  for (int i = 0; i < this->Handles->GetNumberOfIds(); i++)
  {
    if (this->Handles->GetId(i) != source->GetNumberOfCells())
    {
