  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  {
    case VTK_POLY_DATA:
      if ((npts = this->ConstructPoints(input, vtkPolyData::SafeDownCast(output))))
      {
    ...
    case VTK_UNSTRUCTURED_GRID:
      if (this->ConstructPoints(input, vtkUnstructuredGrid::SafeDownCast(output)))
      {
...
vtkDataSet* vtkDataObjectToDataSetFilter::GetOutput()
{
  ...
  return vtkDataSet::SafeDownCast(this->GetExecutive()->GetOutputData(0));
}
...
vtkIdType vtkDataObjectToDataSetFilter::ConstructPoints(vtkDataObject* input, vtkPointSet* ps)
{
  ...
  else // have to copy data into created array
  {
    ...
    for (i = 0; i < 3; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newPts->GetData(), i, fieldArray[i],
  ...
  vtkDataObject* input, vtkRectilinearGrid* rg)
{
  ...
  else // have to copy data into created array
  {
    ...
    if (vtkFieldDataToAttributeDataFilter::ConstructArray(XPts, 0, fieldArray[0],
  ...
  else // have to copy data into created array
  {
    ...
    if (vtkFieldDataToAttributeDataFilter::ConstructArray(YPts, 0, fieldArray[1],
  ...
  else // have to copy data into created array
  {
    ...
    if (vtkFieldDataToAttributeDataFilter::ConstructArray(ZPts, 0, fieldArray[2],
...
int vtkDataObjectToDataSetFilter::ConstructCells(vtkDataObject* input, vtkPolyData* pd)
{
  ...
  if (fieldArray[3])
  {
    ...
    vtkCellArray* triStrips = this->ConstructCellArray(
  ...
  vtkDataArray* da, int comp, vtkIdType compRange[2])
{
  ...
  if (da->GetDataType() == VTK_ID_TYPE && da->GetNumberOfComponents() == 1 && comp == 0 &&
...
void vtkDataObjectToDataSetFilter::ConstructDimensions(vtkDataObject* input)
{
  ...
  {
    vtkFieldData* fd = input->GetFieldData();
    vtkDataArray* fieldArray = vtkFieldDataToAttributeDataFilter::GetFieldArray(
...
void vtkDataObjectToDataSetFilter::ConstructOrigin(vtkDataObject* input)
{
  ...
  else
  {
    ...
    vtkFieldDataToAttributeDataFilter::UpdateComponentRange(fieldArray, this->OriginComponentRange);
