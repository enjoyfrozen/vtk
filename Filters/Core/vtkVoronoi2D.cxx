...
struct VTile
{
  ...
  VTile() : PointId(-1), Locator(nullptr)
  {
    this->TileX[0] = 0.0;
    this->TileX[1] = 0.0;
  ...
  // debugging.
  void PopulatePolyData(vtkPoints *centers, vtkCellArray *tile,
  ...
  int ClipTile(vtkIdType ptId, const double p[3], double vtkNotUsed(tol))
  {
    ...
    //
    for ( i=0, tPtr=this->Verts.begin(); tPtr->Val < 0.0 && i < nSectors;
          ++tPtr, ++i)
    {}
    ...
    for ( numToDelete=0, tPtr=this->Next(tMRight); tPtr != tPLeft;
          tPtr=this->Next(tPtr) )
    {
      numToDelete++;
  ...
  {
    if ( this->SpiralX == this->SpiralY ||
         (this->SpiralX < 0 && this->SpiralX == -this->SpiralY) ||
    ...
    if ( 0 <= ij[0] && ij[0] < this->Divisions[0] &&
         0 <= ij[1] && ij[1] < this->Divisions[1] )
    {
...
// and is mentioned in comments elsewhere.
  ...
  bool BuildTile(vtkIdList* pIds, const double *pts, double tol, vtkIdType maxClips)
  {
    ...
    while ( !this->IsFlowerCovered() && numClips < maxClips &&
            numClipAttempts < numPts)
    {
      if ( (bucket=this->NextSpiralBucket(ij)) >= 0 &&
           (nPts=this->Locator->GetNumberOfPointsInBucket(bucket)) > 0 &&
           this->InFlower(ij) )
      {
        this->Locator->GetBucketIds(bucket,pIds);
...
struct TileVertex
{
  double X;
  double Y;
  TileVertex(double x, double y) : X(x), Y(y) {}
};
...
struct LocalDataType
{
  ...
  LocalDataType() : NumberOfTiles(0), NumberOfPoints(0), Offset(0)
  {
...
struct VoronoiTiles
{
  ...
  VoronoiTiles(vtkIdType npts, double *points, vtkStaticPointLocator2D *loc, double padding,
    ...
    Points(points), NPts(npts), Locator(loc), Tol(tol), ScalarMode(scalarMode),
    MaxClips(maxClips), NumThreadsUsed(0)
  {
    ...
    this->Scalars =
      static_cast<vtkIdTypeArray*>(output->GetCellData()->GetScalars());
    ...
    this->H[0] = (this->Bounds[1]-this->Bounds[0]) /
      static_cast<double>(this->Divisions[0]);
    this->H[1] = (this->Bounds[3]-this->Bounds[2]) /
      static_cast<double>(this->Divisions[1]);
    this->BucketRadius = 0.5 *
      sqrt( this->H[0]*this->H[0] + this->H[1]*this->H[1] );
  ...
  static int Execute(vtkStaticPointLocator2D *loc, vtkIdType numPts, double *points,
                     ...
                     vtkIdType pointOfInterest, vtkIdType maxClips)
  {
  ...
  vtkInformationVector *outputVector)
{
  ...
  if ( (inPoints=input->GetPoints()) == nullptr ||
       (numPts=inPoints->GetNumberOfPoints()) < 1 )
  {
  ...
  this->NumberOfThreadsUsed = VoronoiTiles::Execute(this->Locator, numPts,
                        inPtr, padding, tol, output, this->GenerateScalars,
                        this->PointOfInterest, this->MaximumNumberOfTileClips);
  ...
  if ( this->GenerateVoronoiFlower &&
       this->PointOfInterest >=0 && this->PointOfInterest < numPts )
  {
    ...
    {
      x[0] = vtkMath::Random(center[0]+factor*(bds[0]-center[0]),
                             center[0]+factor*(bds[1]-center[0]));
      x[1] = vtkMath::Random(center[1]+factor*(bds[2]-center[1]),
...
void vtkVoronoi2D::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Maximum Number Of Tile Clips: "
     << this->MaximumNumberOfTileClips << "\n";
  os << indent << "Generate Voronoi Flower: "
     << (this->GenerateVoronoiFlower ? "On\n" : "Off\n");
}
