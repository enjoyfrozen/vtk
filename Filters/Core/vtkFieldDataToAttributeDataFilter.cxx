...
//
int vtkFieldDataToAttributeDataFilter::RequestData(vtkInformation* vtkNotUsed(request),
...
//
void vtkFieldDataToAttributeDataFilter::SetScalarComponent(
  int comp, const char* arrayName, int arrayComp, int min, int max, int normalize)
{
  ...
  vtkFieldDataToAttributeDataFilter::SetArrayName(this, this->ScalarArrays[comp], arrayName);
...
void vtkFieldDataToAttributeDataFilter::ConstructScalars(int num, vtkFieldData* fd,
  vtkDataSetAttributes* attr, vtkIdType componentRange[4][2], char* arrays[4], int arrayComp[4],
  int normalize[4], int numComp)
{
  ...
  for (i = 0; i < numComp; i++)
  {
    fieldArray[i] = vtkFieldDataToAttributeDataFilter::GetFieldArray(fd, arrays[i], arrayComp[i]);
  ...
  {
    updated |= vtkFieldDataToAttributeDataFilter::UpdateComponentRange(fieldArray[i], componentRange[i]);
  ...
  {
    newScalars = vtkDataArray::CreateDataArray(vtkFieldDataToAttributeDataFilter::GetComponentsType(numComp, fieldArray));
    newScalars->SetNumberOfTuples(num);

    for (i = 0; i < numComp; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newScalars, i, fieldArray[i], arrayComp[i], componentRange[i][0],
...
//
void vtkFieldDataToAttributeDataFilter::SetVectorComponent(
  int comp, const char* arrayName, int arrayComp, int min, int max, int normalize)
{
  if (comp < 0 || comp > 2)
  {
  ...
  vtkFieldDataToAttributeDataFilter::SetArrayName(this, this->VectorArrays[comp], arrayName);
...
void vtkFieldDataToAttributeDataFilter::ConstructVectors(int num, vtkFieldData* fd,
  vtkDataSetAttributes* attr, vtkIdType componentRange[3][2], char* arrays[3], int arrayComp[3],
  int normalize[3])
{
  int i, updated;
  ...
  updated = vtkFieldDataToAttributeDataFilter::UpdateComponentRange(fieldArray[0], componentRange[0]);
  updated |= vtkFieldDataToAttributeDataFilter::UpdateComponentRange(fieldArray[1], componentRange[1]);
  updated |= vtkFieldDataToAttributeDataFilter::UpdateComponentRange(fieldArray[2], componentRange[2]);
  ...
  {
    newVectors = vtkDataArray::CreateDataArray(vtkFieldDataToAttributeDataFilter::GetComponentsType(3, fieldArray));
    newVectors->SetNumberOfComponents(3);
    newVectors->SetNumberOfTuples(num);

    for (i = 0; i < 3; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newVectors, i, fieldArray[i], arrayComp[i], componentRange[i][0],
            componentRange[i][1], normalize[i]) == 0)
      {
        newVectors->Delete();
        return;
...
//
void vtkFieldDataToAttributeDataFilter::SetNormalComponent(
  int comp, const char* arrayName, int arrayComp, int min, int max, int normalize)
{
  if (comp < 0 || comp > 2)
  {
  ...
  vtkFieldDataToAttributeDataFilter::SetArrayName(this, this->NormalArrays[comp], arrayName);
  ...
  int normalize[3])
{
  ...
  {
    newNormals = vtkDataArray::CreateDataArray(vtkFieldDataToAttributeDataFilter::GetComponentsType(3, fieldArray));
    newNormals->SetNumberOfComponents(3);
    newNormals->SetNumberOfTuples(num);

    for (i = 0; i < 3; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newNormals, i, fieldArray[i], arrayComp[i], componentRange[i][0],
            componentRange[i][1], normalize[i]) == 0)
      {
        newNormals->Delete();
        return;
...
//
void vtkFieldDataToAttributeDataFilter::SetTCoordComponent(
  int comp, const char* arrayName, int arrayComp, int min, int max, int normalize)
{
  if (comp < 0 || comp > 2)
  {
  ...
  vtkFieldDataToAttributeDataFilter::SetArrayName(this, this->TCoordArrays[comp], arrayName);
  ...
  int normalize[3], int numComp)
{
  ...
  {
    newTCoords = vtkDataArray::CreateDataArray(vtkFieldDataToAttributeDataFilter::GetComponentsType(numComp, fieldArray));
    newTCoords->SetNumberOfComponents(numComp);
    newTCoords->SetNumberOfTuples(num);

    for (i = 0; i < numComp; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newTCoords, i, fieldArray[i], arrayComp[i], componentRange[i][0],
            componentRange[i][1], normalize[i]) == 0)
      {
        newTCoords->Delete();
        return;
...
//
void vtkFieldDataToAttributeDataFilter::SetTensorComponent(
  int comp, const char* arrayName, int arrayComp, int min, int max, int normalize)
{
  if (comp < 0 || comp > 8)
  {
  ...
  vtkFieldDataToAttributeDataFilter::SetArrayName(this, this->TensorArrays[comp], arrayName);
...
void vtkFieldDataToAttributeDataFilter::ConstructTensors(int num, vtkFieldData* fd,
  vtkDataSetAttributes* attr, vtkIdType componentRange[9][2], char* arrays[9], int arrayComp[9],
  int normalize[9])
{
  int i, normalizeAny, updated = 0;
  int numComp = 9;
  ...
  {
    newTensors = vtkDataArray::CreateDataArray(vtkFieldDataToAttributeDataFilter::GetComponentsType(numComp, fieldArray));
    newTensors->SetNumberOfComponents(numComp);
    newTensors->SetNumberOfTuples(num);

    for (i = 0; i < numComp; i++)
    {
      if (vtkFieldDataToAttributeDataFilter::ConstructArray(newTensors, i, fieldArray[i], arrayComp[i], componentRange[i][0],
            componentRange[i][1], normalize[i]) == 0)
      {
        newTensors->Delete();
        return;
...
//
void vtkFieldDataToAttributeDataFilter::ConstructFieldData(
  int vtkNotUsed(num), vtkDataSetAttributes* vtkNotUsed(attr))
{
}
...
int vtkFieldDataToAttributeDataFilter::ConstructArray(vtkDataArray* da, int comp,
  vtkDataArray* fieldArray, int fieldComp, vtkIdType min, vtkIdType max, int normalize)
{
...
vtkDataArray* vtkFieldDataToAttributeDataFilter::GetFieldArray(
  vtkFieldData* fd, const char* name, int comp)
{
...
void vtkFieldDataToAttributeDataFilter::SetArrayName(
  vtkObject* self, char*& name, const char* newName)
{
...
int vtkFieldDataToAttributeDataFilter::UpdateComponentRange(
  vtkDataArray* da, vtkIdType compRange[2])
{
  if (compRange[0] == -1)
  {
    compRange[0] = 0;
