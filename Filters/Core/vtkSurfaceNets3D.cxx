...
struct SurfaceNets
{
  ...
  unsigned char ClassifyYEdge(T* inPtr, vtkIdType i, TriadType case0,
                              vtkIdType row, TriadType case1)
  {
  ...
  // expressed in the triad coordinates.
  unsigned char ClassifyZEdge(T* inPtr, vtkIdType i, TriadType case0,
                              vtkIdType slice, TriadType case1)
  {
    // If on padded boundary, edge is never intersected.
...
void SurfaceNets<T>::ClassifyYZEdges(T* inPtr, vtkIdType row, vtkIdType slice)
{
  ...
  bool xInts = !((eMD[3] >= numTriads) && (eMDY[3] >= numTriads) &&
                 (eMDZ[3] >= numTriads));
...
void ComputeSmoothingConstraints(vtkConstrainedSmoothingFilter* smoother,
                                 double spacing[3], double constraintScale)
{
  smoother->SetConstraintDistance((vtkMath::Norm(spacing) / 2.0) * constraintScale);
  smoother->SetConstraintBox(constraintScale * spacing[0],
                             constraintScale * spacing[1],
                             constraintScale * spacing[2]);
}
