...
=========================================================================*/
#include "vtkTexturedCapsuleSource.h"
...
vtkTexturedCapsuleSource::vtkTexturedCapsuleSource(int res)
{
  ...
  // DEPRECATED: The sphere wireframe generates triangles within the natural latitude and longitude
  // lines.
  this->LatLongTessellation = 1;
  this->CylinderLength = 1.0;
  this->OutputPointsPrecision = vtkAlgorithm::DEFAULT_PRECISION;
  this->SetNumberOfInputPorts(0);
}

int vtkTexturedCapsuleSource::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  // NOTE: Num of polys increase by theta_R * 2 (i.e., one more band of triangles
  // around the barrel of the cylinder).
  ...
  // This is to ensure we always have a band at the equator for both hemispheres.
  ...
  for (i = 0; i <= this->ThetaResolution; i++)
  {
    ...
    for (j = 0; j <= phiResolutionHalf; j++)
    {
      ...
      x[1] = this->Center[1] + this->Radius * cos((double)phi) + this->CylinderLength / 2;
      x[2] = this->Center[2] + -1 * radius * sin((double)theta);
      newPoints->InsertNextPoint(x);

      // CylinderLength is not needed for the normal computation.
      n[0] = radius * cos((double)theta);
      n[1] = this->Radius * cos((double)phi);
      n[2] = -1 * radius * sin((double)theta);
...
//------------------------------------------------------------------------------
int vtkTexturedCapsuleSource::RequestInformation(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  vtkInformation* outInfo = outputVector->GetInformationObject(0);
