  ...
  if (vtkDataSet* input = vtkDataSet::SafeDownCast(inputDO))
  {
    ...
    if (node->GetProperties()->Has(vtkSelectionNode::FIELD_TYPE()))
    {
      ...
      if (fieldType == vtkSelectionNode::POINT)
      {
        ...
        {
          thresholdByPointVals = node->GetProperties()->Get(vtkSelectionNode::CONTAINING_CELLS());
        }
      }
    }
  ...
  for (cellId = 0; cellId < input->GetNumberOfCells(); cellId++)
  {
    ...
    if (usePointScalars)
    {
      ...
      for (i = 0; (i < numCellPts) && (passThrough || !keepCell); i++)
      {
        ...
        int inside = vtkExtractSelectedThresholds::EvaluateValue(inScalars, comp_no, ptId, lims, &above, &below, nullptr);
        totalAbove += above;
        totalBelow += below;
  ...
  vtkDataArray* lims, int* AboveCount, int* BelowCount, int* InsideCount)
{
  ...
  if (!vtkArrayDispatch::Dispatch::Execute(lims, TestItem{}, value, keepCell, above, below, inside))
  {
