...
public:
  ...
  vtkIdType Prepare(vtkIdType numInputCells, bool presorted)
  {
    assert(numInputCells > 0);

    if (presorted == false && (this->ModifiedTime > this->SortTime))
    {
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  const vtkIdType numCells = this->CellList->Prepare(numCellsInput, this->AssumeSortedAndUniqueIds);
