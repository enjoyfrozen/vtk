  ...
  vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  // downstream.
  vtkNew<vtkIntArray> mode_shape;
  mode_shape->SetName("mode_shape");
  mode_shape->SetNumberOfComponents(1);
  mode_shape->SetNumberOfTuples(1);
  mode_shape->SetTypedComponent(0, 0, this->ModeShape);

  vtkNew<vtkIntArray> mode_shape_range;
  mode_shape_range->SetName("mode_shape_range");
  mode_shape_range->SetNumberOfComponents(2);
  mode_shape_range->SetNumberOfTuples(1);
  mode_shape_range->SetTypedTuple(0, this->ModeShapesRange);

  output->GetFieldData()->AddArray(mode_shape);
  output->GetFieldData()->AddArray(mode_shape_range);
