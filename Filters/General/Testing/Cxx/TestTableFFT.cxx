...
bool FuzzyCompare(vtkTable* in, vtkTable* expected, double epsilon)
{
  ...
  for (std::size_t col = 0; col < in->GetNumberOfColumns(); ++col)
  {
    ...
    for (std::size_t i = 0; i < inArray->GetNumberOfValues(); ++i)
    {
...
int TestTableFFT(int vtkNotUsed(argc), char* vtkNotUsed(argv)[])
{
  ...
  return details::FuzzyCompare(fftFilter->GetOutput(), reference, 1.0e-6) ? EXIT_SUCCESS
                                                                          : EXIT_FAILURE;
}
