...
//------------------------------------------------------------------------------
int vtkMergeTimeFilter::FillInputPortInformation(int, vtkInformation* info)
{
...
//------------------------------------------------------------------------------
bool vtkMergeTimeFilter::AreTimesInTolerance(double t1, double t2)
{
  if (t1 == t2)
  {
    return true;
  ...
  if (!this->UseRelativeTolerance)
  {
    return std::abs(t2 - t1) < this->Tolerance;
  }

  if (t1 != 0)
  {
    return std::abs((t1 - t2) / t1) < this->Tolerance;
  }

  // TODO find correct check when t1 is null and using relative tolerance ??
  return std::abs(t1 - t2) < this->Tolerance;
}
...
double vtkMergeTimeFilter::MapToInputTime(int input, double outputTime)
{
  ...
  {
    if (this->AreTimesInTolerance(time, outputTime))
    {
      return time;
    }
  }
...
void vtkMergeTimeFilter::MergeTimeSteps(const std::vector<double>& timeSteps)
{
  ...
  for (double t : timeSteps)
  {
    ...
    auto insideTolerance = [=](double val) { return this->AreTimesInTolerance(val, t); };
    ...
    {
      newTimeSteps.push_back(t);
    }
  }
  std::sort(newTimeSteps.begin(), newTimeSteps.end());
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (int i = 0; i < numInputs; i++)
  {
    ...
    if (inInfo->Has(vtkStreamingDemandDrivenPipeline::TIME_STEPS()))
    {
      ...
      for (int t = 0; t < nbOfTimes; t++)
      {
        double toInsert = inputTimes[t];
