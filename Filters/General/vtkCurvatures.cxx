  ...
  vtkCellArray* facets, vtkPolyData* output, double* gaussCurvatureData)
{
  ...
  while (facets->GetNextCell(f, vert))
  {
    ...
    alpha0 = vtkMath::AngleBetweenVectors(e1, e2);
    alpha1 = vtkMath::AngleBetweenVectors(e2, e0);
    alpha2 = vtkMath::AngleBetweenVectors(e0, e1);
...
void vtkCurvatures::GetMaximumCurvature(vtkPolyData* input, vtkPolyData* output)
{
  ...
  for (vtkIdType i = 0; i < numPts; i++)
  {
    ...
    else if (tmp >= -0.1)
    {
      ...
      if (tmp < -0,1)
      {
        vtkWarningMacro(<< "The Gaussian or mean curvature at point "
            << i << " have a large computation error... The maximum curvature is likely off.");
      }
    }
...
void vtkCurvatures::GetMinimumCurvature(vtkPolyData* input, vtkPolyData* output)
{
  ...
  for (vtkIdType i = 0; i < numPts; i++)
  {
    ...
    else if (tmp >= -0.1)
    {
      ...
      {
        vtkWarningMacro(<< "The Gaussian or mean curvature at point "
            << i << " have a large computation error... The minimum curvature is likely off.");
      }
    }
