  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (cellId = 0; cellId < numCells && !abort; cellId++)
  {
    ...
    if (this->GenerateClippedOutput)
    {
      ...
      {
        vtkErrorMacro(<< "Do not support cells of dimension " << cell->GetCellDimension());
      }
    }
    else
    {
...
//
void vtkBoxClipDataSet::SetBoxClip(const double* n0, const double* o0, const double* n1,
  const double* o1, const double* n2, const double* o2, const double* n3, const double* o3,
  const double* n4, const double* o4, const double* n5, const double* o5)
{
  if ((this->Orientation == 1)

    && (this->PlaneNormal[0][0] == n0[0]) && (this->PlaneNormal[0][1] == n0[1]) &&
    (this->PlaneNormal[0][2] == n0[2])

    && (this->PlaneNormal[1][0] == n1[0]) && (this->PlaneNormal[1][1] == n1[1]) &&
    (this->PlaneNormal[1][2] == n1[2])

    && (this->PlaneNormal[2][0] == n2[0]) && (this->PlaneNormal[2][1] == n2[1]) &&
    (this->PlaneNormal[2][2] == n2[2])

    && (this->PlaneNormal[3][0] == n3[0]) && (this->PlaneNormal[3][1] == n3[1]) &&
    (this->PlaneNormal[3][2] == n3[2])

    && (this->PlaneNormal[4][0] == n4[0]) && (this->PlaneNormal[4][1] == n4[1]) &&
    (this->PlaneNormal[4][2] == n4[2])

    && (this->PlaneNormal[5][0] == n5[0]) && (this->PlaneNormal[5][1] == n5[1]) &&
    (this->PlaneNormal[5][2] == n5[2])

    && (this->PlanePoint[0][0] == o0[0]) && (this->PlanePoint[0][1] == o0[1]) &&
    (this->PlanePoint[0][2] == o0[2])

    && (this->PlanePoint[1][0] == o1[0]) && (this->PlanePoint[1][1] == o1[1]) &&
    (this->PlanePoint[1][2] == o1[2])

    && (this->PlanePoint[2][0] == o2[0]) && (this->PlanePoint[2][1] == o2[1]) &&
    (this->PlanePoint[2][2] == o2[2])

    && (this->PlanePoint[3][0] == o3[0]) && (this->PlanePoint[3][1] == o3[1]) &&
    (this->PlanePoint[3][2] == o3[2])

    && (this->PlanePoint[4][0] == o4[0]) && (this->PlanePoint[4][1] == o4[1]) &&
    (this->PlanePoint[4][2] == o4[2])

    && (this->PlanePoint[5][0] == o5[0]) && (this->PlanePoint[5][1] == o5[1]) &&
...
{
  if ((this->Orientation == 0) && (this->BoundBoxClip[0][0] == xmin) &&
    (this->BoundBoxClip[0][1] == xmax) && (this->BoundBoxClip[1][0] == ymin) &&
    (this->BoundBoxClip[1][1] == ymax) && (this->BoundBoxClip[2][0] == zmin) &&
...
void vtkBoxClipDataSet::InterpolateEdge(
  vtkDataSetAttributes* attributes, vtkIdType toId, vtkIdType fromId1, vtkIdType fromId2, double t)
{
...
void vtkBoxClipDataSet::MinEdgeF(
  const unsigned int* id_v, const vtkIdType* cellIds, unsigned int* edgF)
{
...
//
void vtkBoxClipDataSet::WedgeToTetra(
  const vtkIdType* wedgeId, const vtkIdType* cellIds, vtkCellArray* newCellArray)
{
  int i;
  int id;
  vtkIdType xmin;
...
//
void vtkBoxClipDataSet::PyramidToTetra(
...
//
void vtkBoxClipDataSet::CreateTetra(
  vtkIdType npts, const vtkIdType* cellIds, vtkCellArray* newCellArray)
{
  vtkIdType tabp[5];
  vtkIdType tab[3][4];
          ...
          case 4: // We have two wedges
            switch (edges_inter)
            {
              ...
              case 1356:
                ...
                break;
              default:
                vtkErrorMacro(<< "Intersection not found: Num_inter = " << num_inter
  ...
  vtkPointData** outPD, vtkCellData* inCD, vtkIdType cellId, vtkCellData** outCD)
{
  ...
  for (idtetranew = 0; idtetranew < totalnewtetra; idtetranew++)
  {
    ...
    for (planes = 0; planes < 6; planes++)
    {
      ...
      value =
        this->BoundBoxClip[cutInd][planes % 2]; // The plane is always parallel to unitary cube.
  ...
  vtkCellData* inCD, vtkIdType cellId, vtkCellData* outCD)
{
  ...
  for (idtrianglenew = 0; idtrianglenew < totalnewtriangle; idtrianglenew++)
  {
    ...
    unsigned int tab2[3][4] = {
      { 1, 2, 1, 0 },
      { 2, 0, 0, 1 },
      { 0, 1, 0, 1 },
    };
    unsigned int tab1[3][2] = {
      { 2, 1 },
      { 0, 2 },
      { 1, 0 },
    };
        ...
        if (num_inter == 0)
        {
          ...
          for (i = 0; i < 3; i++)
          {
            if (((v_triangle[i][cutInd] < value) && ((planes % 2) == 0)) ||
              ...
              ((v_triangle[i][cutInd] > value) &&
                ((planes % 2) == 1))) // because there is not intersection.
            {
              outside = 1;
              break;
            }
  ...
  for (unsigned int idlinenew = 0; idlinenew < totalnewline; idlinenew++)
  {
    ...
    for (planes = 0; planes < 6; planes++)
    {
      ...
      for (idcellnew = 0; idcellnew < totalnewcells; idcellnew++)
      {
        ...
        if (((planes % 2 == 0) && (v_line[0][cutInd] >= value) && (v_line[1][cutInd] >= value)) ||
          ((planes % 2 == 1) && (v_line[0][cutInd] <= value) && (v_line[1][cutInd] <= value)))
        {
  ...
  vtkCellData* outCD)
{
  ...
  for (unsigned int idlinenew = 0; idlinenew < totalnewvert; idlinenew++)
  {
    ...
    for (int k = 0; k < 6; k++)
    {
      double value = (this->PlaneNormal[k][0] * (v[0] - this->PlanePoint[k][0]) +
        this->PlaneNormal[k][1] * (v[1] - this->PlanePoint[k][1]) +
        this->PlaneNormal[k][2] * (v[2] - this->PlanePoint[k][2]));
      if (value > 0)
      {
