...
void vtkPCAStatistics::GetEigenvalues(int request, vtkDoubleArray* eigenvalues)
{
  ...
  vtkStringArray* rowNames =
    vtkArrayDownCast<vtkStringArray>(outputMeta->GetColumnByName("Column"));
    ...
    if (rowName == ss.str())
    {
      ...
      for (int val = 0; val < numberOfEigenvalues; val++)
      {
        ...
        vtkDoubleArray* currentCol =
          vtkArrayDownCast<vtkDoubleArray>(outputMeta->GetColumn(val + 2));
...
public:
  ...
  virtual bool InitializePCA(vtkTable* inData, vtkTable* reqModel, int normScheme, int basisScheme,
  ...
  int basisScheme, int fixedBasisSize, double fixedBasisEnergy)
{
  ...
  vtkDoubleArray* evalm =
    vtkArrayDownCast<vtkDoubleArray>(reqModel->GetColumnByName(VTK_MULTICORRELATIVE_AVERAGECOL));
  if (!evalm)
  {
  ...
  if (mrmr < ermr)
  {
    vtkGenericWarningMacro("Expected " << (2 * m + 1) << " or more columns in request but found "
  ...
  for (vtkIdType b = 1; b < nb; ++b)
  {
    ...
    switch (this->NormalizationScheme)
    {
      ...
      case DIAGONAL_SPECIFIED:
        vtkPCAStatisticsNormalizeSpec(normData, cov, this->GetSpecifiedNormalization(), reqModel,
