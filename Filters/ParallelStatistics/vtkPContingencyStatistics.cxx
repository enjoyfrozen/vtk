  ...
  vtkTable* contingencyTab, vtkStdString& xyPacked, std::vector<vtkIdType>& kcValues)
{
  ...
  vtkIdTypeArray* card =
    vtkArrayDownCast<vtkIdTypeArray>(contingencyTab->GetColumnByName("Cardinality"));
  ...
  const vtkStdString& buffer, std::vector<vtkStdString>& strings)
{
  ...
  for (const char* start = &buffer[0]; start != bufferEnd; ++start)
  {
    for (const char* finish = start; finish != bufferEnd; ++finish)
    {
      if (!*finish)
      {
        strings.emplace_back(vtkStdString(start));
        start = finish;
        break;
      }
    }
  ...
  vtkTable* inData, vtkTable* inParameters, vtkMultiBlockDataSet* outMeta)
{
  ...
  {
    vtkErrorMacro("Packing error on process " << myRank << ".");
  ...
  {
    vtkErrorMacro("Process " << myRank << "could not gather (k,c) values.");
  ...
  vtkIdType rProc)
{
  ...
  {
    vtkErrorMacro("Process " << com->GetLocalProcessId() << " could not broadcast (x,y) values.");
  ...
  {
    vtkErrorMacro("Process " << com->GetLocalProcessId() << " could not broadcast (k,c) values.");
