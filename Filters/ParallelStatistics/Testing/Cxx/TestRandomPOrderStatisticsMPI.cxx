...
void RandomOrderStatistics(vtkMultiProcessController* controller, void* arg)
{
  ...
  *(args->retVal) = 0;
  ...
  com->AllReduce(min_l, min_g, nVariables, vtkCommunicator::MIN_OP);

  com->AllReduce(max_l, max_g, nVariables, vtkCommunicator::MAX_OP);
...
int TestRandomPOrderStatisticsMPI(int argc, char* argv[])
{
  ...
  clArgs.AddArgument(
    "--skip-int", vtksys::CommandLineArguments::NO_ARGUMENT, &skipInt, "Skip integer variable");

  // Parse whether string variable should be skipped
  clArgs.AddArgument("--skip-string", vtksys::CommandLineArguments::NO_ARGUMENT, &skipString,
  ...
  clArgs.AddArgument("--std-dev", vtksys::CommandLineArguments::SPACE_ARGUMENT, &stdev,
    "Standard deviation of pseudo-random Gaussian sample");

  // Parse maximum histogram size
  clArgs.AddArgument("--max-histo-size", vtksys::CommandLineArguments::SPACE_ARGUMENT,
  ...
  clArgs.AddArgument(
    "--quantize", vtksys::CommandLineArguments::NO_ARGUMENT, &quantize, "Allow re-quantizing");
