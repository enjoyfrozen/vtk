  ...
  *(args->retVal) = 0;
    ...
    for (int i = 0; i < numProcs; ++i)
    {
      ...
      if (GT_g[i] != testIntValue)
      {
        vtkGenericWarningMacro(
          "Incorrect grand total:" << GT_g[i] << " <> " << testIntValue << ")");
        *(args->retVal) = 1;
      }
    }
  }
  ...
  else
  {
    ...
    for (vtkIdType k = 0; k < nRowSumm; ++k)
    {
      ...
      com->AllGather(H_l, H_g, nEntropies);
        ...
        for (int i = 0; i < numProcs; ++i)
        {
          ...
          if (testDoubleValue1 > H_g[nEntropies * i])
          {
            vtkGenericWarningMacro("Reported inconsistent information entropies: H(X,Y) = "
              << H_g[nEntropies * i] << " < " << testDoubleValue1 << " = H(Y|X)+ H(X|Y).");
            *(args->retVal) = 1;
          }
        }
  ...
  com->AllGather(cdf_l, cdf_g, nRowSumm);
