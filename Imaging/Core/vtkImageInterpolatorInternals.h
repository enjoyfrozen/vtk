...
// cast will round the value down. After the cast, we can can
// subtract the bias from the integer result.
...
// always be rounded to the integer, even when the value is less than
// the integer.  Values further than this tolerance from an integer
...
#else
  // and again for little-endian architectures
  union {
    double d;
    unsigned short s[4];
    unsigned int i[2];
