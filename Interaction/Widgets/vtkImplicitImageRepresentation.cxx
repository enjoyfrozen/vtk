...
//----------------------------------------------------------------------------
void vtkImplicitImageRepresentation::
PlaceWidget(vtkAlgorithmOutput* aout)
{
  vtkImageData *img =
    vtkImageData::SafeDownCast(aout->GetProducer()->GetOutputDataObject(aout->GetIndex()));

  this->PlaceWidget(img);
  this->Reslice->SetInputConnection(aout);
 }

//----------------------------------------------------------------------------
void vtkImplicitImageRepresentation::
PlaceWidget(vtkImageData *img)
{
  this->ImageData = img;
  if (!this->ImageData)
  {
...
//----------------------------------------------------------------------------
void vtkImplicitImageRepresentation::
SetLookupTable(vtkLookupTable* table)
{
