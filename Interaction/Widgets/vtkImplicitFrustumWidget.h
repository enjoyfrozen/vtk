 ...
 * of four parts: 1) an frustum contained in a 2) bounding box, with an
 * 3) frustum axis, which is rooted at an 4) origin point in the bounding
 ...
 *
 * If the frustum axis is selected:
 ...
 *  - 'Down/Left' Move frustum away from viewer
 *  - 'Up/Right' Move frustum towards viewer
 * In all the cases, independent of what is picked, the widget responds to the
 * following VTK events:
 *  - MiddleButtonPressEvent - move the frustum
 ...
 * into the vtkImplicitFrustumWidget's widget events:
 * <pre>
 *   vtkWidgetEvent::Select -- some part of the widget has been selected
 *   vtkWidgetEvent::EndSelect -- the selection process has completed
 *   vtkWidgetEvent::Move -- a request for widget motion has been invoked
 *   vtkWidgetEvent::Up and vtkWidgetEvent::Down -- MoveFrustumAction
 * </pre>
 *
 * In turn, when these widget events are processed, the vtkImplicitFrustumWidget
 ...
 *
 * @sa
 * vtk3DWidget vtkFrustumRepresentation vtkFrustum
 */
...
private:
  ...
  static void MoveFrustumAction(vtkAbstractWidget* widget);
  ...
  WidgetStateType WidgetState = vtkImplicitFrustumWidget::Idle;
};
