...
const char TestDistanceWidget3DEventLog[] = "# StreamVersion 1\n"
...
int TestDistanceWidget3D(int argc, char* argv[])
{
  ...
  {
    std::cerr << "Error setting glyph scale to " << glyphScale << ", returned "
              << rep->GetGlyphScale() << std::endl;
    return EXIT_FAILURE;
  }
  ...
  {
    std::cerr << "Error setting label position to 0.45, returned : " << rep->GetLabelPosition()
              << std::endl;
    return EXIT_FAILURE;
  }
  for (int maxTicks = 1; maxTicks < 100; maxTicks += 10)
  {
    ...
    {
      std::cerr << "Error setting maximum number of ruler ticks to " << maxTicks
                << ", get returned " << rep->GetMaximumNumberOfRulerTicks() << std::endl;
      return EXIT_FAILURE;
    }
  }
