 ...
 * @brief   class defining a representation for vtkTensorWidget
 *
 ...
 *
 * To use this representation, specify a 3x3 symmetric matrix defining the
 * tensor. (This implicitly defines an orthogonal basis from the three tensor
 * eigenvectors.) Then use PlaceWidget() to define a bounding box: the
...
public:
  ...
  {
    std::copy(this->Tensor,this->Tensor+9, tensor);
  }
