...
// to refWorldPos.
int vtkClosedSurfacePointPlacer::ComputeWorldPosition(vtkRenderer* ren, double displayPos[2],
  double refWorldPos[3], double worldPos[3], double* vtkNotUsed(worldOrient))
{
  this->BuildPlanes();
  ...
  for (int n = 0; n < nPlanes; n++)
  {
    ...
    vtkPlane::IntersectWithLine(l0, l1, plane->GetNormal(), plane->GetOrigin(), t, node.p);

    // The IF below insures that the line and the plane aren't parallel.
    if (t != VTK_DOUBLE_MAX)
    {
      node.Plane = plane;
      node.Distance = vtkClosedSurfacePointPlacer::GetDistanceFromObject(node.p, this->InnerBoundingPlanes, d);
  ...
  std::sort(intersections.begin(), intersections.end(), vtkClosedSurfacePointPlacerNode::Sort);
  ...
  vtkDebugMacro(<< "Reference Pos: (" << refWorldPos[0] << "," << refWorldPos[1] << ","
...
double vtkClosedSurfacePointPlacer ::GetDistanceFromObject(
