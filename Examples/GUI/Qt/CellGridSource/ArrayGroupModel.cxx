...
QVariant ArrayGroupModel::data(const QModelIndex& index, int role) const
{
  ...
  if (index.row() < 0 || index.column() < 0 ||
    index.row() >= this->rowCount(index.parent()) ||
  ...
  {
    std::string value = stringArray->GetValue(
      stringArray->GetNumberOfComponents() * index.row() + colData.Component);
...
bool ArrayGroupModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
  ...
  else if (auto* stringArray = vtkStringArray::SafeDownCast(array))
  {
    stringArray->SetValue(
      stringArray->GetNumberOfComponents() * index.row() + colData.Component, value.toString().toStdString());
    this->Data->Modified();
    Q_EMIT dataChanged(index, index);
    return true;
  }
