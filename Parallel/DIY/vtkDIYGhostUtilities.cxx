...
void ExchangeBlockStructuresForUnstructuredData(diy::Master& master)
{
  ...
  master.foreach ([](BlockType* block, const diy::Master::ProxyWithLink& cp) {
    ...
    vtkPointSet* interface = vtkPointSet::SafeDownCast(
    ...
    for (int id = 0; id < static_cast<int>(cp.link()->size()); ++id)
    {
      ...
      vtkSmartPointer<vtkIdList> ids = ExtractPointIdsInsideBoundingBox(
          interface->GetPoints(), block->NeighborBoundingBoxes.at(blockId.gid));

      vtkNew<vtkIdList> identity;
      identity->SetNumberOfIds(ids->GetNumberOfIds());
      std::iota(identity->begin(), identity->end(), 0);

      if (!interface->GetNumberOfPoints())
      {
      ...
      if (vtkIdTypeArray* globalIds = vtkArrayDownCast<vtkIdTypeArray>(
            interface->GetPointData()->GetGlobalIds()))
      {
        vtkNew<vtkIdTypeArray> gids;
        gids->InsertTuples(identity, ids, globalIds);

        cp.enqueue<vtkDataArray*>(blockId, interface->GetPointData()->GetGlobalIds());
      }
      else
      {
        vtkNew<vtkPoints> points;
        points->SetDataType(interface->GetPoints()->GetDataType());
        points->InsertPoints(identity, ids, interface->GetPoints());
    ...
    std::vector<PointSetT*>& inputs)
{
  ...
  for (int localId = 0; localId < static_cast<int>(inputs.size()); ++localId)
  {
    ...
    information.InterfaceExtractor = vtkSmartPointer<vtkAlgorithm>::Take(
        InstantiateInterfaceExtractor<PointSetT>(input->GetPoints()->GetDataType()));
...
void CloneGrid(GridDataSetT* grid, GridDataSetT* clone)
{
  ...
  cloneCellData->SetCopyGlobalIds(true);
  ...
  clonePointData->SetCopyGlobalIds(true);
    ...
    UnstructuredGridBlockStructure& blockStructure)
{
  ...
  std::string offsetsLog = "offsets ";
  for (vtkIdType id = 0; id < offsets->GetNumberOfValues(); ++id)
  {
    offsetsLog += std::to_string((int)offsets->GetTuple1(id)) + " ";
  }
  std::string connectivityLog = "connectivity ";
  for (vtkIdType id = 0; id < connectivity->GetNumberOfValues(); ++id)
  {
    connectivityLog += std::to_string((int)connectivity->GetTuple1(id)) + " ";
  }
