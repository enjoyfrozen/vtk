...
{
  CellArrayInserter(vtkCellArray* srcCells, vtkIdTypeArray* srcFaceLocations,
      vtkIdTypeArray* srcFaces, vtkCellArray* dstCells, vtkIdTypeArray*& dstFaceLocations,
      vtkIdTypeArray*& dstFaces, vtkIdTypeArray* matchingReceivedPointIds,
      ...
      vtkIdType connectivitySizeInDest, vtkIdType facesSizeInDest)
    ...
    , FacesOffset(facesSizeInDest)
  {
    ...
    vtkIdType offset = -1;
    for (const auto& pair : this->RedirectionMapForDuplicatePointIds)
    {
      this->PointIdOffsetIntervals.insert({ pair.first, ++offset });
    }
    this->PointIdOffsetIntervals.insert({ std::numeric_limits<vtkIdType>::max(), ++offset });
  }
  ...
  void operator()(vtkIdType startId, vtkIdType endId)
  {
    ...
    for (vtkIdType cellId = startId; cellId < endId; ++cellId)
    {
      ...
      // this->SourceFaces is non nullptr if and only if VTK_POLYHEDRON cells are sent by neighbor
      if (this->SourceFaces && this->SourceFaceLocations->GetValue(cellId) != -1)
      {
        vtkIdType id = this->SourceFaceLocations->GetValue(cellId);
        vtkIdType numberOfFaces = this->SourceFaces->GetValue(id++);

        this->DestFaceLocations->SetValue(this->CellIdOffset + cellId, this->FacesOffset);
        this->DestFaces->SetValue(this->FacesOffset++, numberOfFaces);

        for (vtkIdType faceId = 0; faceId < numberOfFaces; ++faceId)
        {
          vtkIdType faceSize = this->SourceFaces->GetValue(id++);
          this->DestFaces->SetValue(this->FacesOffset++, faceSize);
          ...
          for (vtkIdType facePointId = 0; facePointId < faceSize; ++facePointId)
          {
            ...
            if (pointId >= 0)
            {
              if (this->RedirectionMapForDuplicatePointIds.empty())
              {
                this->DestFaces->SetValue(this->FacesOffset + facePointId,
                    this->PointIdOffset + pointId);
              }
              else
              {
                ...
                {
                  this->DestFaces->SetValue(this->FacesOffset + facePointId, it->second);
                }
              }
            }
            else
...
//----------------------------------------------------------------------------
template<class ArrayT>
void InsertCells(vtkIdTypeArray* matchingReceivedPointIds, vtkCellArray* srcCells,
    vtkIdTypeArray* srcFaceLocations, vtkIdTypeArray* srcFaces, vtkCellArray* dstCells,
    ...
    vtkIdType facesSizeInDest)
{
  CellArrayInserter<ArrayT> inserter(srcCells, srcFaceLocations, srcFaces,
      dstCells, dstFaceLocations, dstFaces,
      matchingReceivedPointIds, redirectionMapForDuplicatePointIds,
      numberOfPointsInDest, numberOfCellsInDest, connectivitySizeInDest, facesSizeInDest);
    ...
    vtkIdType facesSizeInDest)
{
  ...
  if (vtkArrayDownCast<ArrayType32>(srcCells->GetOffsetsArray()))
  {
    InsertCells<ArrayType32>(matchingReceivedPointIds, srcCells, srcFaceLocations, srcFaces,
        dstCells, dstFaceLocations, dstFaces, redirectionMapForDuplicatePointIds,
        numberOfPointsInDest, numberOfCellsInDest, connectivitySizeInDest, facesSizeInDest);
  }
  else
  {
    InsertCells<ArrayType64>(matchingReceivedPointIds, srcCells, srcFaceLocations, srcFaces,
        dstCells, dstFaceLocations, dstFaces, redirectionMapForDuplicatePointIds,
        numberOfPointsInDest, numberOfCellsInDest, connectivitySizeInDest, facesSizeInDest);
  }
}
    ...
    std::vector<vtkUnstructuredGrid*>& inputs, std::vector<vtkUnstructuredGrid*>& outputs)
{
  ...
  for (int localId = 0; localId < static_cast<int>(outputs.size()); ++localId)
  {
    ...
    {
      outputCellArray->Use64BitStorage();
    }
    else
    {
      outputCellArray->Use32BitStorage();
    }
    ...
    if (connectivitySize >> 32)
    {
      outputCellArray->ConvertTo64BitStorage();
    }
    else
    {
    ...
    int gid, UnstructuredGridBlockStructure& blockStructure, vtkUnstructuredGrid* output)
{
  ...
  InsertCells(blockStructure.MatchingReceivedPointIdsSortedLikeTarget, buffer.CellArray,
      buffer.FaceLocations, buffer.Faces, outputCellArray, outputFaceLocations, outputFaces,
      blockStructure.RedirectionMapForDuplicatePointIds,
