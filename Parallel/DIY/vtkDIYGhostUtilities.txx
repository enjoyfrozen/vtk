...
//----------------------------------------------------------------------------
static void InflateBoundingBoxIfNecessaryImpl(
...
//----------------------------------------------------------------------------
static void InflateBoundingBoxIfNecessary(vtkPolyData* input, const double* bounds, vtkBoundingBox& bb)
{
  ...
  diy::all_to_all(
    master, assigner, [&master, &inputs](BlockType* block, const diy::ReduceProxy& srp) {
      ...
      if (srp.round() == 0)
      {
        ...
        vtkDIYGhostUtilities_detail::InflateBoundingBoxIfNecessary(input, bounds, bb);
