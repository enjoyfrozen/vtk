...
public:
   ...
   * `SetNumberOfThreads()` will be queued in the `Controller`,
   ...
   * `Stop()` will be queued in the `Controller`, which executes all received command serially in the
   * background.
   * When the `Controller` is done executing this command, `IsRunning()` effectively returns `false`.
   */
  void Stop();
   ...
   * `Start()` will be queued in the `Controller`, which executes all received command serially in the
   * background.
   * When the `Controller` is done executing this command, `IsRunning()` effectively returns `true`.
   */
  void Start();
...
private:
  ///@{
  /**
   * Worker typedefs that holds the inserted functions and their parameters.
   */
  class BaseWorker;
  template <class FT, class... ArgsT>
  class WorkerWrapper;
  ...
  ///@{
  /**
   * Declaration of the types needed to handle the controller.
   */
  class vtkInternalController;
  template<class ControllerPointerT>
   ...
   * thread. This allows those methods to not be blocking.
   */
  vtkSmartPointer<vtkInternalController> Controller;

  using ControllerHandler = ControllerHelper<decltype(*Controller)>;
};
