...
public:
   ...
   * `Stop()` will be queued in the `Controller`, which executes all received command serially in
   ...
   * `Start()` will be queued in the `Controller`, which executes all received command serially in
   * the background. When the `Controller` is done executing this command, `IsRunning()` effectively
   * returns `true`.
   */
  void Start();
...
private:
  ...
  /**
   * Instantiate a Controller based on its static type. Returns `nullptr` if the type of
   * `Controller` is not `vtkThreadedCallbackQueue::vtkInternalController`.
   */
  template <class ControllerT>
  static vtkSmartPointer<vtkInternalController> CreateController();
