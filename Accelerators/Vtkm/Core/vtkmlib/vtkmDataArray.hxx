...
{
  using SubVecFlatAccess = VecFlatAccessImpl<typename vtkm::VecTraits<VecType>::ComponentType>;
  ...
  {
    return vtkm::VecTraits<VecType>::GetNumberOfComponents(vec) * SubVecFlatAccess::NUM_COMPONENTS;
  }
  ...
  static ValueType GetComponent(const VecType& vec, vtkm::IdComponent flatCompIdx)
  {
    ...
    return SubVecFlatAccess::GetComponent(vec[compIdxThis], compIdxSub);
  }
  ...
  static void SetComponent(VecType& vec, vtkm::IdComponent flatCompIdx, const ValueType& val)
  {
    auto compIdxThis = flatCompIdx / SubVecFlatAccess::NUM_COMPONENTS;
    auto compIdxSub = flatCompIdx % SubVecFlatAccess::NUM_COMPONENTS;
    SubVecFlatAccess::SetComponent(vec[compIdxThis], compIdxSub, val);
  }
  ...
  static void CopyTo(const VecType& vec, ValueType* dest)
  {
    ...
    {
      SubVecFlatAccess::CopyTo(vec[i], dest + (i * SubVecFlatAccess::NUM_COMPONENTS));
    }
  }
  ...
  static void CopyFrom(VecType& vec, const ValueType* src)
  {
    for (vtkm::IdComponent i = 0; i < vtkm::VecTraits<VecType>::GetNumberOfComponents(vec); ++i)
    {
      SubVecFlatAccess::CopyFrom(vec[i], src + (i * SubVecFlatAccess::NUM_COMPONENTS));
    }
  }
};
...
struct VecFlatAccess
{
  ...
  template <typename VecType>
  VTKM_EXEC_CONT
  static auto GetComponent(const VecType& vec, vtkm::IdComponent flatCompIdx)
  {
  ...
  template <typename VecType>
  VTKM_EXEC_CONT
  static void SetComponent(VecType& vec, vtkm::IdComponent flatCompIdx,
  ...
  VTKM_EXEC_CONT
  static void SetComponent(vtkm::VecFromPortal<PortalType>& vec, vtkm::IdComponent flatCompIdx,
  ...
  template <typename VecType>
  VTKM_EXEC_CONT
  static void CopyTo(const VecType& vec, typename VecFlatAccessImpl<VecType>::ValueType* dest)
  {
  ...
  template <typename VecType>
  VTKM_EXEC_CONT
  static void CopyFrom(VecType& vec, const typename VecFlatAccessImpl<VecType>::ValueType* src)
  {
  ...
  template <typename PortalType>
  VTKM_EXEC_CONT
  static void CopyFrom(vtkm::VecFromPortal<PortalType>& vec,
