...
Start-Process -NoNewWindow -Wait "$outdir\$installer" -ArgumentList "/quiet /passive InstallLauncherAllUsers=0 AssociateFiles=0 Shortcuts=0 PrependPath=0 InstallAllUsers=0 TargetDir=$installdir Include_launcher=0 Include_doc=0 Include_debug=0 Include_tcltk=0 Include_test=0"
