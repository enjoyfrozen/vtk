...
//------------------------------------------------------------------------------
int vtkEnSightGoldReader::ReadAsymmetricTensorsPerNode(const char* fileName, const char* description,
...
//------------------------------------------------------------------------------
int vtkEnSightGoldReader::ReadAsymmetricTensorsPerElement(const char* fileName, const char* description,
  int timeStep, vtkMultiBlockDataSet* compositeOutput)
{
  std::cout<<"vtkEnSightGoldReader"<<std::endl;
          ...
          for (i = 0; i < 9; i++)
          {
            for (j = 0; j < numCellsPerElement; j++)
            {
              ...
              tensors->InsertComponent(
                this->GetCellIds(idx, elementType)->GetId(j), i, value);
            }
          }
          lineRead = this->ReadNextDataLine(line);
        } // end while
