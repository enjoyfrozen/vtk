    ...
    else if (strncmp(line, "tensor", 6) == 0)
    {
      ...
      if (!((strcmp(symm, "symm") == 0 || strcmp(symm, "asym") == 0) && strcmp(per, "per") == 0))
      {
        ...
        if (strcmp(per, "per") == 0)
        {
          ...
          vtkWarningMacro("Looks almost like a valid case file, continuing assuming a symmetric tensor");
        }
        else
        {
      ...
      if (strcmp(subLine, "node:") == 0)
      {
        ...
        this->VariableMode = asym ? vtkEnSightReader::TENSOR_ASYM_PER_NODE : vtkEnSightReader::TENSOR_SYMM_PER_NODE;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      else if (strcmp(subLine, "element:") == 0)
      {
        ...
        this->VariableMode = asym ? vtkEnSightReader::TENSOR_ASYM_PER_ELEMENT : vtkEnSightReader::TENSOR_SYMM_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
    ...
    if (validTime)
    {
      switch (this->VariableTypes[i])
      {
        ...
        case vtkEnSightReader::TENSOR_ASYM_PER_NODE:
          this->ReadAsymmetricTensorsPerNode(fileName, this->VariableDescriptions[i], timeStepInFile, output);
          break;
