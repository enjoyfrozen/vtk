...
protected:
  ...
  struct PartInfo
  {
    PartInfo(vtkLSDynaPart* p, const int& type, const vtkIdType& pId, const vtkIdType& start,
      const vtkIdType& npts)
      : numCells(1)
      , // we are inserting the first cell when we create this so start with 1
      startId(start)
      , cellStructureSize(npts)
      , // start with number of points in first cell
      partId(pId)
    {
  ...
  struct PartInsertion
  {
    ...
    PartInsertion(std::vector<PartInfo>* pInfo)
      : numCellsInserted(0)
    {
...
public:
  ...
  //---------------------------------------------------------------------------
  void RegisterCell(const int& partType, const vtkIdType& matId, const vtkIdType& npts)
  {
    if (!this->Info[partType].empty())
    {
      ...
      else
      {
        ...
        PartInfo newInfo(
          this->Parts[matId], partType, matId, (info->startId + info->numCells), npts);
  ...
  //---------------------------------------------------------------------------
  void ConstructPart(const vtkIdType& index, const std::string& name, const int& materialId,
    const int& numGlobalNodes, const int& wordSize)
  {
    vtkLSDynaPart* p = vtkLSDynaPart::New();
    p->InitPart(name, index, materialId, numGlobalNodes, wordSize);
    this->Parts[index] = p;
  }
  ...
  //---------------------------------------------------------------------------
  void InsertCell(
    const int& partType, const int& cellType, const vtkIdType& npts, vtkIdType conn[8])
  {
    ...
    if (this->CellInsertionIterators[partType].pIt->part)
    {
      ...
      this->CellInsertionIterators[partType].pIt->part->AddCell(cellType, npts, conn);
    }
  ...
  //---------------------------------------------------------------------------
  vtkLSDynaPart* GetPart(const vtkIdType& index) { return this->Parts[index]; }
  ...
  //---------------------------------------------------------------------------
  bool GetInfoForPart(vtkLSDynaPart* part, vtkIdType& numCells, vtkIdType& cellArrayLength) const

  {
    ...
    for (it = this->Info[part->PartType()].begin(); it != this->Info[part->PartType()].end(); ++it)
    {
  ...
  void DumpPartInfo()
  {
    for (int i = 0; i < LSDynaMetaData::NUM_CELL_TYPES; ++i)
    {
      ...
      std::vector<PartInfo>::const_iterator it;
      for (it = this->Info[i].begin(); it != this->Info[i].end(); ++it)
      {
        const PartInfo* info = &(*it);
  ...
  //---------------------------------------------------------------------------
  bool GetNextCellPart(vtkIdType& startId, vtkIdType& numCells, vtkLSDynaPart*& part)
  {
       ...
       ++partMIt, ++statusIt, ++nameIt, ++materialIdIt)
  {
    if (*statusIt)
    {
      ...
      this->Storage->ConstructPart((*partMIt) - 1, *nameIt, *materialIdIt,
        this->MetaData->NumberOfNodes, this->MetaData->Fam.GetWordSize());
    }
  }
}
  ...
  vtkIdType& numCellsToSkipStart, vtkIdType& numCellsToSkipEnd) const
{
  ...
  else
  {
    ...
    numCellsToSkipEnd =
      this->MetaData->NumberOfCells[partType] - (numberOfCells + numCellsToSkipStart);
  }
}
...
{
  this->SetupPointPropertyForReading(
  ...
  const bool& isGeometryPoints, const bool& isRoadPoints)
{
  ...
  if (!isRoadPoints)
  {
    enum LSDynaMetaData::LSDYNA_TYPES validCellTypes[5] = { LSDynaMetaData::PARTICLE,
      LSDynaMetaData::BEAM, LSDynaMetaData::SHELL, LSDynaMetaData::THICK_SHELL,
    ...
    for (int i = 0; i < 5; ++i)
    {
      ...
      while (this->Storage->GetNextPart(part))
      {
        part->AddPointProperty(name, numComps, isIdType, isProperty, isGeometryPoints);
        validParts[idx++] = part;
      }
    }
  }
...
void vtkLSDynaPartCollection::FillPointProperty(const vtkIdType& numTuples,
  const vtkIdType& numComps, vtkLSDynaPart** parts, vtkIdType numParts)
{
