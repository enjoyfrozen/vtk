...
std::string vtkDICOMFilePath::Join(const std::string& second) const
{
  ...
  if (r > 0)
  {
    ...
    else if (extended && r >= 2 && IsSeparator(second[0]) && IsSeparator(second[1]))
    {
...
#else
  struct stat fs;
  if (stat(this->Path.c_str(), &fs) == 0 && !S_ISDIR(fs.st_mode) && !S_ISREG(fs.st_mode))
  {
    result = true;
  }
#endif
...
std::string vtkDICOMFilePath::GetRealPath() const
{
  ...
  HANDLE h = CreateFileW(
    widepath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
  delete[] widepath;
  ...
  if (n != 0)
  {
    ...
    if (path[0] == '\0' || strncmp("\\\\?\\", path, 4) == 0 || strncmp("\\\\.\\", path, 4) == 0)
    {
      result = path;
    }
    else
...
void vtkDICOMFilePath::PushBack(const std::string& second)
{
  ...
  else if (this->Path.length() == 4 && HasExtendedPrefix(this->Path))
  {
    ...
    (DriveLetter(this->Path) == DriveLetter(second) || RootLength(second) == 1))
  {
    this->Path = this->Join(second);
  }
...
bool vtkDICOMFilePath::HasExtendedPrefix(const std::string& path)
{
  ...
  return (path.length() >= 4 && path[0] == '\\' && path[1] == '\\' && path[3] == '\\' &&
    (path[2] == '\?' || path[2] == '.'));
}
#endif
...
wchar_t* vtkDICOMFilePath::ConvertToWideChar(const char* filename)
{
  ...
  if (n > 0)
  {
    ...
    n = MultiByteToWideChar(CP_UTF8, 0, filename, -1, wideFilename, n);
    if (n == 0)
    {
      delete[] wideFilename;
      wideFilename = 0;
...
{
  char* filename = 0;
  int n = WideCharToMultiByte(CP_UTF8, 0, wideFilename, -1, NULL, 0, 0, 0);
  if (n > 0)
  {
    filename = new char[n];
    n = WideCharToMultiByte(CP_UTF8, 0, wideFilename, -1, filename, n, 0, 0);
    if (n == 0)
    {
      delete[] filename;
      filename = 0;
...
{
  char* filename = 0;
  int n = WideCharToMultiByte(CP_ACP, 0, wideFilename, -1, NULL, 0, 0, 0);
  if (n > 0)
  {
    filename = new char[n];
    n = WideCharToMultiByte(CP_ACP, 0, wideFilename, -1, filename, n, 0, 0);
    if (n == 0)
    {
      delete[] filename;
      filename = 0;
