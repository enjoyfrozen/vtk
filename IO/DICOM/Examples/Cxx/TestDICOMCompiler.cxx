...
{
  const unsigned int table[256] = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419,
...
int main(int argc, char* argv[])
{
  ...
  vtkSmartPointer<vtkDICOMMetaData> data = vtkSmartPointer<vtkDICOMMetaData>::New();
  ...
  for (int j = 0; j < m; j++)
  {
    ...
    for (; k <= kl; k++)
    {
      ...
      vtkSmartPointer<vtkTypeInt64Array> offsetArray = vtkSmartPointer<vtkTypeInt64Array>::New();
      ...
      for (int i = 0; i < l; i++)
      {
        ...
        const char* instanceUID = data->Get(i, DC::SOPInstanceUID).GetCharData();
        if (instanceUID == 0)
        {
          instanceUID = data->Get(i, DC::MediaStorageSOPInstanceUID).GetCharData();
        }
        compiler->SetSOPInstanceUID(instanceUID);
        compiler->SetTransferSyntaxUID(data->Get(i, DC::TransferSyntaxUID).GetCharData());
        compiler->SetSeriesInstanceUID(data->Get(i, DC::SeriesInstanceUID).GetCharData());
        compiler->SetImplementationClassUID(data->Get(i, DC::ImplementationClassUID).GetCharData());
