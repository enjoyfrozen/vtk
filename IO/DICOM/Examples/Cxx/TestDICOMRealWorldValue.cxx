...
int main(int argc, char* argv[])
{
  ...
  else if (argc == 1)
  {
    ...
    mappingItem.Set(DC::MeasurementUnitsCodeSequence, vtkDICOMValue(vtkDICOMVR::SQ, unitsItem));
    ...
    mappingItem.Set(DC::RealWorldValueFirstValueMapped, vtkDICOMValue(vtkDICOMVR::US, 0));
    mappingItem.Set(DC::RealWorldValueLastValueMapped, vtkDICOMValue(vtkDICOMVR::US, 4095));
    mappingItem.Set(DC::RealWorldValueIntercept, 0.234);
    mappingItem.Set(DC::RealWorldValueSlope, 0.438);

    meta->Set(DC::RealWorldValueMappingSequence, vtkDICOMValue(vtkDICOMVR::SQ, mappingItem));
  }
  else
  {
  ...
  {
    const vtkDICOMItem& mappingItem = meta->Get(DC::RealWorldValueMappingSequence).GetItem(0);

    std::string lutName = mappingItem.Get(DC::LUTLabel).AsString();

    std::string units =
      mappingItem.Get(vtkDICOMTagPath(DC::MeasurementUnitsCodeSequence, 0, DC::CodeValue))
        .AsString();
