...
void vtkDICOMToRAS::CheckNeedToReorder()
{
  ...
  vtkMatrix4x4* inputMatrix = (this->RASToDICOM == 0 ? this->PatientMatrix : this->RASMatrix);
  if (inputMatrix)
  {
  ...
  if (this->RASToDICOM)
  {
    ...
    this->ReorderColumns = (this->AllowColumnReordering && hdir[0] + hdir[1] < 0);

    // for RAS, vdir should be inferior (-z) or posterior (+y)
    this->ReorderRows = (this->AllowRowReordering && vdir[1] - vdir[2] < 0);
  }
  else
  {
...
//----------------------------------------------------------------------------
void vtkDICOMToRAS::ComputeMatrix(int extent[6], double spacing[3], double origin[3])
{
...
//----------------------------------------------------------------------------
int vtkDICOMToRAS::RequestInformation(vtkInformation* vtkNotUsed(request),
...
//----------------------------------------------------------------------------
int vtkDICOMToRAS::RequestUpdateExtent(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
...
template <class T>
void vtkDICOMToRASExecute(const T* inPtr, T* outPtr, const int flip[3], int numComponents,
  const int inExt[6], const int outExt[6], const int outExecuteExt[6], vtkAlgorithm* progress)
{
...
//----------------------------------------------------------------------------
void vtkDICOMToRAS::ThreadedRequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* vtkNotUsed(outputVector),
  vtkImageData*** inData, vtkImageData** outData, int outExecuteExt[6], int threadId)
{
  vtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  ...
  if (outScalarType == inScalarType)
  {
    switch (inScalarType)
    {
      vtkTemplateAliasMacro(vtkDICOMToRASExecute(static_cast<const VTK_TT*>(inPtr),
        static_cast<VTK_TT*>(outPtr), flip, numComponents, inExt, outExt, outExecuteExt, progress));
      default:
