...
vtkDICOMDataElement* vtkDICOMMetaData::FindDataElement(vtkDICOMTag tag)
{
  unsigned int m = METADATA_HASH_SIZE - 1;
  unsigned int i = (tag.ComputeHash() & m);
  vtkDICOMDataElement** htable = this->Table;
...
//----------------------------------------------------------------------------
const vtkDICOMValue* vtkDICOMMetaData::FindAttributeValue(int idx, vtkDICOMTag tag)
{
...
//----------------------------------------------------------------------------
const vtkDICOMValue* vtkDICOMMetaData::FindAttributeValue(int idx, const vtkDICOMTagPath& tagpath)
{
...
//----------------------------------------------------------------------------
const vtkDICOMValue& vtkDICOMMetaData::Get(int idx, const vtkDICOMTagPath& tagpath)
{
...
//----------------------------------------------------------------------------
const vtkDICOMValue& vtkDICOMMetaData::Get(int idx, int frame, const vtkDICOMTagPath& tagpath)
{
  // if either of these is present in an enhanced DICOM file, then they
  // will be searched before the root is searched
  const DC::EnumType fgs[2] = { DC::PerFrameFunctionalGroupsSequence,
...
{
  if (this->FileIndexArray == 0 || sliceIdx < 0 ||
    sliceIdx >= this->FileIndexArray->GetNumberOfTuples() || compIdx < 0 || compIdx >= numComp)
  {
    return -1;
  }
...
{
  if (this->FrameIndexArray == 0 || sliceIdx < 0 ||
    sliceIdx >= this->FrameIndexArray->GetNumberOfTuples() || compIdx < 0 || compIdx >= numComp)
  {
    return -1;
  }
...
vtkDICOMDataElement* vtkDICOMMetaData::FindDataElementOrInsert(vtkDICOMTag tag)
{
  unsigned int m = METADATA_HASH_SIZE - 1;
  unsigned int i = (tag.ComputeHash() & m);
  vtkDICOMDataElement** htable = this->Table;
...
int vtkDICOMMetaData::FindItemsOrInsert(
  int idx, bool useidx, const vtkDICOMTagPath& tagpath, vtkDICOMItem* itemarray[])
{
  ...
  for (int k = 0; k < count; k++)
  {
    ...
    if (n > m)
    {
      ...
      if (v.IsValid())
      {
        vrForXS = (v.AsUnsignedShort() == 0 ? vtkDICOMVR::US : vtkDICOMVR::SS);
      }
    }
...
vtkDICOMItem* vtkDICOMMetaData::FindItemOrInsert(int idx, const vtkDICOMTagPath& tagpath)
{
...
void vtkDICOMMetaData::Set(vtkDICOMTag tag, double v)
{
  ...
  {
    vtkErrorMacro("SetAttributeValue: could not find tag (" << tag << ") in the dictionary");
  }
}
...
void vtkDICOMMetaData::Set(const vtkDICOMTagPath& tagpath, const vtkDICOMValue& v)
{
  if (tagpath.HasTail())
  {
...
void vtkDICOMMetaData::Set(const vtkDICOMTagPath& tagpath, const std::string& v)
{
  if (tagpath.HasTail())
  {
    vtkDICOMItem** items = new vtkDICOMItem*[this->NumberOfInstances];
...
//----------------------------------------------------------------------------
// Insert an attribute for a particular image
void vtkDICOMMetaData::Set(int idx, const vtkDICOMTagPath& tagpath, const vtkDICOMValue& v)
{
  if (tagpath.HasTail())
  {
...
void vtkDICOMMetaData::Set(int idx, const vtkDICOMTagPath& tagpath, const std::string& v)
{
  if (tagpath.HasTail())
  {
    vtkDICOMItem* item = this->FindItemOrInsert(idx, tagpath);
    ...
    if (otable != 0)
    {
      ...
      while (iter != iterEnd)
      {
        ...
        else if (this->NumberOfInstances == o->NumberOfInstances)
        {
          ...
          e->Tag = iter->Tag;
          vtkDICOMValue* nvptr =
            e->Value.AllocateMultiplexData(iter->Value.GetVR(), this->NumberOfInstances);
          for (int i = 0; i < this->NumberOfInstances; i++)
          {
            nvptr[i] = vptr[i];
          }
  ...
  int idx, vtkDICOMTag ptag, const std::string& creator)
{
  ...
  {
    ctag = iter->GetTag();
    const vtkDICOMValue* vp = (iter->IsPerInstance() ? &iter->GetValue(idx) : &iter->GetValue());
    if (vp->AsString() == creator)
    {
      otag = vtkDICOMTag(g, (ctag.GetElement() << 8) | (ptag.GetElement() & 0x00FF));
      break;
    }
    ++iter;
  }
...
//----------------------------------------------------------------------------
vtkDICOMTag vtkDICOMMetaData::ResolvePrivateTag(vtkDICOMTag ptag, const std::string& creator)
{
  ...
  if (otag == vtkDICOMTag(0xFFFF, 0xFFFF))
  {
    ...
    for (unsigned short e = 0x0010; e <= 0x00FF; e++)
    {
      ...
      if (!d->Value.IsValid())
      {
        ...
        d->Tag = ctag;
        d->Value = this->MakeValueWithSpecificCharacterSet(vtkDICOMVR::LO, creator);
