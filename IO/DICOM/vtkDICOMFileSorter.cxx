...
class vtkDICOMFileSorter::StringArrayVector : public std::vector<vtkSmartPointer<vtkStringArray> >
{
};
...
bool vtkDICOMFileSorter::CompareInstance(const FileInfo& fi1, const FileInfo& fi2)
{
...
void vtkDICOMFileSorter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "InputFileName: " << (inputFileName ? inputFileName : "(NULL)") << "\n";
...
void vtkDICOMFileSorter::SetInputFileName(const char* name)
{
  ...
  if (name)
  {
    this->SetInputFileNames(0);
  }

  if (name == this->InputFileName ||
    (name && this->InputFileName && strcmp(name, this->InputFileName) == 0))
  {
    return;
  }
...
{
  vtkSmartPointer<vtkUnsignedShortArray> groups = vtkSmartPointer<vtkUnsignedShortArray>::New();
  ...
  parser->AddObserver(vtkCommand::ErrorEvent, this, &vtkDICOMFileSorter::RelayError);
  ...
  for (vtkIdType j = 0; j < numberOfStrings; j++)
  {
    ...
    for (li = sortedFiles.begin(); li != sortedFiles.end(); ++li)
    {
      ...
      int c1 = vtkDICOMUtilities::CompareUIDs(studyUID, (*li)[0].StudyUID.GetCharData());
      int c2 = 0;
      if (c1 == 0)
      {
        c2 = vtkDICOMUtilities::CompareUIDs(seriesUID, (*li)[0].SeriesUID.GetCharData());
      }
  ...
  if (this->InputFileName) // The input was a single file
  {
    ...
    if (d.GetError() == 0)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        if (vtkDICOMUtilities::PatternMatches(pattern.c_str(), filename) && !d.IsDirectory(i))
        {
...
void vtkDICOMFileSorter::Update()
{
  if (this->GetMTime() > this->UpdateTime.GetMTime() ||
    (this->InputFileNames && this->InputFileNames->GetMTime() > this->UpdateTime.GetMTime()))
  {
    this->Execute();
