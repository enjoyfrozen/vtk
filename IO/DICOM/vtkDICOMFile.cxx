...
vtkDICOMFile::vtkDICOMFile(const char* filename, Mode mode)
{
  ...
  if (wideFilename)
  {
    if (mode == In)
    {
      this->Handle = CreateFileW(wideFilename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
        NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    }
    else if (mode == Out)
    {
      this->Handle = CreateFileW(wideFilename, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
size_t vtkDICOMFile::Read(unsigned char* data, size_t len)
{
  ...
  if (n != len || len == 0)
  {
    ...
    this->Error = (ferror(static_cast<FILE*>(this->Handle)) == 0 ? 0 : UnknownError);
  }
  return n;
#endif
}
...
int vtkDICOMFile::Access(const char* filename, Mode mode)
{
  ...
  int errorCode = 0;
  struct stat fs;
  if (stat(filename, &fs) != 0 || access(filename, (mode == In ? R_OK : W_OK)) != 0)
  {
    int e = errno;
    if (e == EACCES || e == EPERM)
    {
