...
class vtkDICOMDirectory::SeriesVector : public std::vector<vtkDICOMDirectory::SeriesItem>
{
};

class vtkDICOMDirectory::StudyVector : public std::vector<vtkDICOMDirectory::StudyItem>
{
};

class vtkDICOMDirectory::PatientVector : public std::vector<vtkDICOMDirectory::PatientItem>
{
};

class vtkDICOMDirectory::VisitedVector : public std::vector<std::string>
{
};
...
struct vtkDICOMDirectory::FileInfoPair
{
  ...
  FileInfoPair(const char* key, FileInfo* ptr)
    : Key(key)
    , Info(ptr)
  {
  }
...
bool vtkDICOMDirectory::CompareInstanceUIDs(const FileInfoPair& p, const char* uid)
{
  return (vtkDICOMUtilities::CompareUIDs(p.Key, uid) < 0);
}

bool vtkDICOMDirectory::CompareInstance(const FileInfo& fi1, const FileInfo& fi2)
{
...
bool vtkDICOMDirectory::CompareSeriesUIDs(const SeriesInfo* si, const char* uid)
{
  return (vtkDICOMUtilities::CompareUIDs(si->SeriesUID.GetCharData(), uid) < 0);
}

bool vtkDICOMDirectory::CompareSeriesIds(const SeriesInfo* si1, const SeriesInfo* si2)
{
  ...
  if (c == 0)
  {
    c = vtkDICOMUtilities::CompareUIDs(si1->StudyUID.GetCharData(), si2->StudyUID.GetCharData());

    if (c == 0)
    {
      c =
        vtkDICOMUtilities::CompareUIDs(si1->SeriesUID.GetCharData(), si2->SeriesUID.GetCharData());
    }
  }
...
bool vtkDICOMDirectory::CompareSeriesInfo(const SeriesInfo& si1, const SeriesInfo& si2)
{
...
static const DC::EnumType SeriesTags[] = { DC::SpecificCharacterSet, DC::SeriesDate, DC::SeriesTime,
  DC::Modality, DC::SeriesDescription, DC::SeriesInstanceUID, DC::SeriesNumber,
  DC::ItemDelimitationItem };

static const DC::EnumType StudyTags[] = { DC::SpecificCharacterSet, DC::StudyDate, DC::StudyTime,
  DC::ReferringPhysicianName, DC::PatientAge, DC::StudyInstanceUID, DC::StudyID,
  DC::AccessionNumber, DC::StudyDescription, DC::ItemDelimitationItem };

static const DC::EnumType PatientTags[] = { DC::SpecificCharacterSet, DC::PatientName,
  DC::PatientID, DC::PatientBirthDate, DC::PatientSex, DC::ItemDelimitationItem };
...
class vtkDICOMDirectory::SeriesInfoList : public std::list<vtkDICOMDirectory::SeriesInfo>
{
};

class vtkDICOMDirectory::SeriesInfoVector : public std::vector<vtkDICOMDirectory::SeriesInfo*>
{
};
...
{
public:
  void SetFrom(const vtkDICOMItem& patientRecord, const vtkDICOMItem& studyRecord,
    const vtkDICOMItem& seriesRecord);
};

void SortedTags::SetFrom(const vtkDICOMItem& patientRecord, const vtkDICOMItem& studyRecord,
  const vtkDICOMItem& seriesRecord)
{
  this->clear();
  this->reserve(patientRecord.GetNumberOfDataElements() + studyRecord.GetNumberOfDataElements() +
    seriesRecord.GetNumberOfDataElements());

  const vtkDICOMItem* records[3] = { &patientRecord, &studyRecord, &seriesRecord };
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    while (iter != iterEnd)
    {
      ...
      std::vector<vtkDICOMTag>::iterator pos = std::lower_bound(begin(), end(), tag);
      if (pos == end() || *pos != tag)
      {
        this->insert(pos, tag);
      }
...
void vtkDICOMDirectory::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "DirectoryName: " << (inputDirectory ? inputDirectory : "(NULL)") << "\n";
  ...
  os << indent
     << "FindLevel: " << (this->FindLevel == vtkDICOMDirectory::IMAGE ? "IMAGE\n" : "SERIES\n");
...
void vtkDICOMDirectory::SetDirectoryName(const char* name)
{
  if (name == this->DirectoryName ||
    (name && this->DirectoryName && strcmp(name, this->DirectoryName) == 0))
  {
    return;
  }
...
void vtkDICOMDirectory::SetFilePattern(const char* name)
{
  if (name == this->FilePattern ||
    (name && this->FilePattern && strcmp(name, this->FilePattern) == 0))
  {
    return;
  }
...
bool MatchesOsirixDatabase(vtkDICOMTag tag, const vtkDICOMValue& u, const vtkDICOMValue& v)
{
  bool needsCleanCompare = false;
  unsigned short g = tag.GetGroup();
  if (u.GetNumberOfValues() > 0 && v.GetNumberOfValues() > 0 && (g == 0x0008 || g == 0x0010))
  {
    const DC::EnumType tagsToClean[] = { DC::StudyDescription, DC::SeriesDescription,
      DC::PatientName, DC::InstitutionName, DC::ReferringPhysicianName, DC::PerformingPhysicianName,
      DC::ItemDelimitationItem };
  ...
  if (needsCleanCompare)
  {
    vtkDICOMValue uclean(
      u.GetVR(), vtkDICOMCharacterSet::ISO_IR_192, OsirixCleanString(u.AsUTF8String()));
    vtkDICOMValue vclean(
      v.GetVR(), vtkDICOMCharacterSet::ISO_IR_192, OsirixCleanString(v.AsUTF8String()));
...
//----------------------------------------------------------------------------
bool vtkDICOMDirectory::MatchesQuery(const vtkDICOMItem& record, vtkDICOMItem& results)
{
  bool matched = true;
...
//----------------------------------------------------------------------------
int vtkDICOMDirectory::MatchesImageQuery(const vtkDICOMItem& record, const vtkDICOMItem& results)
{
    ...
    for (iter = this->Query->Begin(); iter != this->Query->End(); ++iter)
    {
      ...
      if (v.GetVR() == vtkDICOMVR::SQ)
      {
        if (v.GetNumberOfValues() > 0 ||
          (!results.Get(tag).IsValid() && !record.Get(tag).IsValid()))
        {
          fullyMatched = false;
        }
      }
      else if (tag != DC::SpecificCharacterSet && tag.GetGroup() != 0x0004)
      {
        ...
        else if (!results.Get(tag).IsValid() && !record.Get(tag).IsValid())
        {
          fullyMatched = false;
        }
      }
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::AddSeriesWithQuery(int patient, int study, vtkStringArray* files,
  const vtkDICOMItem& patientRecord, const vtkDICOMItem& studyRecord,
  const vtkDICOMItem& seriesRecord, const vtkDICOMItem* imageRecords[])
{
  if (this->Query == 0)
  {
    this->AddSeriesFileNames(
      patient, study, files, patientRecord, studyRecord, seriesRecord, imageRecords);
    return;
  }
  ...
  if (this->MatchesQuery(patientRecord, results) && this->MatchesQuery(studyRecord, results) &&
    this->MatchesQuery(seriesRecord, results))
  {
    ...
    else if (this->QueryFiles == -1)
    {
      ...
      for (iter = this->Query->Begin(); iter != this->Query->End(); ++iter)
      {
        ...
        if (v.GetVR() == vtkDICOMVR::SQ)
        {
          if (v.GetNumberOfValues() > 0 || !results.Get(tag).IsValid())
          {
            fullyMatched = false;
            break;
          }
    ...
    parser->AddObserver(vtkCommand::ErrorEvent, this, &vtkDICOMDirectory::RelayError);
    ...
    for (vtkIdType i = 0; i < n; i++)
    {
      ...
      else if (r >= 0)
      {
        ...
        this->FillImageRecord(&storeImageRecords[i], meta, &skip[0], skip.size());
    ...
    {
      this->AddSeriesFileNames(
        patient, study, a, patientRecord, studyRecord, seriesRecord, &newImageRecords[0]);
    }
  }
}

//----------------------------------------------------------------------------
void vtkDICOMDirectory::AddSeriesFileNames(int patient, int study, vtkStringArray* files,
  const vtkDICOMItem& patientRecord, const vtkDICOMItem& studyRecord,
  const vtkDICOMItem& seriesRecord, const vtkDICOMItem* imageRecords[])
{
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::CopyRecord(vtkDICOMMetaData* meta, const vtkDICOMItem* item, int instance)
{
...
void vtkDICOMDirectory::FillImageRecord(
  vtkDICOMItem* item, vtkDICOMMetaData* meta, const vtkDICOMTag* skip, size_t nskip)
{
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::FillSeriesRecord(vtkDICOMItem* item, vtkDICOMMetaData* meta)
{
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::FillStudyRecord(vtkDICOMItem* item, vtkDICOMMetaData* meta)
{
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::FillPatientRecord(vtkDICOMItem* item, vtkDICOMMetaData* meta)
{
...
{
  vtkSmartPointer<vtkDICOMMetaData> meta = vtkSmartPointer<vtkDICOMMetaData>::New();
  vtkSmartPointer<vtkDICOMMetaData> query = vtkSmartPointer<vtkDICOMMetaData>::New();
  vtkSmartPointer<vtkDICOMParser> parser = vtkSmartPointer<vtkDICOMParser>::New();
  parser->SetDefaultCharacterSet(this->DefaultCharacterSet);
  parser->SetOverrideCharacterSet(this->OverrideCharacterSet);
  ...
  for (const DC::EnumType* tagPtr = ScanTags; *tagPtr != DC::ItemDelimitationItem; ++tagPtr)
  {
  ...
  for (vtkIdType j = 0; j < numberOfStrings; j++)
  {
    ...
    SeriesInfoVector::iterator vib =
      std::lower_bound(seriesByUID.begin(), seriesByUID.end(), seriesUID, CompareSeriesUIDs);

    // Iterate through all possible matches
    for (SeriesInfoVector::iterator vi = vib; vi != seriesByUID.end() &&
         vtkDICOMUtilities::CompareUIDs((*vi)->SeriesUID.GetCharData(), seriesUID) == 0;
         ++vi)
    {
      SeriesInfo& v = *(*vi);
      ...
      if ((seriesUID == 0 || seriesUID[0] == '\0') && seriesNumber != v.SeriesNumber)
      {
        continue;
      }

      // Ensure that the StudyInstanceUID also matches
      if (vtkDICOMUtilities::CompareUIDs(v.StudyUID.GetCharData(), studyUID) != 0)
      {
        continue;
      }
      ...
      std::vector<FileInfoPair>::iterator im =
        std::lower_bound(v.FilesByUID.begin(), v.FilesByUID.end(), imageUID, CompareInstanceUIDs);
        ...
        if (vtkDICOMUtilities::CompareUIDs(imageUID, im->Key) == 0)
        {
          ...
          if (imageUID == 0 || imageUID[0] == '\0')
          {
            ...
            do
            {
              ...
              im != v.FilesByUID.end() && vtkDICOMUtilities::CompareUIDs(im->Key, imageUID) == 0);
          }
          else
          {
  ...
  for (SeriesInfoVector::iterator vi = seriesByUID.begin(); vi != seriesByUID.end(); ++vi)
  {
    SeriesInfo& v = *(*vi);
    ...
    if (!lastInfo || v.PatientID != lastInfo->PatientID)
    {
      ...
      if (cp)
      {
        ...
        v.PatientName =
          vtkDICOMValue(vtkDICOMVR::PN, vtkDICOMCharacterSet::ISO_IR_192, cs.CaseFoldedUTF8(cp, l));
      }
      lastInfo = &v;
    }
  ...
  for (li = seriesList.begin(); li != seriesList.end(); ++li)
  {
    ...
    this->AddSeriesFileNames(patientCount - 1, studyCount - 1, sa, v.PatientRecord, v.StudyRecord,
      v.SeriesRecord, &imageRecords[0]);
  }
}
...
bool SimpleSQL::Open(const char* fname)
{
  ...
  for (size_t i = 0; i < fullpath.length(); i++)
  {
    ...
    if (c < ' ' || c > '~')
    {
      ...
      for (size_t j = 0; j < sizeof(uri_reserved); j++)
      {
        if (c == uri_reserved[j])
        {
          use_percent = true;
  ...
  int r = sqlite3_open_v2(uri.c_str(), &this->DBase, SQLITE_OPEN_READONLY | SQLITE_OPEN_URI, 0);
  if (r == SQLITE_OK)
  {
    char* errmsg;
...
bool SimpleSQL::Prepare(const char* query)
{
  ...
  return (sqlite3_prepare_v2(this->DBase, query, l, &this->Statement, &ep) == SQLITE_OK);
}
...
vtkVariant SimpleSQL::GetValue(int column)
{
  ...
  switch (sqlite3_column_type(this->Statement, column))
  {
    ...
    case SQLITE_TEXT:
    {
      const char* x = reinterpret_cast<const char*>(sqlite3_column_text(this->Statement, column));
      v = vtkVariant(x);
      break;
    }
    case SQLITE_BLOB:
    {
      const char* x = static_cast<const char*>(sqlite3_column_blob(this->Statement, column));
...
void vtkDICOMDirectory::ProcessOsirixDatabase(const char* fname)
{
  ...
  enum
  {
    ...
    SE_NUMBEROFIMAGES,
    SE_DATE,
    SE_SERIESSOPCLASSUID,
    SE_MODALITY,
    SE_NAME,
    SE_SERIESDICOMUID,
    SE_SERIESDESCRIPTION,
    SE_NCOLS
  };
  ...
  for (std::vector<StudyRow>::iterator st = studyTable.begin(); st != studyTable.end(); ++st)
  {
    ...
    patientItem.Set(DC::SpecificCharacterSet, vtkDICOMCharacterSet::ISO_IR_192);
    ...
    patientItem.Set(DC::PatientSex, st->col[ST_PATIENTSEX].ToString());

    studyItem.Set(DC::SpecificCharacterSet, vtkDICOMCharacterSet::ISO_IR_192);
    std::string studyName = st->col[ST_STUDYNAME].ToString();
    if (studyName != "Localizers")
    {
      studyItem.Set(DC::StudyDescription, studyName);
    }
    studyItem.Set(DC::StudyID, st->col[ST_ID].ToString());
    studyItem.Set(DC::StudyInstanceUID, st->col[ST_STUDYINSTANCEUID].ToString());
    studyItem.Set(DC::InstitutionName, st->col[ST_INSTITUTIONNAME].ToString());
    studyItem.Set(DC::AccessionNumber, st->col[ST_ACCESSIONNUMBER].ToString());
    ...
    for (std::vector<SeriesRow>::iterator se = seriesTable.begin() + seIdx; se != seriesTable.end();
         ++se)
    {
      ...
      if (this->RequirePixelData && se->col[SE_NUMBEROFIMAGES].ToTypeInt64() == 0)
      {
      ...
      vtkDICOMValue sopClassUID(vtkDICOMVR::UI, se->col[SE_SERIESSOPCLASSUID].ToString());
      std::string seriesUID = se->col[SE_SERIESDICOMUID].ToString();
      // Remove any text before the UID
      size_t k = 0;
      while (k < seriesUID.length() && (seriesUID[k] <= '0' || seriesUID[k] >= '9'))
      {
        k++;
      }
      if (k > 0)
      ...
      seriesItem.Set(DC::SpecificCharacterSet, vtkDICOMCharacterSet::ISO_IR_192);
      seriesItem.Set(DC::SeriesDescription, se->col[SE_NAME].ToString());
      seriesItem.Set(DC::ProtocolName, se->col[SE_SERIESDESCRIPTION].ToString());
      ...
      for (std::vector<ImageRow>::iterator im = imageTable.begin() + imIdx; im != imageTable.end();
           ++im)
      {
        // Break when we find a series that isn't part of the study
        ...
        if (fpath != lastpath)
        {
          ...
          imageRecord.Set(
            DC::SOPInstanceUID, DecompressUID(im->col[IM_COMPRESSEDSOPINSTANCEUID].ToString()));
          imageRecord.Set(DC::InstanceNumber, im->col[IM_INSTANCENUMBER].ToString());
          int rows = im->col[IM_STOREDHEIGHT].ToInt();
          if (rows)
          {
            imageRecord.Set(DC::Rows, rows);
          }
          int cols = im->col[IM_STOREDWIDTH].ToInt();
          if (cols)
          {
            imageRecord.Set(DC::Columns, cols);
          }
      ...
      if (n > 0)
      {
        ...
        this->AddSeriesWithQuery(
          patientIdx, studyIdx, fileNames, patientItem, studyItem, seriesItem, &imageRecords[0]);
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::ProcessDirectoryFile(const char* dirname, vtkDICOMMetaData* meta)
{
  ...
  // Get the first entry.
  unsigned int offset =
    meta->Get(DC::OffsetOfTheFirstDirectoryRecordOfTheRootDirectoryEntity).AsUnsignedInt();
  ...
  while (offset != 0)
  {
    ...
    std::map<unsigned int, unsigned int>::iterator iter = offsetToIndexMap.find(offset);
    offset = 0;
    ...
    if (iter != offsetToIndexMap.end() && iter->second != 0xffffffffu)
    {
      ...
      offset = items[j].Get(DC::OffsetOfTheNextDirectoryRecord).AsUnsignedInt();

      offsetOfChild = items[j].Get(DC::OffsetOfReferencedLowerLevelDirectoryEntity).AsUnsignedInt();

      entryType = items[j].Get(DC::DirectoryRecordType).AsString();
    ...
    else
    {
      ...
      while (offset == 0 && offsetStack.size() > 0)
      {
        ...
        else if (entryType == "SERIES")
        {
          ...
          {
            // Add the series if it passes the query
            this->AddSeriesWithQuery(patientIdx, studyIdx, fileNames, items[patientItem],
              items[studyItem], items[seriesItem], &imageRecords[0]);
...
//----------------------------------------------------------------------------
void vtkDICOMDirectory::ProcessDirectory(const char* dirname, int depth, vtkStringArray* files)
{
  ...
  for (int i = 0; i < n; i++)
  {
      ...
      strcmp(fname, "DICOMDIR") != 0)
    {
      ...
      else if (d.IsDirectory(i))
      {
        ...
        vtkDICOMUtilities::PatternMatches(this->FilePattern, fileString.c_str()))
      {
  ...
  if (this->InputFileNames)
  {
    for (vtkIdType i = 0; i < this->InputFileNames->GetNumberOfValues(); i++)
    {
      ...
      else if (vtkDICOMUtilities::PatternMatches("*.sql", fname.c_str()))
      {
        ...
        vtkDICOMUtilities::PatternMatches(this->FilePattern, fname.c_str()))
      {
...
void vtkDICOMDirectory::SetInternalFileName(const char* name)
{
  ...
  if (this->InternalFileName != 0 && name != 0 && strcmp(this->InternalFileName, name) == 0)
  {
    return;
  }
...
void vtkDICOMDirectory::RelayError(vtkObject* o, unsigned long e, void* data)
{
  if (e == vtkCommand::ErrorEvent)
  {
    ...
    if (parser)
    {
      ...
      if (this->CurrentPatientRecord && this->CurrentStudyRecord && this->CurrentSeriesRecord &&
        this->CurrentImageRecord)
      {
        ...
        msg << "\",\nPatientID=\"" << this->CurrentPatientRecord->Get(DC::PatientID).AsString();
        msg << "\", StudyDate=\"" << this->CurrentStudyRecord->Get(DC::StudyDate).AsString();
        msg << "\", StudyTime=\"" << this->CurrentStudyRecord->Get(DC::StudyTime).AsString();
        msg << "\",\nStudyID=\"" << this->CurrentStudyRecord->Get(DC::StudyID).AsString();
        msg << "\", SeriesNumber=\"" << this->CurrentSeriesRecord->Get(DC::SeriesNumber).AsString();
        msg << "\", InstanceNumber=\""
