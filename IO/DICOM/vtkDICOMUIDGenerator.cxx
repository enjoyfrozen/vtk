...
void vtkDICOMUIDGenerator::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "UIDPrefix: " << (this->UIDPrefix ? this->UIDPrefix : "(NULL)") << "\n";
}
...
void vtkGenerateRandomBytes(unsigned char* bytes, vtkIdType n)
{
  ...
  HCRYPTPROV hProv;
  r = CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_SILENT | CRYPT_VERIFYCONTEXT);
  if (r == 0 && GetLastError() == NTE_BAD_KEYSET)
  {
    r = CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_SILENT | CRYPT_NEWKEYSET);
  }
  if (r != 0)
  {
  ...
  {
    memset(bytes, '\0', n);
    vtkGenericWarningMacro("vtkDICOMUIDGenerator::GenerateUID() failed to read from "
...
char vtkDICOMTagToDigit(vtkDICOMTag tag)
{
  ...
  if (tag == DC::SOPInstanceUID || tag == DC::MediaStorageSOPInstanceUID)
  {
    d = '2';
  }
  else if (tag == DC::SeriesInstanceUID || tag == DC::ConcatenationUID)
  {
    d = '3';
  }
  ...
  else if (tag == DC::FrameOfReferenceUID || tag == DC::VolumeFrameOfReferenceUID ||
    tag == DC::SourceFrameOfReferenceUID || tag == DC::SynchronizationFrameOfReferenceUID ||
...
void vtkGeneratePrefixedUID(
  const unsigned char* r, vtkIdType m, const char* prefix, char d, char uid[64])
{
  size_t i = 0;
  while (*prefix != '\0' && i < 62)
  {
  ...
  if (d >= '1' && d <= '9')
  {
    ...
    static const int maxDigits[16] = { 3, 5, 8, 10, 13, 15, 17, 20, 22, 25, 27, 29, 32, 34, 37,
      39 };
...
std::string vtkDICOMUIDGenerator::GenerateUID(vtkDICOMTag tag)
{
  ...
  if (prefix[0] == '\0' ||
    (prefix[0] == '2' && prefix[1] == '.' && prefix[2] == '2' && prefix[3] == '5' &&
      (prefix[4] == '.' || prefix[4] == '\0')))
  {
