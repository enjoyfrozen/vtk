...
{
public:
  static bool FillBuffer(vtkDICOMParser* parser, const unsigned char*& cp, const unsigned char*& ep)
  {
    return parser->FillBuffer(cp, ep);
  }

  static bool SeekBuffer(
    vtkDICOMParser* parser, const unsigned char*& cp, const unsigned char*& ep, vtkTypeInt64 offset)
  {
    return parser->SeekBuffer(cp, ep, offset);
  }

  static vtkTypeInt64 GetBytesRemaining(
    vtkDICOMParser* parser, const unsigned char* cp, const unsigned char* ep)
  {
    return parser->GetBytesRemaining(cp, ep);
  }

  static vtkTypeInt64 GetBytesProcessed(
    vtkDICOMParser* parser, const unsigned char* cp, const unsigned char* ep)
  {
  ...
  static void ParseError(
    vtkDICOMParser* parser, const unsigned char* cp, const unsigned char* ep, const char* message)
  {
...
public:
  ...
  DecoderContext(vtkDICOMMetaData* meta, int index, vtkDICOMCharacterSet dcs, bool ocs)
    : Prev(0)
    , Item(0)
    , MetaData(meta)
    , Index(index)
    , CurrentTag(0, 0)
    , DefaultCharacterSet(dcs)
    , CharacterSet(ocs ? dcs : vtkDICOMCharacterSet(vtkDICOMCharacterSet::Unknown))
...
vtkDICOMVR DecoderContext::FindDictVR(vtkDICOMTag tag)
{
  ...
  else if ((tag.GetGroup() & 0x1) != 0 && tag.GetElement() >= 0x0010 && tag.GetElement() < 0x0100)
  {
  ...
  else
  {
    ...
    if (de.IsValid())
    {
      ...
      else if (vr == vtkDICOMVR::OX)
      {
        ...
        vr = vtkDICOMVR::OW;
        vtkDICOMTag reftag =
          (tag.GetGroup() == 0x5400 ? DC::WaveformBitsAllocated : DC::BitsAllocated);
...
public:
  ...
  void SetQuery(
    const vtkDICOMDataElementIterator& iter, const vtkDICOMDataElementIterator& iterEnd);
  ...
  virtual bool ReadElements(
    const unsigned char*& cp, const unsigned char*& ep, unsigned int l, vtkDICOMTag delimiter) = 0;
  ...
  virtual vtkDICOMTag Peek(const unsigned char*& cp, const unsigned char*& ep) = 0;

  // Copy bytes from sp to end marker cp into the value "v".
  // If the parameter "v" is NULL, then it will be ignored.
  void CopyBuffer(vtkDICOMValue* v, const unsigned char* sp, const unsigned char* cp);
  ...
  bool CheckBuffer(const unsigned char*& cp, const unsigned char*& ep, size_t n);

  // This overload of CheckBuffer is used within SkipElements to
  // automatically copy the skipped bytes into the value "v".
  // If the parameter "v" is NULL, then it will be ignored.
  bool CheckBuffer(const unsigned char*& cp, const unsigned char*& ep, size_t n, vtkDICOMValue* v,
    const unsigned char*& sp);

  // Get the current byte offset from the start of the file.
  size_t GetByteOffset(const unsigned char* cp, const unsigned char* ep);
...
protected:
  ...
  DecoderBase(vtkDICOMParser* parser, vtkDICOMMetaData* data, int idx)
    : Parser(parser)
    , BaseContext(data, idx, parser->GetDefaultCharacterSet(), parser->GetOverrideCharacterSet())
    , Item(0)
    , MetaData(data)
    , Index(idx)
    , ImplicitVR(false)
...
public:
  ...
  static void GetValues(const unsigned char* ip, unsigned long long* v, size_t n);
  ...
  template <class T>
  size_t ReadData(const unsigned char*& cp, const unsigned char*& ep, T* ptr, size_t n);
  ...
  size_t SkipData(const unsigned char*& cp, const unsigned char*& ep, size_t n);
  ...
  // the first element that is not in the specified group.
  bool ReadElements(
    const unsigned char*& cp, const unsigned char*& ep, unsigned int l, vtkDICOMTag delimiter)
  {
    size_t bytesRead;
  ...
  bool ReadElements(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
    vtkDICOMTag delimiter, size_t& bytesRead);

  // Query the elements of one item within a sequence.
  bool QueryOneItem(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
    vtkDICOMTag delimiter, size_t& bytesRead);
  ...
  // If the parameter "v" is NULL, then it will be ignored.
  bool SkipElements(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
    vtkDICOMTag delimiter, vtkDICOMValue* v);

  // Read the vr and vl and return the number of bytes read
  // (will return zero if an error occurred)
  size_t ReadElementHead(const unsigned char*& cp, const unsigned char*& ep, vtkDICOMTag tag,
    vtkDICOMVR& vr, unsigned int& vl);
  ...
  size_t ReadElementValue(const unsigned char*& cp, const unsigned char*& ep, vtkDICOMVR vr,
    unsigned int vl, vtkDICOMValue& v);

  // Peek ahead to see what the next element is.
  vtkDICOMTag Peek(const unsigned char*& cp, const unsigned char*& ep)
  {
    unsigned short g = 0;
    unsigned short e = 0;
...
//----------------------------------------------------------------------------
inline bool DecoderBase::CheckBuffer(const unsigned char*& cp, const unsigned char*& ep, size_t n)
{
  bool r = true;
...
//----------------------------------------------------------------------------
inline size_t DecoderBase::GetByteOffset(const unsigned char* cp, const unsigned char* ep)
{
  return vtkDICOMParserInternalFriendship::GetBytesProcessed(this->Parser, cp, ep);
}
...
void DecoderBase::HandleMissingAttributes(vtkDICOMTag tag)
{
  ...
  --iter;
  if (iter->GetTag() != this->LastWrittenTag && iter->GetTag().GetGroup() != 0x0002)
  {
    int count = 0;
    do
    {
      count++;
      --iter;
    } while (iter->GetTag() != this->LastWrittenTag && iter->GetTag().GetGroup() != 0x0002);
  ...
  while (this->Query != this->QueryEnd && this->Query->GetTag() < tag)
  {
      ...
      ((qtag.GetGroup() & 1) == 0 || qtag.GetElement() > 0x00ff))
    {
      ...
      if (!matched && (qtag.GetGroup() & 1) != 0)
      {
        ...
        vtkDICOMTag ctag = vtkDICOMTag(qtag.GetGroup(), qtag.GetElement() >> 8);
        ...
        if (iter != this->QueryEnd && iter->GetTag() == ctag)
        {
          ...
          if (this->Item)
          {
            vtkDICOMTag ptag = this->Item->ResolvePrivateTag(qtag, iter->GetValue().AsString());
            matched = (ptag != vtkDICOMTag(0xffff, 0xffff) && this->Item->Get(ptag).IsValid());
          }
          else if (this->Index < 0)
          {
            vtkDICOMTag ptag = this->MetaData->ResolvePrivateTag(qtag, iter->GetValue().AsString());
            matched = (ptag != vtkDICOMTag(0xffff, 0xffff) && this->MetaData->Has(ptag));
          }
          else
          {
            vtkDICOMTag ptag =
              this->MetaData->ResolvePrivateTag(this->Index, qtag, iter->GetValue().AsString());
        ...
        else
        {
          ...
          if (this->Item)
          {
            matched = this->Item->Get(qtag).Matches(this->Query->GetValue());
          }
          else
          {
            matched = this->MetaData->Get(qtag).Matches(this->Query->GetValue());
          }
        }
      }
...
bool DecoderBase::QueryContains(vtkDICOMTag tag)
{
  ...
  if (creator.IsValid())
  {
    ...
    while (iter != this->QueryEnd && iter->GetTag() <= gtag)
    {
      ...
      {
        tag = vtkDICOMTag(g, (iter->GetTag().GetElement() << 8) | (e & 0x00FF));
        break;
      }
      ++iter;
    }
...
//----------------------------------------------------------------------------
template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, char* op, size_t n)
{
  do
  {
    *op++ = static_cast<char>(*ip++);
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, unsigned char* op, size_t n)
{
  do
  {
    *op++ = *ip++;
...
template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, short* op, size_t n)
{
  do
  {
    *op++ = static_cast<short>(Decoder<E>::GetInt16(ip));
    ip += 2;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, unsigned short* op, size_t n)
{
  do
  {
    *op++ = Decoder<E>::GetInt16(ip);
    ip += 2;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, int* op, size_t n)
{
  do
  {
    *op++ = static_cast<int>(Decoder<E>::GetInt32(ip));
    ip += 4;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, unsigned int* op, size_t n)
{
  do
  {
    *op++ = Decoder<E>::GetInt32(ip);
    ip += 4;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, long long* op, size_t n)
{
  do
  {
    *op++ = static_cast<long long>(Decoder<E>::GetInt64(ip));
    ip += 8;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, unsigned long long* op, size_t n)
{
  do
  {
    *op++ = Decoder<E>::GetInt64(ip);
    ip += 8;
  } while (--n);
}

template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, float* op, size_t n)
{
  union {
    float f;
    unsigned int i;
...
template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, double* op, size_t n)
{
  union {
    double d;
...
template <int E>
void Decoder<E>::GetValues(const unsigned char* ip, vtkDICOMTag* op, size_t n)
{
  do
  {
...
template <int E>
template <class T>
size_t Decoder<E>::ReadData(const unsigned char*& cp, const unsigned char*& ep, T* ptr, size_t n)
{
  size_t l = n * sizeof(T);
...
template <int E>
size_t Decoder<E>::SkipData(const unsigned char*& cp, const unsigned char*& ep, size_t l)
{
  ...
  if (l >= static_cast<size_t>(this->Parser->GetBufferSize()))
  {
    ...
    vtkTypeInt64 n = vtkDICOMParserInternalFriendship::GetBytesRemaining(this->Parser, cp, ep);
    m = (m <= n ? m : n);
...
//----------------------------------------------------------------------------
template <int E>
size_t Decoder<E>::ReadElementHead(const unsigned char*& cp, const unsigned char*& ep,
  vtkDICOMTag tag, vtkDICOMVR& vr, unsigned int& vl)
{
  ...
  else
  {
    ...
    if (!vr.IsValid())
    {
      ...
      if (cp[-4] < 0x20 || cp[-4] >= 0x7f || cp[-3] < 0x20 || cp[-3] >= 0x7f)
      {
    ...
    if (!implicit && vr.HasLongVL())
    {
      ...
      if (!this->CheckBuffer(cp, ep, 4))
      {
        vtkDICOMParserInternalFriendship::ParseError(
          this->Parser, cp, ep, "Unexpected end of file.");
        return 0;
      }
      vl = Decoder<E>::GetInt32(cp);
      cp += 4;
...
//----------------------------------------------------------------------------
template <int E>
size_t Decoder<E>::ReadElementValue(const unsigned char*& cp, const unsigned char*& ep,
  vtkDICOMVR vr, unsigned int vl, vtkDICOMValue& v)
{
  ...
  if (vl == HxFFFFFFFF)
  {
    if (vr == vtkDICOMVR::UN)
    {
      ...
      this->ImplicitLE->SkipElements(cp, ep, vl, vtkDICOMTag(HxFFFE, HxE0DD), &v);
    ...
    else if (vr == vtkDICOMVR::OB)
    {
      ...
      {
        vtkDICOMParserInternalFriendship::ParseError(
          this->Parser, cp, ep, "Encapsulated object is missing (FFFE,E000) tag.");
        return 0;
      }

      v.AllocateUnsignedCharData(vr, 0);
      this->SkipElements(cp, ep, vl, vtkDICOMTag(HxFFFE, HxE0DD), &v);
      return v.GetNumberOfValues();
    }
    else if (vr != vtkDICOMVR::SQ)
    {
      ...
      vtkDICOMParserInternalFriendship::ParseError(
        this->Parser, cp, ep, "Illegal item length FFFFFFFF encountered.");
      return 0;
    }
  }
  else if (static_cast<size_t>(ep - cp) < static_cast<size_t>(vl))
  {
    ...
    vtkTypeInt64 bytesRemaining =
      vtkDICOMParserInternalFriendship::GetBytesRemaining(this->Parser, cp, ep);
    if (static_cast<vtkTypeInt64>(vl) > bytesRemaining)
    {
      vtkDICOMParserInternalFriendship::ParseError(
        this->Parser, cp, ep, "Item length exceeds the bytes remaining in file.");
      return 0;
    }
  }
  ...
  switch (vr.GetType())
  {
    ...
    {
      char* ptr;
      if (vr.HasSpecificCharacterSet() && this->LastTag > DC::SpecificCharacterSet)
      {
    ...
    case VTK_DICOM_ITEM:
    {
      ...
      while (l < static_cast<size_t>(vl) || vl == HxFFFFFFFF)
      {
        ...
        if (g == HxFFFE && e == HxE000)
        {
          ...
          vtkDICOMItem item(this->Context->GetCharacterSet(), this->Context->GetVRForXS(),
            delimited, static_cast<unsigned int>(offset));
          DecoderContext context(
            &item, this->Parser->GetDefaultCharacterSet(), this->Parser->GetOverrideCharacterSet());
...
//----------------------------------------------------------------------------
template <int E>
bool Decoder<E>::ReadElements(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
  vtkDICOMTag delimiter, size_t& bytesRead)
{
  size_t tl = 0;
...
//----------------------------------------------------------------------------
template <int E>
bool Decoder<E>::QueryOneItem(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
  vtkDICOMTag delimiter, size_t& bytesRead)
{
...
//----------------------------------------------------------------------------
template <int E>
bool Decoder<E>::SkipElements(const unsigned char*& cp, const unsigned char*& ep, unsigned int l,
  vtkDICOMTag delimiter, vtkDICOMValue* v)
{
  if (l == HxFFFFFFFF)
  {
    ...
    for (;;)
    {
      if (!this->CheckBuffer(cp, ep, 8, v, sp))
      {
        return false;
      }
      unsigned short g = Decoder<E>::GetInt16(cp);
      ...
      if (this->ReadElementHead(cp, ep, tag, vr, vl) == 0)
      {
        return false;
      }
      ...
      // break if delimiter found
      if (!readGroup && this->LastTag == delimiter)
      {
        break;
      }
      ...
      if (vl == HxFFFFFFFF)
      {
        ...
        else if (!this->ImplicitLE->SkipElements(cp, ep, vl, newdelim, v))
        {
          return false;
        }
        sp = cp;
...
void vtkDICOMParser::Update()
{
  ...
  if (this->MetaData && this->MetaData->GetNumberOfDataElements() > 0 &&
...
bool vtkDICOMParser::ReadFile(vtkDICOMMetaData* data, int idx)
{
  ...
  if (ep - cp >= 132 && cp[128] == 'D' && cp[129] == 'I' && cp[130] == 'C' && cp[131] == 'M')
  {
    // DICM magic number at offset 128, where it belongs in DICOM
    cp += 132;
  }
  else if (ep - cp >= 4 && cp[0] == 'D' && cp[1] == 'I' && cp[2] == 'C' && cp[3] == 'M')
  {
...
bool vtkDICOMParser::ReadMetaHeader(
  const unsigned char*& cp, const unsigned char*& ep, vtkDICOMMetaData* meta, int idx)
{
  ...
  const unsigned char*& cp, const unsigned char*& ep, vtkDICOMMetaData* meta, int idx)
{
  ...
  {
    if (!decoder->CheckBuffer(cp, ep, 8))
    {
      return false;
    }
  ...
  if (hasQuery)
  {
    ...
    else
    {
      ...
      while (metaIter != metaEnd && iter != iterEnd && iter->GetTag().GetGroup() <= 0x0002)
      {
  ...
  while (!readFailure)
  {
    ...
    if (!readFailure && lastTag == delimiter && lastTag.GetElement() != 0x0000)
    {
      ...
      if ((vl != 0xFFFFFFFF && r < static_cast<vtkTypeInt64>(vl)) || !this->SkipValue(cp, ep, vl))
      {
        this->ParseError(cp, ep, "Premature end of file while reading PixelData.");
        readFailure = true;
      }
    }
  }
...
//----------------------------------------------------------------------------
bool vtkDICOMParser::FillBuffer(const unsigned char*& ucp, const unsigned char*& ep)
{
  ...
  const unsigned char*& ucp, const unsigned char*& ep, vtkTypeInt64 offset)
{
  ...
  if (!this->InputFile->GetError() && this->InputFile->SetPosition(pos + offset))
  {
...
//----------------------------------------------------------------------------
bool vtkDICOMParser::SkipValue(const unsigned char*& cp, const unsigned char*& ep, unsigned int vl)
{
  if (vl != 0xFFFFFFFF)
  {
...
//----------------------------------------------------------------------------
vtkTypeInt64 vtkDICOMParser::GetBytesRemaining(const unsigned char* cp, const unsigned char* ep)
{
  return static_cast<vtkTypeInt64>(this->FileSize - this->BytesRead + (ep - cp));
}

//----------------------------------------------------------------------------
vtkTypeInt64 vtkDICOMParser::GetBytesProcessed(const unsigned char* cp, const unsigned char* ep)
{
  ...
  const unsigned char* cp, const unsigned char* ep, const char* message)
{
  ...
  vtkErrorMacro(
    "At byte offset " << this->FileOffset << " in file " << this->FileName << ": " << message);
}
