...
bool vtkDICOMMRGenerator::GenerateMRSeriesModule(vtkDICOMMetaData* source)
{
  ...
  static const DC::EnumType optional[] = { DC::ReferencedPerformedProcedureStepSequence, // 1C
...
bool vtkDICOMMRGenerator::GenerateMRImageModule(vtkDICOMMetaData* source)
{
  ...
  if (source)
  {
    ...
    if (this->SourceInstanceArray != 0 && source == this->SourceMetaData)
    {
      ...
      const char* ped = sourceAdapter->Get(DC::InPlanePhaseEncodingDirection).GetCharData();
      if (ped != 0 && ped[0] != '\0')
      {
  ...
  if (this->Dimensions[3] > 1)
  {
    ...
    meta->Set(DC::NumberOfTemporalPositions, this->Dimensions[3]);
    meta->Set(DC::TemporalResolution, this->Spacing[3]);
  }

  // required items: use simple read/write validation
  DC::EnumType required[] = { DC::ScanOptions, DC::MRAcquisitionType, DC::EchoTime,
    DC::EchoTrainLength, DC::ItemDelimitationItem };

  // optional and conditional: direct copy of values with no checks
  static const DC::EnumType optional[] = { DC::RepetitionTime, // 2C, not req'd if EP and not SK
    DC::InversionTime, // 2C, req'd if ScanningSequence is IR
    DC::TriggerTime,   // 2C, req'd for cardiac gating
    DC::SequenceName, DC::AngioFlag, DC::NumberOfAverages, DC::ImagingFrequency, DC::ImagedNucleus,
    DC::EchoNumbers, // can be per-instance
    DC::MagneticFieldStrength,
    // DC::SpacingBetweenSlices, // see above
    DC::NumberOfPhaseEncodingSteps, DC::PercentSampling, DC::PercentPhaseFieldOfView,
    DC::PixelBandwidth, DC::NominalInterval, DC::BeatRejectionFlag, DC::LowRRValue, DC::HighRRValue,
    DC::IntervalsAcquired, DC::IntervalsRejected, DC::PVCRejection, DC::SkipBeats, DC::HeartRate,
    // DC::CardiacNumberOfImages, // see above
    DC::TriggerWindow, DC::ReconstructionDiameter, DC::ReceiveCoilName, DC::TransmitCoilName,
    DC::AcquisitionMatrix,
    // DC::InPlanePhaseEncodingDirection, // see above
    DC::FlipAngle, DC::SAR, DC::VariableFlipAngleFlag, DC::dBdt,
    // DC::TemporalPositionIdentifier, // per-instance
    // DC::NumberOfTemporalPositions,
    // DC::TemporalResolution,
    DC::AnatomicRegionSequence, DC::PrimaryAnatomicStructureSequence, DC::ItemDelimitationItem };
...
bool vtkDICOMMRGenerator::GenerateMRInstance(vtkInformation* info)
{
  this->SetPixelRestrictions(
    RepresentationSigned | RepresentationUnsigned, BitsStored12 | BitsStored16, 1);
  ...
  if (!this->GenerateSOPCommonModule(source, SOPClass) || !this->GeneratePatientModule(source) ||
    ...
    !this->GenerateGeneralEquipmentModule(source) || !this->GenerateGeneralImageModule(source) ||
    !this->GenerateImagePlaneModule(source) || !this->GenerateImagePixelModule(source) ||
    !this->GenerateContrastBolusModule(source) || !this->GenerateDeviceModule(source) ||
    !this->GenerateSpecimenModule(source) || !this->GenerateMRSeriesModule(source) ||
    !this->GenerateMRImageModule(source) || !this->GenerateOverlayPlaneModule(source) ||
