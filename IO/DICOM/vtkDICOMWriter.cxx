...
void vtkDICOMWriter::ComputeInternalFileName(int slice)
{
  ...
  if (this->InternalFileName == 0 || strlen(this->InternalFileName) + 10 < n)
  {
  ...
  {
    sprintf(this->InternalFileName, this->FilePattern, this->FilePrefix, slice);
  }
  else
  {
...
int vtkDICOMWriter::GenerateMetaData(vtkInformation* info)
{
  ...
  if (inMeta == 0)
  {
    inMeta = vtkDICOMMetaData::SafeDownCast(metaInfo->Get(vtkDICOMAlgorithm::META_DATA()));
  }
  if (inMatrix == 0)
  {
  ...
  this->Generator->SetNumberOfOverlays(this->GetNumberOfInputConnections(1));
  ...
  {
    const char* allowedTypes[] = { "ORIGINAL\\PRIMARY", "DERIVED\\PRIMARY", "ORIGINAL\\SECONDARY",
      "DERIVED\\SECONDARY", 0 };
  ...
  meta->Set(DC::PixelData, vtkDICOMValue(vtkDICOMVR::OW, &empty, empty));

  return 1;
}

//----------------------------------------------------------------------------
void vtkDICOMWriter::GenerateOverlays(int minFileIdx, int maxFileIdx, const int wholeExtent[4])
{
  ...
  for (int fileIdx = minFileIdx; fileIdx <= maxFileIdx; fileIdx++)
  {
    ...
    nbits *= overlayCols;
    vtkDICOMValue v;
    unsigned char* buffer = v.AllocateUnsignedCharData(vtkDICOMVR::OB, (nbits + 7) / 8);
    ...
    for (int frameIdx = 0; frameIdx < numFrames; frameIdx++)
    {
      ...
      if (sliceIdx < extent[4] || sliceIdx > extent[5] || componentIdx >= nComp)
      {
        continue;
      }

      // Get the pointer to the data
      int sliceExtent[6] = { extent[0], extent[1], extent[2], extent[3], sliceIdx, sliceIdx };
      unsigned char* ptr =
        static_cast<unsigned char*>(data->GetScalarPointerForExtent(sliceExtent));
    ...
    short overlayOrigin[2] = { static_cast<short>(firstRow + 1), static_cast<short>(firstCol + 1) };
    meta->Set(fileIdx, DC::OverlayOrigin, vtkDICOMValue(vtkDICOMVR::SS, overlayOrigin, 2));
...
//----------------------------------------------------------------------------
int vtkDICOMWriter::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* vtkNotUsed(outputVector))
{
  ...
  vtkSmartPointer<vtkDICOMCompiler> compiler = vtkSmartPointer<vtkDICOMCompiler>::New();
  if (this->TransferSyntaxUID)
  {
  ...
  // write the image
  unsigned char* dataPtr = static_cast<unsigned char*>(data->GetScalarPointerForExtent(extent));
  bool flipImage = (this->MemoryRowOrder == vtkDICOMWriter::BottomUp);
  ...
  for (int fileIdx = minFileIdx; fileIdx <= maxFileIdx; fileIdx++)
  {
    ...
    compiler->SetSOPInstanceUID(meta->Get(fileIdx, DC::SOPInstanceUID).GetCharData());
    compiler->SetSeriesInstanceUID(meta->Get(fileIdx, DC::SeriesInstanceUID).GetCharData());
...
void vtkDICOMWriter::Write()
{
  ...
  if (this->Streaming && this->FileDimensionality == 2)
  {
    ...
    {
      int extent[6] = { wholeExtent[0], wholeExtent[1], wholeExtent[2], wholeExtent[3],
      ...
      for (int i = wholeExtent[4]; i <= wholeExtent[5]; i++)
      {
        ...
        for (int j = 0; j < numOverlays; j++)
        {
          ...
          int ovExtent[6];
          ovInfo->Get(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), ovExtent);
          if (i >= ovExtent[4] && i <= ovExtent[5])
          {
            ovExtent[4] = i;
            ovExtent[5] = i;
          }
          ovInfo->Set(vtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), ovExtent, 6);
        }
