...
void vtkDICOMApplyRescaleExecute(vtkDICOMApplyRescale* self, vtkImageData* inData, T* inPtr0,
  vtkImageData* outData, F* outPtr0, vtkDICOMRealWorldMapping* mapArray, int extent[6], int id)
{
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  vtkDICOMMetaData* meta =
    vtkDICOMMetaData::SafeDownCast(metaInfo->Get(vtkDICOMAlgorithm::META_DATA()));

  if (meta)
  {
    size_t sz =
      (static_cast<size_t>(numComponents) * static_cast<size_t>(extent[5] - extent[4] + 1));
    ...
    for (int c = 0; c < numComponents; c++)
    {
      for (int zIdx = extent[4]; zIdx <= extent[5]; zIdx++)
      {
        vtkDICOMRealWorldMapping* mapping = &this->Mapping[(zIdx - extent[4]) * numComponents + c];
        ...
        const vtkDICOMValue& rwvms = meta->Get(i, j, DC::RealWorldValueMappingSequence);
        const vtkDICOMItem* rwvmi = rwvms.GetSequenceData();

        if (rwvmi)
        {
          // use the real world value mapping first item
          mapping->First = rwvmi->Get(DC::RealWorldValueFirstValueMapped).AsInt();
          mapping->Last = rwvmi->Get(DC::RealWorldValueLastValueMapped).AsInt();
          mapping->Slope = rwvmi->Get(DC::RealWorldValueSlope).AsDouble();
          mapping->Intercept = rwvmi->Get(DC::RealWorldValueIntercept).AsDouble();

          // use lookup table if present
          const vtkDICOMValue& v = rwvmi->Get(DC::RealWorldValueLUTData);
          if (v.GetNumberOfValues() >= static_cast<size_t>(mapping->Last - mapping->First + 1))
          {
  ...
  else
  {
    ...
    rval = this->Superclass::RequestData(request, inputVector, outputVector);
  }
...
//----------------------------------------------------------------------------
void vtkDICOMApplyRescale::ThreadedRequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector, vtkImageData***,
  vtkImageData**, int extent[6], int id)
{
  vtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  vtkImageData* inData = vtkImageData::SafeDownCast(inInfo->Get(vtkDataObject::DATA_OBJECT()));
  ...
  if (outScalarType == VTK_FLOAT)
  {
    switch (scalarType)
    {
      vtkTemplateAliasMacro(
        vtkDICOMApplyRescaleExecute(this, inData, static_cast<VTK_TT*>(inVoidPtr), outData,
