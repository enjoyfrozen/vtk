...
protected:
  ...
  vtkDICOMErrorSilencer(const vtkDICOMErrorSilencer& c)
    : vtkCommand(c)
  {
  }
...
struct vtkDICOMReaderFileInfo
{
  ...
  vtkDICOMReaderFileInfo(int i, int n)
    : FileIndex(i)
    , FramesInFile(n)
  {
  }
...
//----------------------------------------------------------------------------
bool vtkDICOMReader::ValidateStructure(vtkIntArray* fileArray, vtkIntArray* frameArray)
{
  vtkDICOMMetaData* meta = this->MetaData;
  int numFiles = meta->GetNumberOfInstances();
  ...
  for (vtkIdType i = 0; i < numSlices; i++)
  {
    for (int j = 0; j < numComponents; j++)
    {
      ...
      int numFrames = meta->Get(fileIndex, DC::NumberOfFrames).AsInt();
  ...
  static const DC::EnumType imagePixelAttribs[] = {
    ...
    DC::Rows, DC::Columns, DC::BitsAllocated, DC::ItemDelimitationItem
  };

  for (const DC::EnumType* tags = imagePixelAttribs; *tags != DC::ItemDelimitationItem; tags++)
  {
    int firstValue = 0;

    for (int fileIndex = 0; fileIndex < numFiles; fileIndex++)
    {
      if (usedFiles[fileIndex] == 0)
      {
        continue;
      }
      ...
      if (i <= 0 ||
        (*tags == DC::BitsAllocated && i != 1 && i != 8 && i != 12 && i != 16 && i != 32 &&
          i != 64))
      {
      ...
      if (errorText)
      {
        ...
        if (v.IsValid())
        {
          vtkErrorMacro(<< errorText << i << " for " << de.GetTag() << " \"" << de.GetName()
                        << "\" in " << this->InternalFileName);
        }
        else
        {
          vtkErrorMacro(<< errorText << "for " << de.GetTag() << " \"" << de.GetName() << "\" in"
  ...
  int scalarType, int bitsStored, int pixelRepresentation)
{
  ...
  for (vtkIdType iSlice = 0; iSlice < numSlices; iSlice++)
  {
    ...
    for (int iComp = 0; iComp < numComp; iComp++)
    {
      ...
      double m = this->MetaData->Get(iFile, iFrame, DC::RescaleSlope).AsDouble();
      double b = this->MetaData->Get(iFile, iFrame, DC::RescaleIntercept).AsDouble();
  ...
  if (outputFloat && scalarType != VTK_FLOAT && scalarType != VTK_DOUBLE)
  {
...
//----------------------------------------------------------------------------
int vtkDICOMReader::RequestInformation(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  if (numFiles <= 0)
  {
    ...
    {
      vtkErrorMacro("Bad DataExtent " << this->DataExtent[4] << "," << this->DataExtent[5] << ".");
    }
  ...
  this->Parser->AddObserver(vtkCommand::ErrorEvent, this, &vtkDICOMReader::RelayError);
  ...
  double ratio = 1.0;
  vtkDICOMValue pixelAspectRatio = this->MetaData->Get(fileIndex, frameIndex, DC::PixelAspectRatio);
  ...
  vtkDICOMValue pixelSpacing = this->MetaData->Get(fileIndex, frameIndex, DC::PixelSpacing);
  ...
  int bitsAllocated = this->MetaData->Get(fileIndex, DC::BitsAllocated).AsInt();
  int pixelRepresentation = this->MetaData->Get(fileIndex, DC::PixelRepresentation).AsInt();
  int numComponents = this->MetaData->Get(fileIndex, DC::SamplesPerPixel).AsInt();
  int planarConfiguration = this->MetaData->Get(fileIndex, DC::PlanarConfiguration).AsInt();
  ...
  const vtkDICOMValue& slopeValue = this->MetaData->Get(fileIndex, frameIndex, DC::RescaleSlope);
  ...
  if (slopeValue.IsValid() && interceptValue.IsValid())
  {
    if (this->AutoRescale)
    {
      ...
      scalarType = this->ComputeRescaledScalarType(scalarType, bitsStored, pixelRepresentation);
    }
    else
    {
  ...
  if (this->OutputScalarType != -1)
  {
    switch (this->OutputScalarType)
    {
      ...
      case VTK_DOUBLE:
        ...
        break;
      default:
        vtkWarningMacro("Illegal OutputScalarType: " << this->OutputScalarType);
        break;
    }
  }
  ...
  this->NumberOfPackedComponents = (planarConfiguration ? 1 : numComponents);
  this->NumberOfPlanarComponents = (planarConfiguration ? numComponents : 1);
  this->NumberOfScalarComponents = (numComponents * this->FileIndexArray->GetNumberOfComponents());
  ...
  // endianness
  std::string transferSyntax = this->MetaData->Get(fileIndex, DC::TransferSyntaxUID).AsString();

  bool bigEndian =
    (transferSyntax == "1.2.840.10008.1.2.2" || transferSyntax == "1.2.840.113619.5.2");
  ...
  for (vtkIdType iSlice = numSlices - 1; iSlice >= 0; --iSlice)
  {
    ...
    vtkDICOMValue pv = this->MetaData->Get(iFile, iFrame, DC::ImagePositionPatient);
    vtkDICOMValue ov = this->MetaData->Get(fileIndex, frameIndex, DC::ImageOrientationPatient);
  ...
  if (nPoints > 0)
  {
    ...
    for (size_t iPoint = 0; iPoint < nPoints; iPoint++)
    {
      for (int ii = 0; ii < 3; ii++)
      {
        ...
        {
          A[ii][jj] =
            ((points[3 * iPoint + ii] - centroid[ii]) * (points[3 * iPoint + jj] - centroid[jj]));
        }
      }
    }
  ...
  outInfo->Set(vtkDICOMAlgorithm::PATIENT_MATRIX(), *this->PatientMatrix->Element, 16);
  ...
  int overlayType = (this->OverlayBitfield <= 255 ? VTK_UNSIGNED_CHAR : VTK_UNSIGNED_SHORT);
  int overlayComponents = this->FileIndexArray->GetNumberOfComponents();

  vtkDataObject::SetPointDataActiveScalarInfo(oInfo, overlayType, overlayComponents);

  oInfo->Set(vtkDICOMAlgorithm::PATIENT_MATRIX(), *this->PatientMatrix->Element, 16);
...
//----------------------------------------------------------------------------
void vtkDICOMReader::RescaleBuffer(int fileIdx, int frameIdx, int fileType, int outputType,
  int fileNumComponents, int numComponents, void* fileBuffer, void* outputBuffer,
  vtkIdType bufferSize)
{
  ...
  for (int c = 0; c < fileNumComponents; c++)
  {
    ...
    while (n > 0)
    {
      ...
      switch (fileType)
      {
        vtkTemplateAliasMacro(vtkDICOMConvertBuffer(
          static_cast<const VTK_TT*>(filePtr), temp, fileNumComponents, 1, nn));
      }
      ...
      switch (outputType)
      {
        vtkTemplateAliasMacro(
          vtkDICOMConvertBuffer(temp, static_cast<VTK_TT*>(outputPtr), 1, numComponents, nn));
      }
...
//----------------------------------------------------------------------------
void vtkDICOMReader::YBRToRGB(int fileIdx, int, void* buffer, vtkIdType bufferSize)
{
  ...
  double matrix[3][3] = { { 1.0, 0.0, 2.0 * (1.0 - Kr) },
    { 1.0, -2.0 * Kb * (1.0 - Kb) / Kg, -2.0 * Kr * (1.0 - Kr) / Kg },
    { 1.0, 2.0 * (1.0 - Kb), 0.0 } };

  // get information from the meta data
  vtkDICOMMetaData* meta = this->MetaData;
  const vtkDICOMValue& photometric = meta->Get(fileIdx, DC::PhotometricInterpretation);
  const vtkDICOMValue& transferSyntax = meta->Get(fileIdx, DC::TransferSyntaxUID);

  // catch JPEG baseline images with incorrect PhotometricInterpretation
  if (transferSyntax.Matches("1.2.840.10008.1.2.4.50") || photometric.Matches("YBR_FULL*"))
  {
...
{
  const unsigned char* readPtr = static_cast<const unsigned char*>(filePtr);
  unsigned char* writePtr = static_cast<unsigned char*>(buffer);
...
void vtkDICOMReader::MaskBits(
  void* buffer, vtkIdType bufferSize, int scalarSize, int bitsStored, int pixelRepresentation)
{
  size_t n = bufferSize / scalarSize;

  if (scalarSize == 1)
  {
    vtkDICOMMaskBits(static_cast<unsigned char*>(buffer), n, bitsStored, pixelRepresentation);
  }
  else if (scalarSize == 2)
  {
    vtkDICOMMaskBits(static_cast<unsigned short*>(buffer), n, bitsStored, pixelRepresentation);
  }
  else if (scalarSize == 4)
  {
    vtkDICOMMaskBits(static_cast<unsigned int*>(buffer), n, bitsStored, pixelRepresentation);
  }
}

//----------------------------------------------------------------------------
void vtkDICOMReader::UnpackBits(const void* filePtr, void* buffer, vtkIdType bufferSize, int bits)
{
  if (bits == 12)
  {
    const unsigned char* readPtr = static_cast<const unsigned char*>(filePtr);
...
void vtkDICOMReader::UnpackOverlay(
  const void* filePtr, vtkIdType bitskip, vtkIdType count, void* buffer, vtkIdType incr, int bit)
{
  const unsigned char* readPtr = static_cast<const unsigned char*>(filePtr);
  unsigned char* writePtr = static_cast<unsigned char*>(buffer);
...
bool vtkDICOMReader::ReadFileNative(
  const char* filename, int fileIdx, unsigned char* buffer, vtkIdType bufferSize)
{
  ...
  std::string transferSyntax = this->MetaData->Get(fileIdx, DC::TransferSyntaxUID).AsString();
  ...
  int bitsAllocated = this->MetaData->Get(fileIdx, DC::BitsAllocated).AsInt();
  ...
  if (transferSyntax == "1.2.840.10008.1.2.5")
  {
    ...
    unsigned int numFrames = this->MetaData->Get(fileIdx, DC::NumberOfFrames).AsUnsignedInt();
    numFrames = (numFrames == 0 ? 1 : numFrames);

    // assume the remainder of the file is all pixel data
    readSize = static_cast<size_t>(offsetAndSize[1] - offsetAndSize[0]);
    if (readSize < 8)
    {
      readSize = 8;
    }
    ...
    while (bytesRemaining >= 8 && bufferPos < bufferSize)
    {
      ...
      if (!isOffsetTable)
      {
        ...
        codec.Decode(this->MetaData, filePtr, length, buffer + bufferPos, frameSize);
        bufferPos += frameSize;
      }
      filePtr += length;
  ...
  else if (this->MetaData->GetAttributeValue(fileIdx, DC::PhotometricInterpretation)
             .Matches("YBR_*_422"))
  {
  ...
  {
    this->SetErrorCode(vtkErrorCode::PrematureEndOfFileError);
    vtkErrorMacro("DICOM file is truncated, " << (readSize - resultSize) << " bytes are missing.");
    success = false;
  }
  ...
  const char* filename, int fileIdx, unsigned char* buffer, vtkIdType bufferSize)
{
  ...
  OFCondition status =
    fileformat->getDataset()->chooseRepresentation(EXS_LittleEndianExplicit, NULL);
  ...
  status = fileformat->getDataset()->findAndGetUint8Array(DCM_PixelData, pixelData, &count, OFTrue);
  ...
  {
    vtkErrorMacro(<< filename << ": The uncompressed image size is " << imageSize
                  << " bytes, expected " << bufferSize << " bytes.");
    delete fileformat;
    return false;
  }
  ...
  {
    vtkErrorMacro(<< filename << ": The uncompressed image size is " << image.GetBufferLength()
                  << " bytes, expected " << bufferSize << " bytes.");
    this->SetErrorCode(vtkErrorCode::FileFormatError);
    return false;
  }
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  if (request->Has(vtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED()))
  {
    ...
    for (int i = 0; i < n; i++)
    {
      if (i != outputPort && !(this->UpdateOverlayFlag && this->OverlayBitfield && outputPort == 1))
      {
...
//----------------------------------------------------------------------------
int vtkDICOMReader::RequestData(vtkInformation* request,
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  dataInfo->Set(vtkDICOMAlgorithm::PATIENT_MATRIX(), *this->PatientMatrix->Element, 16);
  ...
  for (size_t idx = 0; idx < files.size(); idx++)
  {
    ...
    this->UpdateProgress(static_cast<double>(idx) / static_cast<double>(files.size()));
    ...
    bool needBuffer = (planarToPacked || numFrames != framesInFile || scalarSize != fileScalarSize);
    ...
    else
    {
      ...
      bufferPtr =
        (dataPtr + (sliceIdx - extent[4]) * sliceSize + componentIdx * filePixelSize * numPlanes);
    }
    ...
    this->NeedsYBRToRGB = (this->AutoYBRToRGB && numComponents == 3 && scalarSize == 1);

    // this is the method that actually reads the file
    this->ReadOneFile(this->InternalFileName, fileIdx, bufferPtr, framesInFile * fileFrameSize);
    ...
    {
      int pixelRepresentation = this->MetaData->Get(fileIdx, DC::PixelRepresentation).AsInt();
      vtkDICOMReader::MaskBits(
        bufferPtr, framesInFile * fileFrameSize, fileScalarSize, bitsStored, pixelRepresentation);
    }
    ...
    for (int sIdx = 0; sIdx < numFrames; sIdx++)
    {
      ...
      unsigned char* slicePtr = (dataPtr + (sliceIdx - extent[4]) * sliceSize +
      ...
      for (int pIdx = 0; pIdx < numPlanes; pIdx++)
      {
        ...
        {
          this->RescaleBuffer(fileIdx, frameIdx, this->FileScalarType, scalarType,
            numFileComponents, numComponents, planePtr, slicePtr, filePlaneSize);
        }
  ...
  for (int sIdx = extent[4]; sIdx <= extent[5]; sIdx++)
  {
    for (int cIdx = 0; cIdx < nComp; cIdx++)
    {
      ...
      for (int i = 0; i <= maxOverlay; i++)
      {
        ...
        const vtkDICOMValue& overlayData = this->MetaData->Get(fileIdx, vtkDICOMTag(g, 0x3000));
        unsigned int vl = overlayData.GetVL();
        const unsigned char* bptr = overlayData.GetUnsignedCharData();
        if (bptr == 0)
        {
          bptr = reinterpret_cast<const unsigned char*>(overlayData.GetUnsignedShortData());
        }
        if (bptr == 0)
        {
          continue;
        ...
        int startX = 0;
        int startY = 0;
        const vtkDICOMValue& ov = this->MetaData->Get(fileIdx, vtkDICOMTag(g, 0x0050));
        ...
        int numFrames = this->MetaData->Get(fileIdx, vtkDICOMTag(g, 0x0015)).AsInt();
        int frameOrigin = this->MetaData->Get(fileIdx, vtkDICOMTag(g, 0x0051)).AsInt();
        frameOrigin = (frameOrigin > 0 ? frameOrigin - 1 : 0);

        // make sure this frame exists in the overlay
        if (numFrames && (frameIdx < frameOrigin || frameIdx > frameOrigin + numFrames - 1))
        {
          continue;
        }
        ...
        {
          this->SetErrorCode(vtkErrorCode::FileFormatError);
          vtkErrorMacro("OverlayData length " << vl << " too small for size " << sizeX << "x"
                                              << sizeY << " (frames=" << numFrames << ")");
          success = false;
          continue;
        }

        // compute the number of frames to skip when reading the file
        vtkIdType inSkip = static_cast<vtkIdType>(frameIdx - frameOrigin) * sizeX * sizeY;

        // compute the initial offset into the output
        vtkIdType outSkip = static_cast<vtkIdType>(sIdx - extent[4]) * (extent[3] - extent[2] + 1) *
          (extent[1] - extent[0] + 1) * nComp * scalarSize;
        ...
        if (extent[0] < startX)
        {
          outSkip += static_cast<vtkIdType>(startX - extent[0]) * scalarSize * nComp;
        }
        else
        {
        ...
        for (int j = 0; j < countY; j++)
        {
          vtkDICOMReader::UnpackOverlay(
            bptr, inSkip, countX, ptr + outSkip, nComp * scalarSize, (i & 0x7));
          inSkip += sizeX;
          outSkip += outRowInc;
        }
      }
...
void vtkDICOMReader::UpdateMedicalImageProperties()
{
  ...
  properties->SetPatientName(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::PatientID);
  properties->SetPatientID(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  properties->SetPatientAge(meta->Get(DC::PatientAge).GetCharData());
  properties->SetPatientSex(meta->Get(DC::PatientSex).GetCharData());
  properties->SetPatientBirthDate(meta->Get(DC::PatientBirthDate).GetCharData());
  ...
  properties->SetSeriesDescription(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::StudyID);
  properties->SetStudyID(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::StudyDescription);
  properties->SetStudyDescription(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  properties->SetModality(meta->Get(DC::Modality).GetCharData());
  vptr = &meta->Get(DC::Manufacturer);
  properties->SetManufacturer(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::ManufacturerModelName);
  properties->SetManufacturerModelName(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::StationName);
  properties->SetStationName(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::InstitutionName);
  properties->SetInstitutionName(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  vptr = &meta->Get(DC::ConvolutionKernel);
  properties->SetConvolutionKernel(vptr->IsValid() ? vptr->AsUTF8String().c_str() : NULL);
  ...
  for (int j = 0; j < m; j++)
  {
    properties->SetNthWindowLevelPresetComment(j, comment.GetString(j).c_str());
  }
