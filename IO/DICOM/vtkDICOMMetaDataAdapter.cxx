...
//----------------------------------------------------------------------------
vtkDICOMMetaDataAdapter::vtkDICOMMetaDataAdapter(vtkDICOMMetaData* meta, int i)
{
  this->ConstructionHelper(meta, i);
}

//----------------------------------------------------------------------------
void vtkDICOMMetaDataAdapter::ConstructionHelper(vtkDICOMMetaData* meta, int i)
{
  this->Meta = meta;
  this->PerFrame = 0;
  this->Shared = 0;
  ...
  if (this->Shared && this->Shared->IsValid() && this->PerFrame && this->PerFrame->IsValid())
  {
    this->NumberOfInstances = meta->Get(this->MetaInstance, DC::NumberOfFrames).AsInt();
...
//----------------------------------------------------------------------------
// Copy constructor
vtkDICOMMetaDataAdapter::vtkDICOMMetaDataAdapter(const vtkDICOMMetaDataAdapter& other)
  : Meta(other.Meta)
  , PerFrame(other.PerFrame)
  , Shared(other.Shared)
...
//----------------------------------------------------------------------------
// Assignment
vtkDICOMMetaDataAdapter& vtkDICOMMetaDataAdapter::operator=(const vtkDICOMMetaDataAdapter& other)
{
  if (this != &other)
  {
...
//----------------------------------------------------------------------------
const vtkDICOMValue& vtkDICOMMetaDataAdapter::Get(int idx, vtkDICOMTag tag) const
{
  ...
  int idx, vtkDICOMTag ptag, const std::string& creator)
{
  ...
  if (this->PerFrame)
  {
    ...
    // if it wasn't in a PerFrame or Shared functional group
    vtkDICOMTag tag = meta->ResolvePrivateTag(this->MetaInstance, ptag, creator);
    if (tag == vtkDICOMTag(0xFFFF, 0xFFFF))
    {
      tag = tagFromPrivSeq;
    }
...
//----------------------------------------------------------------------------
vtkDICOMTag vtkDICOMMetaDataAdapter::ResolvePrivateTag(vtkDICOMTag ptag, const std::string& creator)
{
