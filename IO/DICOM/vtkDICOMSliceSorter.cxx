...
const vtkDICOMValue& vtkDICOMSliceSorterGetFrame(const vtkDICOMSequence& frameSeq,
  const vtkDICOMSequence& sharedSeq, unsigned int i, vtkDICOMTag stag, vtkDICOMTag vtag)
{
...
double vtkDICOMSliceSorterComputeLocation(
  const vtkDICOMValue& pv, const vtkDICOMValue& ov, double checkOrientation[14], bool* checkPtr)
{
  double location = 0.0;
  ...
  else
  {
    ...
    else
    {
      ...
      if (checkOrientation[13] == 0 &&
        v[0] * v[0] + v[1] * v[1] + v[2] * v[2] > positionTolerance * positionTolerance)
      {
        for (int i = 0; i < 3; i++)
        {
      ...
      if (d >
        (positionTolerance * positionTolerance + t * t * directionTolerance * directionTolerance))
      {
...
double vtkDICOMSliceSorterComputeSpacing(
  const std::vector<vtkDICOMSliceSorterSortInfo>& info, int* missing)
{
  double a = 0;
...
void vtkDICOMSliceSorter::SortFiles(vtkIntArray* files, vtkIntArray* frames)
{
  ...
  std::stable_sort(info.begin(), info.end(), vtkDICOMSliceSorterSortInfo::CompareInstance);
  ...
  double spacingBetweenSlices = meta->Get(DC::SpacingBetweenSlices).AsDouble();
  ...
  if (meta->Has(DC::SharedFunctionalGroupsSequence))
  {
    ...
    vtkDICOMValue desiredStackId = vtkDICOMValue(vtkDICOMVR::SH, this->DesiredStackID);
    ...
    for (int ii = 0; ii < numFiles; ii++)
    {
      ...
      vtkDICOMSequence frameSeq = meta->Get(i, DC::PerFrameFunctionalGroupsSequence);
      vtkDICOMSequence sharedSeq = meta->Get(i, DC::SharedFunctionalGroupsSequence);
      ...
      if (ii == 0 && numberOfFrames > 0)
      {
        firstStackId = vtkDICOMSliceSorterGetFrame(
          frameSeq, sharedSeq, 0, DC::FrameContentSequence, DC::StackID);
      }
      ...
      if (numberOfFrames > 0)
      {
        ...
        if (this->TimeSequence.GetGroup() != 0 && this->TimeTag.GetGroup() != 0 &&
          vtkDICOMSliceSorterGetFrame(frameSeq, sharedSeq, 0, this->TimeSequence, this->TimeTag)
            .IsValid())
        {
          timeSequence = this->TimeSequence;
          timeTag = this->TimeTag;
        }
        else if (vtkDICOMSliceSorterGetFrame(frameSeq, sharedSeq, 0,
                   DC::CardiacSynchronizationSequence, DC::NominalCardiacTriggerDelayTime)
                   .IsValid())
        {
          timeSequence = DC::CardiacSynchronizationSequence;
          timeTag = DC::NominalCardiacTriggerDelayTime;
        }
        else if (vtkDICOMSliceSorterGetFrame(frameSeq, sharedSeq, 0, DC::TemporalPositionSequence,
                   DC::TemporalPositionTimeOffset)
                   .IsValid())
        {
          ...
          temporalSpacing = 1000.0; // convert seconds to milliseconds
                   ...
                   frameSeq, sharedSeq, 0, DC::FrameContentSequence, DC::TemporalPositionIndex)
                   .IsValid())
        {
          ...
          timeTag = DC::TemporalPositionIndex;
                   ...
                   frameSeq, sharedSeq, 0, DC::MREchoSequence, DC::EffectiveEchoTime)
                   .IsValid())
        {
      ...
      for (int k = 0; k < numberOfFrames; k++)
      {
        ...
        {
          t = vtkDICOMSliceSorterGetFrame(frameSeq, sharedSeq, k, timeSequence, timeTag).AsDouble();
        }
        ...
        vtkDICOMValue stackId = vtkDICOMSliceSorterGetFrame(
          frameSeq, sharedSeq, k, DC::FrameContentSequence, DC::StackID);
        ...
        if (stackId.IsValid())
        {
          ...
          {
            if (stackName == this->StackIDs->GetValue(si))
            {
              break;
            }
          }
          ...
          position = vtkDICOMSliceSorterGetFrame(
            frameSeq, sharedSeq, k, DC::FrameContentSequence, DC::InStackPositionNumber)
                       .AsInt();
        }

        // check for valid Image Plane Module information
        vtkDICOMValue pv = vtkDICOMSliceSorterGetFrame(
          frameSeq, sharedSeq, k, DC::PlanePositionSequence, DC::ImagePositionPatient);
        vtkDICOMValue ov = vtkDICOMSliceSorterGetFrame(
          frameSeq, sharedSeq, k, DC::PlaneOrientationSequence, DC::ImageOrientationPatient);
        ...
        if (stackId == desiredStackId)
        {
          ...
          double location =
            vtkDICOMSliceSorterComputeLocation(pv, ov, stackCheckNormal, &canSortStackByIPP);

          location /= spacingBetweenSlices;

          stackInfo.push_back(vtkDICOMSliceSorterSortInfo(i, k, inst, position, location, t));
        }
        else if (stackId == firstStackId)
        {
          // compute location from orientation and IPP
          double location =
            vtkDICOMSliceSorterComputeLocation(pv, ov, checkOrientation, &canSortByLocation);
          location /= spacingBetweenSlices;

          // force output of one single volume
          if ((ii > 0 || k > 0) && !firstStackId.IsValid() && pv.IsValid() && !canSortByLocation)
          {
            canSortByLocation = true;
            volumeBreaks.push_back(info.size());
          }

          info.push_back(vtkDICOMSliceSorterSortInfo(i, k, inst, position, location, t));
        }
      }
    }
  ...
  else
  {
    ...
    vtkDICOMTag timeTag;
    if (this->TimeTag.GetGroup() != 0 && meta->Get(this->TimeTag).IsValid())
    {
      timeTag = this->TimeTag;
    }
    ...
    for (int ii = 0; ii < numFiles; ii++)
    {
      ...
      location = vtkDICOMSliceSorterComputeLocation(pv, ov, checkOrientation, &canSortByLocation);
      location /= spacingBetweenSlices;
      ...
      if (numberOfFrames <= 1)
      {
        ...
        info.push_back(vtkDICOMSliceSorterSortInfo(i, 0, inst, position, location, t));
      }
      else
      {
        // multi-frame image
        ...
        if (timeSlotVector.IsValid() || sliceVector.IsValid())
        {
          ...
          for (int k = 0; k < numberOfFrames; k++)
          {
            ...
            info.push_back(vtkDICOMSliceSorterSortInfo(i, k, inst, position, frameloc, t));
          }
        }
        else
        {
          ...
          for (int k = 0; k < numberOfFrames; k++)
          {
            ...
            info.push_back(vtkDICOMSliceSorterSortInfo(i, k, inst, inst, location, t));
          }
        }
      }
    }
  ...
  else if (numSlices > 1)
  {
    ...
    {
      std::stable_sort(info.begin(), info.end(), vtkDICOMSliceSorterSortInfo::CompareLocation);
    }
    else
    {
      std::stable_sort(info.begin(), info.end(), vtkDICOMSliceSorterSortInfo::ComparePosition);
    }
    ...
    while (iter != info.end())
    {
      ...
      if (nextIter != info.end())
      {
        ...
        if (canSortByLocation)
        {
          positionIncreased = vtkDICOMSliceSorterSortInfo::CompareLocation(*iter, *nextIter);
        }
        else
        {
          positionIncreased = vtkDICOMSliceSorterSortInfo::ComparePosition(*iter, *nextIter);
        }
      }
  ...
  if (temporalPositions > 0 && this->TimeAsVector == 0 && this->DesiredTimeIndex < 0)
  {
  ...
  if (canSortByLocation)
  {
    ...
    if (locations > 1)
    {
      ...
      if (missing > 0)
      {
        vtkWarningMacro("Series has " << missing << " gaps due to missing slices.");
      }
