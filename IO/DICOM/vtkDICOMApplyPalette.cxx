...
class vtkDICOMPerFilePalette : public std::vector<vtkSmartPointer<vtkDICOMLookupTable> >
{
};
...
template <class T>
void vtkDICOMApplyPaletteExecute(vtkDICOMApplyPalette* self, vtkImageData* inData, T* inPtr0,
  vtkImageData* outData, unsigned char* outPtr0, int extent[6], vtkDICOMPerFilePalette* palette,
  bool supplemental, int id)
{
  vtkInformation* dataInfo = outData->GetInformation();
  vtkDICOMMetaData* meta =
    vtkDICOMMetaData::SafeDownCast(dataInfo->Get(vtkDICOMAlgorithm::META_DATA()));
  ...
  for (int c = 0; c < inputComponents; c++)
  {
    ...
    for (int zIdx = extent[4]; zIdx <= extent[5]; zIdx++)
    {
      ...
      if (supplemental)
      {
        ...
        monochrome = meta->Get(i, f, DC::PixelPresentation).Matches("MONOCHROME*");
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (meta && meta->Get(DC::SamplesPerPixel).Matches(1))
  {
    ...
    this->IsSupplemental = (v.Matches("COLOR") || v.Matches("MIXED") || v.Matches("TRUE_COLOR"));
  ...
  if (hasPalette)
  {
    ...
    vtkDICOMMetaData* outMeta =
      vtkDICOMMetaData::SafeDownCast(outInfo->Get(vtkDICOMAlgorithm::META_DATA()));
...
//----------------------------------------------------------------------------
void vtkDICOMApplyPalette::ThreadedRequestData(vtkInformation* vtkNotUsed(request),
  ...
  vtkImageData**, int extent[6], int id)
{
  ...
  switch (scalarType)
  {
    vtkTemplateAliasMacro(
      vtkDICOMApplyPaletteExecute(this, inData, static_cast<VTK_TT*>(inVoidPtr), outData,
        static_cast<unsigned char*>(outVoidPtr), extent, this->Palette, this->IsSupplemental, id));
  }
}
