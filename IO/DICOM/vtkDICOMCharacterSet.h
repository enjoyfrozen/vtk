 ...
 */
class VTKIODICOM_EXPORT vtkDICOMCharacterSet
{
public:
  enum EnumType
  {
  ...
  vtkDICOMCharacterSet(const char* name, size_t nl) { this->Key = KeyFromString(name, nl); }
  //@}
   ...
   */
  static void SetGlobalDefault(vtkDICOMCharacterSet cs) { GlobalDefault = cs.GetKey(); }
  static vtkDICOMCharacterSet GetGlobalDefault() { return GlobalDefault; }
   ...
   */
  static void SetGlobalOverride(bool b) { GlobalOverride = b; }
  ...
  std::string ToUTF8(const std::string& text) const { return ToUTF8(text.data(), text.length()); }
  ...
  bool IsISO8859() const { return (this->Key >= ISO_IR_100 && this->Key <= X_LATIN10); }
  ...
  {
    return (this->Key == ISO_IR_127 || this->Key == ISO_IR_138 || this->Key == X_CP1255 ||
      this->Key == X_CP1256);
  }
  //@}
...
private:
  ...
  static size_t UTF8ToJISX(int charset, const char* t, size_t l, std::string* s);
  static size_t JISXToUTF8(int csGL, int csGR, const char* t, size_t l, std::string* s, int m);
...
VTKIODICOM_EXPORT ostream& operator<<(ostream& o, const vtkDICOMCharacterSet& a);
