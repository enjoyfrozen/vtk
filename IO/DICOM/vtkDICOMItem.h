 ...
 */
class VTKIODICOM_EXPORT vtkDICOMItem
{
private:
...
public:
  ...
  explicit vtkDICOMItem(
    vtkDICOMCharacterSet cs, vtkDICOMVR vrForXS, int delimited = 0, unsigned int byteOffset = 0);
  ...
  bool IsDelimited() const { return (this->L == 0 || this->L->Delimited != 0); }
  //@}
  ...
  const vtkDICOMValue& GetAttributeValue(vtkDICOMTag tag) const { return this->Get(tag); }
   ...
   */
  unsigned int GetByteOffset() const { return (this->L == 0 ? 0 : this->L->ByteOffset); }
  //@}

  //@{
  //! Get the number of data elements.
  int GetNumberOfDataElements() const { return (this->L ? this->L->NumberOfDataElements : 0); }

  //! Get an iterator for the list of data elements.
  vtkDICOMDataElementIterator Begin() const { return (this->L ? this->L->Head.Next : 0); }

  //! Get an end iterator for the list of data elements.
  vtkDICOMDataElementIterator End() const { return (this->L ? &this->L->Tail : 0); }
  //@}
  ...
  bool operator!=(const vtkDICOMItem& o) const { return !(*this == o); }
  //@}
...
private:
  ...
  vtkDICOMItem* FindItemOrInsert(const vtkDICOMTagPath& tagpath, vtkDICOMTag* tagptr);
