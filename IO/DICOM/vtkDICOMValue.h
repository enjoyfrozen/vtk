 ...
 *  no virtual methods.
 */
class VTKIODICOM_EXPORT vtkDICOMValue
{
private:
...
public:
   ...
   *  control characters.
   */
  vtkDICOMCharacterSet GetCharacterSet() const { return (this->V ? this->V->CharacterSet : 0); }
...
private:
   ...
   */
  size_t CreateValueFromUTF8(vtkDICOMVR vr, vtkDICOMCharacterSet cs, const char* data, size_t l);

  //! A simple string compare with wildcards "*" and "?".
  static bool PatternMatches(const char* pattern, const char* pe, const char* val, const char* ve);

  //! Pattern matching with multiple backslash-delimited values.
  static bool PatternMatchesMulti(const char* pattern, const char* val, vtkDICOMVR vr);
   ...
   */
  static size_t NormalizeDateTime(const char* input, char output[22], vtkDICOMVR vr);
   ...
   *  segments.
   */
  static void NormalizePersonName(const char* input, char output[256], bool isquery = false);
...
{
  static vtkDICOMValue* GetMultiplex(vtkDICOMValue* v) { return v->GetMultiplex(); }

  friend class vtkDICOMMetaData;
};
//! @endcond

VTKIODICOM_EXPORT ostream& operator<<(ostream& os, const vtkDICOMValue& v);
