...
{
public:
  static bool FlushBuffer(vtkDICOMCompiler* comp, unsigned char*& cp, unsigned char*& ep)
  {
    return comp->FlushBuffer(cp, ep);
  }

  static void CompileError(vtkDICOMCompiler* comp, const char* message)
  {
...
public:
  ...
  void SetSOPInstanceUID(const char* uid) { this->SOPInstanceUID = uid; };

  // Set the series UID to use.
  void SetSeriesInstanceUID(const char* uid) { this->SeriesInstanceUID = uid; };

  // Set the study UID to use.
  void SetStudyInstanceUID(const char* uid) { this->StudyInstanceUID = uid; };
  ...
  virtual bool WriteElements(unsigned char*& cp, unsigned char*& ep,
    vtkDICOMDataElementIterator iter, vtkDICOMDataElementIterator iterend) = 0;

  // Copy bytes from sp to end marker cp into the value "v".
  // If the parameter "v" is NULL, then it will be ignored.
  void CopyBuffer(vtkDICOMValue* v, unsigned char* sp, unsigned char* cp);
  ...
  bool CheckBuffer(unsigned char*& cp, unsigned char*& ep, size_t n);
  ...
  unsigned int DataSize(vtkDICOMDataElementIterator iter, vtkDICOMDataElementIterator iterEnd);
...
public:
  ...
  template <class T>
  bool WriteData(unsigned char*& cp, unsigned char*& ep, const T* ptr, size_t n);

  // write the head of a data element, return length (8 or 12)
  size_t WriteElementHead(unsigned char* cp, vtkDICOMTag tag, vtkDICOMVR vr, unsigned int vl);

  // write one data element
  bool WriteDataElement(unsigned char*& cp, unsigned char*& ep, const vtkDICOMDataElement& elem);

  // Write all elements from iter to iterend.
  bool WriteElements(unsigned char*& cp, unsigned char*& ep, vtkDICOMDataElementIterator iter,
...
//----------------------------------------------------------------------------
inline bool EncoderBase::CheckBuffer(unsigned char*& cp, unsigned char*& ep, size_t n)
{
  bool r = true;
  if (n > static_cast<size_t>(ep - cp))
  {
    r = vtkDICOMCompilerInternalFriendship::FlushBuffer(this->Compiler, cp, ep);
...
unsigned int EncoderBase::DataSize(
  vtkDICOMDataElementIterator iter, vtkDICOMDataElementIterator iterEnd)
{
  unsigned int l = 0;
...
//----------------------------------------------------------------------------
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const char* ip, size_t n)
{
  do
  {
    *op++ = static_cast<unsigned char>(*ip++);
  } while (--n);
}

template <int E>
void Encoder<E>::PutValues(unsigned char* op, const unsigned char* ip, size_t n)
{
  do
  {
    *op++ = *ip++;
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const short* ip, size_t n)
{
  do
  {
    Encoder<E>::PutInt16(op, *ip);
    ip++;
    op += 2;
  } while (--n);
}
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const unsigned short* ip, size_t n)
{
  do
  {
    Encoder<E>::PutInt16(op, *ip);
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const int* ip, size_t n)
{
  do
  {
    Encoder<E>::PutInt32(op, *ip);
    ip++;
    op += 4;
  } while (--n);
}
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const unsigned int* ip, size_t n)
{
  do
  {
    Encoder<E>::PutInt32(op, *ip);
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const float* ip, size_t n)
{
  union {
    float f;
    unsigned int i;
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const double* ip, size_t n)
{
  union {
    double d;
...
template <int E>
void Encoder<E>::PutValues(unsigned char* op, const vtkDICOMTag* ip, size_t n)
{
  do
  {
...
template <int E>
template <class T>
bool Encoder<E>::WriteData(unsigned char*& cp, unsigned char*& ep, const T* ptr, size_t n)
{
...
bool Encoder<E>::WriteDataElement(
  unsigned char*& cp, unsigned char*& ep, const vtkDICOMDataElement& elem)
{
  ...
  if (vl == HxFFFFFFFF)
  {
    if (vr == vtkDICOMVR::UN || vr == vtkDICOMVR::OB)
    {
      ...
      assert(n > 8);
      assert(ptr[n - 8] + (ptr[n - 7] << 8) == HxFFFE && ptr[n - 6] + (ptr[n - 5] << 8) == HxE0DD);
      assert(ptr[n - 4] + (ptr[n - 3] << 8) + (ptr[n - 2] << 16) + (ptr[n - 1] << 24) == 0);
#endif
    }
...
//----------------------------------------------------------------------------
template <int E>
bool Encoder<E>::WriteElements(unsigned char*& cp, unsigned char*& ep,
  vtkDICOMDataElementIterator iter, vtkDICOMDataElementIterator iterEnd)
{
  while (iter != iterEnd)
  {
    // check for group length tag and length-to-end tag
    if (iter->GetTag().GetElement() == Hx0000 || iter->GetTag() == vtkDICOMTag(0x0008, 0x0001))
    {
      ...
      if (group == 0x0008 && iter != iterEnd && iter->GetTag() == vtkDICOMTag(0x0008, 0x0001))
      {
        hadLengthToEnd = true;
      ...
      if (this->Depth == 0 &&
        ((groupLength != HxFFFFFFFF && group == 0x7FE0 && iterEnd == groupEnd) ||
          lengthToEnd != HxFFFFFFFF))
      {
        // add the size of the missing PixelData element
        unsigned int pl = vtkDICOMCompilerInternalFriendship::ComputePixelDataSize(this->Compiler);
        unsigned int hl = (this->ImplicitVR ? 8 : 12);
        // check for overflow of lengthToEnd
        if (HxFFFFFFFF - lengthToEnd <= pl || HxFFFFFFFF - lengthToEnd - pl <= hl)
        {
          lengthToEnd = HxFFFFFFFF;
        }
        else
        ...
        if (group == 0x7FE0)
        {
          if (HxFFFFFFFF - groupLength <= pl || HxFFFFFFFF - groupLength - pl <= hl)
          {
      ...
      if (lengthToEnd != HxFFFFFFFF)
      {
        ...
        this->WriteDataElement(cp, ep,
          vtkDICOMDataElement(
            vtkDICOMTag(0x0008, 0x0001), vtkDICOMValue(vtkDICOMVR::UL, lengthToEnd)));
      }
    }
      ...
      iter->GetTag() == vtkDICOMTag(DC::SOPInstanceUID))
    {
      this->WriteDataElement(cp, ep,
        vtkDICOMDataElement(
          vtkDICOMTag(DC::SOPInstanceUID), vtkDICOMValue(vtkDICOMVR::UI, this->SOPInstanceUID)));
      ++iter;
    }
    ...
    {
      this->WriteDataElement(cp, ep,
        vtkDICOMDataElement(vtkDICOMTag(DC::SeriesInstanceUID),
    ...
    {
      this->WriteDataElement(cp, ep,
        vtkDICOMDataElement(vtkDICOMTag(DC::StudyInstanceUID),
...
vtkDICOMCompiler::vtkDICOMCompiler()
{
  ...
  const char* impuid = vtkDICOMUtilities::GetImplementationClassUID();
  ...
  const char* impname = vtkDICOMUtilities::GetImplementationVersionName();
...
void vtkDICOMCompiler::GenerateSeriesUIDs()
{
  ...
  if (this->MetaData)
  {
    ...
    this->SeriesUIDs->InsertNextValue(vtkDICOMUtilities::GenerateUID(DC::SeriesInstanceUID));
  }
}
...
bool vtkDICOMCompiler::WriteFile(vtkDICOMMetaData* data, int idx)
{
  ...
  if ((this->SOPInstanceUID == 0 || this->SeriesInstanceUID == 0) &&
    (idx == 0 || this->SeriesUIDs == 0 ||
      this->SeriesUIDs->GetNumberOfValues() != data->GetNumberOfInstances() + 1))
  {
...
void vtkDICOMCompiler::WriteFrame(const unsigned char* cp, vtkIdType size)
{
  ...
  if (this->Compressed)
  {
    ...
    {
      unsigned int numFrames = this->MetaData->Get(DC::NumberOfFrames).AsUnsignedInt();
    ...
    this->MetaData->Get(DC::BitsAllocated).AsInt() > 8)
  {
    ...
    int scalarSize = (this->MetaData->Get(DC::BitsAllocated).AsInt() + 7) / 8;
...
bool vtkDICOMCompiler::WriteMetaHeader(
  unsigned char*& cp, unsigned char*& ep, vtkDICOMMetaData* meta, int idx)
{
  ...
  std::string classUIDString = meta->Get(DC::SOPClassUID).AsString();
  if (classUIDString == "")
  {
    // if not present (e.g. DICOMDIR) get it from the meta header
    classUIDString = meta->Get(DC::MediaStorageSOPClassUID).AsString();
  }
  ...
  {
    implementationUID = vtkDICOMUtilities::GetImplementationClassUID();
  }

  vtkDICOMItem item;
  item.Set(DC::FileMetaInformationGroupLength, vtkDICOMValue(vtkDICOMVR::UL, l));
  item.Set(DC::FileMetaInformationVersion, vtkDICOMValue(vtkDICOMVR::OB, metaver, 2));
  item.Set(DC::MediaStorageSOPClassUID, vtkDICOMValue(vtkDICOMVR::UI, classUID));
  item.Set(DC::MediaStorageSOPInstanceUID, vtkDICOMValue(vtkDICOMVR::UI, instanceUID));
  item.Set(DC::TransferSyntaxUID, vtkDICOMValue(vtkDICOMVR::UI, this->TransferSyntaxUID));
  item.Set(DC::ImplementationClassUID, vtkDICOMValue(vtkDICOMVR::UI, implementationUID));

  if (this->ImplementationVersionName)
  {
    item.Set(DC::ImplementationVersionName,
      vtkDICOMValue(vtkDICOMVR::SH, this->ImplementationVersionName));
  }
  ...
  const char* creatorUID = meta->Get(DC::PrivateInformationCreatorUID).GetCharData();
  if (creatorUID)
  {
  ...
  unsigned char*& cp, unsigned char*& ep, vtkDICOMMetaData* meta, int idx)
{
  ...
  std::string tsyntax = (this->TransferSyntaxUID ? this->TransferSyntaxUID : "");
  ...
  if (r && hasPixelData && (r = encoder->CheckBuffer(cp, ep, 12)) != false)
  {
    ...
    size_t l = encoder->WriteElementHead(cp, vtkDICOMTag(DC::PixelData), vr, vl);
    cp += l;
  }
...
unsigned int vtkDICOMCompiler::ComputePixelDataSize()
{
  ...
  if (!this->Compressed)
  {
    ...
    if (bitsAllocated > 0)
    {
      ...
      unsigned int m = meta->Get(this->Index, DC::SamplesPerPixel).AsUnsignedInt();
      unsigned int n = meta->Get(this->Index, DC::NumberOfFrames).AsUnsignedInt();
      vl *= ((m > 1) ? m : 1);
      vl *= ((n > 1) ? n : 1);
...
//----------------------------------------------------------------------------
bool vtkDICOMCompiler::FlushBuffer(unsigned char*& ucp, unsigned char*& ep)
{
  bool rval = true;
...
{
  this->SetErrorCode(vtkErrorCode::FileFormatError);
  vtkErrorMacro("Error while writing file " << this->FileName << ": " << message);
}
...
void vtkDICOMCompiler::DiskFullError()
{
  ...
  vtkErrorMacro("Error while writing file " << this->FileName << ": Out of disk space.");
}
...
void vtkDICOMCompiler::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "SOPInstanceUID: " << (this->SOPInstanceUID ? this->SOPInstanceUID : "(NULL)")
     << "\n";
  os << indent
     << "SeriesInstanceUID: " << (this->SeriesInstanceUID ? this->SeriesInstanceUID : "(NULL)")
     << "\n";
  os << indent
     << "StudyInstanceUID: " << (this->StudyInstanceUID ? this->StudyInstanceUID : "(NULL)")
     << "\n";
  os << indent << "ImplementationClassUID: "
     << (this->ImplementationClassUID ? this->ImplementationClassUID : "(NULL)") << "\n";
  os << indent << "ImplementationVersionName: "
     << (this->ImplementationVersionName ? this->ImplementationVersionName : "(NULL)") << "\n";
  os << indent << "SourceApplicationEntityTitle: "
     << (this->SourceApplicationEntityTitle ? this->SourceApplicationEntityTitle : "(NULL)")
     << "\n";
  os << indent
     << "TransferSyntaxUID: " << (this->TransferSyntaxUID ? this->TransferSyntaxUID : "(NULL)")
     << "\n";
