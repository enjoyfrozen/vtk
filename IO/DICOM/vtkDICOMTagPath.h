 ...
 */
class VTKIODICOM_EXPORT vtkDICOMTagPath
{
public:
  //@{
  ...
  vtkDICOMTagPath(
    vtkDICOMTag seqTag, unsigned int i, vtkDICOMTag tag, unsigned int j, vtkDICOMTag tag2)
    : Size(3)
    , Head(seqTag)
    , Index(i)
    , Tail(tag)
  ...
  bool HasTail() const { return (this->Size > 1); }

  //! Get the path head, which should be a sequence if HasTail() is true.
  vtkDICOMTag GetHead() const { return this->Head; }
...
private:
  ...
  vtkDICOMTagPath(vtkDICOMTag seqTag, unsigned int i, vtkDICOMTag tag, unsigned int n, Pair* l)
    : Size(n + 2)
    , Head(seqTag)
    , Index(i)
    , Tail(tag)
...
VTKIODICOM_EXPORT ostream& operator<<(ostream& o, const vtkDICOMTagPath& a);
