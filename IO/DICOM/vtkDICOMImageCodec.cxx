...
//----------------------------------------------------------------------------
const char* vtkDICOMImageCodec::UIDs[21] = { "1.2.840.10008.1.2.1", // LittleEndian
...
//----------------------------------------------------------------------------
int vtkDICOMImageCodec::DecodeRLE(const ImageFormat& image, const unsigned char* source,
  size_t sourceSize, unsigned char* dest, size_t destSize)
{
  ...
  for (unsigned int i = 0; i < n; i++)
  {
    ...
    unsigned int offset = vtkDICOMUtilities::UnpackUnsignedInt(inPtr + (i + 1) * 4);
    if (offset >= sourceSize)
    {
      break;
    }
    // loop over the segment and decompress it
    const signed char* cp = reinterpret_cast<const signed char*>(inPtr + offset);
...
//----------------------------------------------------------------------------
int vtkDICOMImageCodec::EncodeRLE(const ImageFormat& image, const unsigned char* source,
  size_t sourceSize, unsigned char** destP, size_t* destSizeP)
{
  int errorCode = NoError;
  ...
  for (unsigned int i = 0; i < n; i++)
  {
    ...
    const signed char* cp = reinterpret_cast<const signed char*>(source + inOffset);
...
//----------------------------------------------------------------------------
int vtkDICOMImageCodec::Decode(const ImageFormat& image, const unsigned char* source,
  size_t sourceSize, unsigned char* dest, size_t destSize) const
{
...
//----------------------------------------------------------------------------
int vtkDICOMImageCodec::Encode(const ImageFormat& image, const unsigned char* source,
  size_t sourceSize, unsigned char** dest, size_t* destSize) const
{
  int code = MissingCodec;
  if (this->Key == RLE)
