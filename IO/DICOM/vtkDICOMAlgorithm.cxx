...
{
  vtkInformation* inInfo = inputVector[inputPort]->GetInformationObject(inputConnection);
...
//----------------------------------------------------------------------------
void vtkDICOMAlgorithm::CopyMetaDataToOutputInformation(vtkInformationVector** inputVector,
  int inputPort, int inputConnection, vtkInformationVector* outputVector, int outputPort)
{
  // If SetInputConnection was used, meta data is attached to inInfo
  vtkInformation* metaInfo = this->GetMetaDataInformation(inputVector, inputPort, inputConnection);
  ...
  for (int port = firstPort; port <= lastPort; port++)
  {
    ...
    vtkDICOMMetaData* meta = vtkDICOMMetaData::SafeDownCast(metaInfo->Get(META_DATA()));
    if (meta)
    {
      vtkSmartPointer<vtkDICOMMetaData> outMeta = vtkSmartPointer<vtkDICOMMetaData>::New();
...
//----------------------------------------------------------------------------
void vtkDICOMAlgorithm::CopyMetaDataToOutputData(vtkInformation* info, vtkDataObject* data)
{
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (this->GetNumberOfInputPorts() > 0 && this->GetNumberOfInputConnections(0) > 0)
  {
    this->CopyMetaDataToOutputInformation(inputVector, 0, 0, outputVector, -1);
  }
...
//----------------------------------------------------------------------------
void vtkDICOMAlgorithm::ThreadedRequestData(vtkInformation*, vtkInformationVector**,
  vtkInformationVector*, vtkImageData***, vtkImageData**, int[6], int)
{
  vtkWarningMacro(
    "ThreadedRequestData: This method was not implemented in " << this->GetClassName());
}
