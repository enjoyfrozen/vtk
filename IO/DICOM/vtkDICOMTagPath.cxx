...
//----------------------------------------------------------------------------
vtkDICOMTagPath::vtkDICOMTagPath(const vtkDICOMTagPath& p, unsigned int i, vtkDICOMTag tag)
  : Size(p.Size + 1)
  , Head(p.Head)
  , Index(p.Index)
  , Tail(p.Tail)
...
vtkDICOMTagPath vtkDICOMTagPath::GetTail() const
{
  ...
  {
    return vtkDICOMTagPath(
      this->Tail, this->List[0].Index, this->List[0].Tag, this->List[1].Index, this->List[1].Tag);
  }
...
bool vtkDICOMTagPath::operator==(const vtkDICOMTagPath& b) const
{
  ...
  if (this->Size > 0)
  {
    ...
    if (this->Size > 1)
    {
      ...
      if (this->Size == 3)
      {
        return (this->Last.Index == b.Last.Index && this->Last.Tag.Key == b.Last.Tag.Key);
      }
      unsigned int n = this->Size - 2;
      for (unsigned int i = 0; i < n; i++)
      {
        if (this->List[i].Index != b.List[i].Index || this->List[i].Tag.Key != b.List[i].Tag.Key)
        {
          return false;
        }
      }
