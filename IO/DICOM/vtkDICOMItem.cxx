...
vtkDICOMItem::vtkDICOMItem(vtkDICOMMetaData* meta)
{
  ...
  if (meta)
  {
    ...
    if (vcs.IsValid())
    {
      this->L->CharacterSet = vtkDICOMCharacterSet(vcs.GetCharData(), vcs.GetVL());
    }
    const vtkDICOMValue& v = meta->Get(DC::PixelRepresentation);
    if (v.IsValid())
    {
      this->L->VRForXS = (v.AsUnsignedShort() == 0 ? vtkDICOMVR::US : vtkDICOMVR::SS);
    }
  }
}
...
vtkDICOMItem::vtkDICOMItem(
  vtkDICOMCharacterSet cs, vtkDICOMVR vrForXS, int delimited, unsigned int byteOffset)
{
  this->L = new List;
  this->L->Head.Next = &this->L->Tail;
  this->L->Tail.Prev = &this->L->Head;
...
vtkDICOMDataElement* vtkDICOMItem::NewDataElement(vtkDICOMDataElement** iter)
{
  ...
  else if (n >= 4 && (n & (n - 1)) == 0)
  {
    ...
    {
      --n;
    } while (n > 0 && this->L->DataElements[n].Next != 0);
    ...
    if (this->L->DataElements[n].Next != 0)
    {
      ...
      vtkDICOMItem::CopyDataElements(this->L->Head.Next, &this->L->Tail, this->L);
      if (iter)
      {
...
//----------------------------------------------------------------------------
vtkDICOMItem* vtkDICOMItem::FindItemOrInsert(const vtkDICOMTagPath& tagpath, vtkDICOMTag* tagptr)
{
  vtkDICOMItem* item = 0;
...
//----------------------------------------------------------------------------
vtkDICOMTag vtkDICOMItem::ResolvePrivateTag(vtkDICOMTag ptag, const std::string& creator) const
{
...
//----------------------------------------------------------------------------
vtkDICOMTag vtkDICOMItem::ResolvePrivateTagForWriting(vtkDICOMTag ptag, const std::string& creator)
{
...
bool vtkDICOMItem::operator==(const vtkDICOMItem& ob) const
{
  ...
  {
    r = false;
    if (t != 0 && o != 0 && t->NumberOfDataElements == o->NumberOfDataElements)
    {
      r = true;
