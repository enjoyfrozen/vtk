  ...
  , Entries(0)
{
  ...
  else
  {
    ...
    else
    {
      ...
      do
      {
        WideCharToMultiByte(CP_UTF8, 0, fileData.cFileName, -1, name, n, 0, 0);
        ...
        this->AddEntry(
          name, flags, (TypeSymlink | TypeBroken | TypeDirectory | TypeSpecial | TypeHidden));
...
//----------------------------------------------------------------------------
vtkDICOMFileDirectory& vtkDICOMFileDirectory::operator=(const vtkDICOMFileDirectory& o)
{
  if (this != &o)
  {
    delete[] this->Entries;
...
//----------------------------------------------------------------------------
void vtkDICOMFileDirectory::AddEntry(const char* name, unsigned short flags, unsigned short mask)
{
...
int vtkDICOMFileDirectory::Access(const char* dirname, Mode mode)
{
  ...
  int errorCode = 0;
  struct stat fs;
  if (stat(dirname, &fs) != 0 || access(dirname, (mode == In ? R_OK : W_OK)) != 0)
  {
    int e = errno;
    if (e == EACCES || e == EPERM)
    {
...
int vtkDICOMFileDirectory::Create(const char* name)
{
  ...
  while (dirsToCreate.size() > 0 && result == 0)
  {
    ...
    else if (!CreateDirectoryW(widename, NULL))
    {
      ...
      if (e == ERROR_ACCESS_DENIED || e == ERROR_ALREADY_EXISTS)
      {
        result = AccessDenied;
      }
