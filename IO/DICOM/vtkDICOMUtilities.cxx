...
int vtkDICOMUtilities::CompareUIDs(const char* u1, const char* u2)
{
  ...
  else
  {
    while (r == 0 && *u1 != 0 && *u2 != 0)
    {
      ...
      {
        i1++;
      } while (u1[i1] >= '0' && u1[i1] <= '9');
      do
      {
        i2++;
      } while (u2[i2] >= '0' && u2[i2] <= '9');
...
long long vtkDICOMUtilities::GetUniversalTime()
{
  ...
  long long t1 =
    ((static_cast<long long>(ft.dwHighDateTime) << 32) + static_cast<long long>(ft.dwLowDateTime));
...
std::string vtkDICOMUtilities::GenerateDateTime(long long t, const char* z)
{
  ...
  if (z && (z[0] == '+' || z[0] == '-') && (z[1] >= '0' && z[1] <= '9') &&
    (z[2] >= '0' && z[2] <= '9') && (z[3] >= '0' && z[3] <= '9') && (z[4] >= '0' && z[4] <= '9'))
  {
  ...
  else
  {
    ...
    tzs[0] = (zst < 0 ? '-' : '+');
    zst = (zst < 0 ? -zst : zst);
    sprintf(&tzs[1], "%02d%02d", static_cast<int>((zst / 3600) % 24),
  ...
  char dt[84];
  sprintf(dt, "%04d%02d%02d%02d%02d%02d.%06d%s", y, m, d, H, M, S, us, z);
...
long long vtkDICOMUtilities::ConvertDateTime(const char* datetime)
{
  ...
  if (*cp == '-' || *cp == '+')
  {
    ...
    *tp++ = *cp++;
    ...
    tp = normalized + 21;
    zs = ((tp[1] - '0') * 600 + (tp[2] - '0') * 60 + (tp[3] - '0') * 10 + (tp[4] - '0')) * 60 *
      1000000ll;
    if (tp[0] == '-')
    {
      zs = -zs;
  ...
  long long jd = (1461 * (y + 4800 + (m - 14) / 12)) / 4 +
    (367 * (m - 2 - 12 * ((m - 14) / 12))) / 12 - (3 * ((y + 4900 + (m - 14) / 12) / 100)) / 4 + d -
    32075;
...
bool vtkDICOMUtilities::IsDICOMFile(const char* filename)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    {
      cp = &buffer[skip];
      if (cp[0] == 'D' && cp[1] == 'I' && cp[2] == 'C' && cp[3] == 'M' && cp[4] == 2 &&
        cp[5] == 0 && cp[6] == 0 && cp[7] == 0)
      {
        return true;
      }
    }
  ...
  if (cp[0] == 2 && cp[1] == 0 && cp[2] == 0 && cp[3] == 0 && cp[4] == 'U' && cp[5] == 'L' &&
    cp[6] == 4 && cp[7] == 0)
  {
    return true;
  }
  ...
  if (cp[0] == 0x08 && cp[1] == 0x00 && cp[2] <= 0x16 && cp[3] == 0x00 && cp[4] <= 0x40 &&
    cp[5] == 0x00 && cp[6] == 0x00 && cp[7] == 0x00)
  {
    unsigned char e = cp[2];
    cp += (cp[4] + 8);
    if (cp[0] == 0x08 && cp[1] == 0x00 && cp[2] <= 0x18 && cp[3] == 0x00 && cp[4] <= 0x40 &&
      cp[5] == 0x00 && cp[6] == 0x00 && cp[7] == 0x00 && cp[2] > e)
    {
      return true;
    }
  }
...
//----------------------------------------------------------------------------
char vtkDICOMUtilities::ImplementationClassUID[65] = "2.25.190146791043182537444806132342625375407";
...
//----------------------------------------------------------------------------
#define VTK_DICOM_VERSION_CREATOR2(name, x, y, z) name "_" #x "_" #y "_" #z
#define VTK_DICOM_VERSION_CREATOR(name, x, y, z) VTK_DICOM_VERSION_CREATOR2(name, x, y, z)

char vtkDICOMUtilities::ImplementationVersionName[17] = VTK_DICOM_VERSION_CREATOR(
  "VTK_DICOM", DICOM_MAJOR_VERSION, DICOM_MINOR_VERSION, DICOM_PATCH_VERSION);
...
//----------------------------------------------------------------------------
bool vtkDICOMUtilities::PatternMatches(const char* pattern, size_t pl, const char* val, size_t vl)
{
  ...
  while (cp != ep && dp != fp)
  {
    ...
    else if (*cp == *dp)
    {
      ...
      if (static_cast<signed char>(*dp++) < 0)
      {
        while (cp != ep && dp != fp && ((*cp & 0xC0) == 0x80 || (*dp & 0xC0) == 0x80))
        {
          if (*dp != *cp)
          {
            return false;
...
{
  return vtkDICOMUtilities::PatternMatches(pattern, strlen(pattern), val, strlen(val));
}
  ...
  while (cp != ep)
  {
    if (cp[0] != '\0')
    {
      ...
      if (cp + 1 != ep && (cp[1] & 0xC0) == 0x80)
      {
        ...
        unsigned short x = (((unsigned char)(cp[0]) << 8) | (unsigned char)(cp[1]));
