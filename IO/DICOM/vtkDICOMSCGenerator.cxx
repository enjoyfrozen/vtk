...
//----------------------------------------------------------------------------
bool vtkDICOMSCGenerator::GenerateSCMultiFrameImageModule(vtkDICOMMetaData* source)
{
  ...
  if (dims[3] > 0 || (dims[2] == 0 && nframes == 1))
  {
    ...
    meta->Set(DC::FrameTimeVector, vtkDICOMValue(vtkDICOMVR::DS, tvector, nframes));
  }
  if (dims[2] > 0)
  {
    pointers[npointers++] = DC::SliceLocationVector;
    meta->Set(DC::SliceLocationVector, vtkDICOMValue(vtkDICOMVR::DS, zvector, nframes));
  }

  meta->Erase(DC::FrameTime);
  meta->Set(DC::FrameIncrementPointer, vtkDICOMValue(vtkDICOMVR::AT, pointers, npointers));
  ...
  static const DC::EnumType optional[] = { DC::RecognizableVisualFeatures, DC::Illumination,
    ...
    DC::DigitizingDeviceTransportDirection, DC::RotationOfScannedFilm, DC::ItemDelimitationItem };
...
bool vtkDICOMSCGenerator::GenerateSCImageModule(vtkDICOMMetaData* source)
{
  ...
  // optional and conditional: direct copy of values with no checks
  static const DC::EnumType optional[] = { DC::DateOfSecondaryCapture, DC::TimeOfSecondaryCapture,
    DC::NominalScannedPixelSpacing, DC::DocumentClassCodeSequence, DC::ItemDelimitationItem };
...
bool vtkDICOMSCGenerator::GenerateSCEquipmentModule(vtkDICOMMetaData* source)
{
  ...
  // optional and conditional: direct copy of values with no checks
  static const DC::EnumType optional[] = { DC::Modality, DC::SecondaryCaptureDeviceID,
    DC::SecondaryCaptureDeviceManufacturer, DC::SecondaryCaptureDeviceManufacturerModelName,
    DC::SecondaryCaptureDeviceSoftwareVersions, DC::VideoImageFormatAcquired,
    DC::DigitalImageFormatAcquired, DC::ItemDelimitationItem };
...
bool vtkDICOMSCGenerator::GenerateSCMultiFrameInstance(vtkInformation* info)
{
  ...
  {
    this->SetPixelRestrictions(RepresentationUnsigned, BitsStored8, numComponents);
  ...
  else
  {
    this->SetPixelRestrictions(
      RepresentationUnsigned, BitsStored10 | BitsStored12 | BitsStored16, 1);
  ...
  if (!this->GenerateSOPCommonModule(source, SOPClass) || !this->GeneratePatientModule(source) ||
    !this->GenerateClinicalTrialSubjectModule(source) ||
    !this->GenerateGeneralStudyModule(source) || !this->GeneratePatientStudyModule(source) ||
    !this->GenerateClinicalTrialStudyModule(source) || !this->GenerateGeneralSeriesModule(source) ||
    !this->GenerateClinicalTrialSeriesModule(source) ||
    // optional General Equipment Module intentionally omitted
    !this->GenerateSCEquipmentModule(source) || !this->GenerateGeneralImageModule(source) ||
    !this->GenerateImagePixelModule(source) ||
    // the Cine Module is added by GenerateSCMultiFrameImageModule
    !this->GenerateDeviceModule(source) || !this->GenerateSpecimenModule(source) ||
...
bool vtkDICOMSCGenerator::GenerateSCInstance(vtkInformation* info)
{
  ...
  this->SetPixelRestrictions(RepresentationUnsigned | RepresentationSigned,
    BitsStored8 | BitsStored10 | BitsStored12 | BitsStored16, samplesPerPixel);
  ...
  if (!this->GenerateSOPCommonModule(source, SOPClass) || !this->GeneratePatientModule(source) ||
    ...
    !this->GenerateSCEquipmentModule(source) || !this->GenerateGeneralImageModule(source) ||
    !this->GenerateImagePixelModule(source) || !this->GenerateDeviceModule(source) ||
    !this->GenerateSpecimenModule(source) || !this->GenerateSCImageModule(source) ||
