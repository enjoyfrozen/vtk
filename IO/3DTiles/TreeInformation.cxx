 ...
 */
std::array<double, 6> ToLonLatHeight(const char* srsName, int utmZone, char utmHemisphere,
...
vtkSmartPointer<vtkMatrix4x4> ComputeTransform(const char* srsName, int utmZone, char utmHemisphere,
  const std::array<double, 3>& offset, const std::array<double, 6>& bounds)
{
...
TreeInformation::TreeInformation(vtkIncrementalOctreeNode* root, int numberOfNodes,
  ...
  Root(root)
  , Buildings(buildings)
  , BuildingActorStart(buildingActorStart)
  , Offset(offset)
  , Actors(actors)
...
void TreeInformation::SaveGLTF(vtkIncrementalOctreeNode* node)
{
  if (node->IsLeaf() && !this->EmptyNode[node->GetID()])
  {
    ...
    for (int i = 0; i < pointIds->GetNumberOfIds(); ++i)
    {
      ...
      // ostr << buildingId << ": ["
      //      << this->BuildingActorStart[buildingId] << ", "
      //      << this->BuildingActorStart[buildingId + 1] << "] ";
      for (int j = this->BuildingActorStart[buildingId];
           j < this->BuildingActorStart[buildingId + 1]; ++j)
      {
...
{
  this->Generate3DTiles(this->Root, output);
}

//------------------------------------------------------------------------------
void TreeInformation::Generate3DTiles(vtkIncrementalOctreeNode* root, const std::string& output)
{

  this->RootJson["asset"]["version"] = "1.0";
  this->RootJson["geometricError"] = this->ComputeTilesetGeometricError();
  this->RootJson["root"] = this->Generate3DTiles(root);
...
//------------------------------------------------------------------------------
Json::Value TreeInformation::Generate3DTiles(vtkIncrementalOctreeNode* node)
{
  Json::Value tree;
  Json::Value v;
  ...
  if (!node->IsLeaf())
  {
    ...
    for (int i = 0, j = 0; i < 8; i++)
    {
      ...
      {
        v[j++] = this->Generate3DTiles(node->GetChild(i));
      }
      tree["children"] = v;
    }
  }
