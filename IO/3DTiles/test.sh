...
VTK_TILER=/home/danlipsa/projects/VTK/build/bin/vtkTiler-9.0
...
if [ "${CITY}" = "jacksonville" ]; then
    ...
    if [ ! ${GLTF_ONLY} ]; then
        rm -rf ${dir}
        mkdir ${dir}
        ${VTK_TILER} ../../../data/CORE3D/Jacksonville/building_*building*.obj -o 3d-tiles --utm_zone 17 --utm_hemisphere N -b 16 --dont_save_textures --number_of_buildings 17
    fi
    ...
    if [ ! ${GLTF_ONLY} ]; then
        rm -rf ${dir}
        mkdir ${dir}
        ${VTK_TILER} ../../../data/CORE3D/Jacksonville/triangle.obj -o 3d-tiles --utm_zone 17 --utm_hemisphere N -b 2 --dont_save_textures
    fi
    ...
    if [ ! ${GLTF_ONLY} ]; then
        rm -rf $dir
        mkdir $dir
        ${VTK_TILER} ../../../data/Berlin-3D/Charlottenburg-Wilmersdorf/citygml.gml -o berlin-3d-tiles --utm_zone 33 --utm_hemisphere N -b 100 --dont_save_textures --number_of_buildings 800
    fi
    ...
    if [ ! ${GLTF_ONLY} ]; then
        rm -rf $dir
        mkdir $dir
        ${VTK_TILER} ../../../data/Berlin-3D/Charlottenburg-Wilmersdorf/triangle-berlin.gml -o berlin-3d-tiles --utm_zone 33 --utm_hemisphere N -b 100 --dont_save_textures --number_of_buildings 1
    fi
elif [ "${CITY}" = "nyc" ]; then
    dir=ny-3d-tiles
    rm -rf $dir
