...
bool PIO_DATA::reconstruct_chunk_field(int64_t numcell, std::valarray<double>& va,
  const char* prefix, const char* var, int materialId, uint32_t numMaterials)
{
  std::string PreFix = std::string(prefix);
  std::string name = PreFix + "_" + var;
  std::string chunk_nummat_string = PreFix + "_nummat";
  std::string chunk_mat_string = PreFix + "_mat";

  if ((VarMMap.count(name.c_str()) != 1) || (VarMMap.count(chunk_nummat_string.c_str()) != 1) ||
    (VarMMap.count(chunk_mat_string.c_str()) != 1))
  {
    return false;
  }
  const double* cl = GetPIOData(name.c_str());
