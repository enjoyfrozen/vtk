...
void PIOAdaptor::collectMaterialVariableMetaData()
{
  ...
  this->numMaterials = this->pioData->VarMMap.count("matdef");
  ...
  if (this->pioData->VarMMap.count("matident") > 0)
  {
    ...
    for (uint32_t i = 0; i < this->numMaterials; i++)
    {
  ...
  else
  {
    ...
    for (uint32_t i = 0; (i < this->numMaterials) && (ii != b.second); i++, ii++)
    {
  ...
  for (int i = 0; i < numberOfFields; i++)
  {
    ...
    for (size_t j = 0; j < prefixes.size(); j++)
    {
      ...
      if (matchPrefix == 0)
      {
        ...
        if ((matchNummat == std::string::npos) and (matchMat == std::string::npos))
        {
