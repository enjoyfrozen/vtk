  ...
  do
  {
    ...
    if (parcelExists)
    {
      ...
      if (err < 0 || objectInfo.type == H5O_TYPE_GROUP)
      {
        ...
        herr_t err = H5Gget_num_objs(parcelDataTypesHandle, &numParcelDataTypes);
        if (err < 0)
        {
             ...
             ++parcelDataTypeIndex)
        {
          ...
          herr_t err = H5Gget_info(dataTypeHandle, &groupInfo);
          if (err < 0)
          {
          ...
          for (hsize_t i = 0; i < numDataSets; ++i)
          {
            ...
            herr_t status = H5Lget_name_by_idx(dataTypeHandle, ".", H5_INDEX_NAME, H5_ITER_NATIVE,
              i, dataSetGroupName, 256, H5P_DEFAULT);
            if (status < 0)
            {
              continue;
            }
