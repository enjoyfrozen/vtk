  ...
  vtkInformation*, vtkInformationVector**, vtkInformationVector* outputVector)
{
  ...
  do
  {
    ...
    hsize_t numBoundaryNames = GetDataLength(boundaryId, "BOUNDARY_NAMES");
    std::vector<std::string> boundaryNames(numBoundaryNames);
    ReadStrings(boundaryId, "BOUNDARY_NAMES", boundaryNames);
    hsize_t numBoundaries = GetDataLength(boundaryId, "NUM_ELEMENTS");
    std::vector<int> boundaryNumElements(numBoundaries);
    ReadArray(boundaryId, "NUM_ELEMENTS", boundaryNumElements.data(), boundaryNumElements.size());
    std::vector<int> boundaryIds(numBoundaries);
    ReadArray(boundaryId, "BOUNDARY_IDS", boundaryIds.data(), boundaryIds.size());
    ...
    for (size_t i = 0; i < boundaryIds.size(); ++i)
    {
      ...
      multiSurfaces->SetBlock(i, boundarySurface);
      multiSurfaces->GetMetaData(i)->Set(vtkCompositeDataSet::NAME(), boundaryNames[i]);
    ...
    for (int polyId = 0; polyId < numPolygons; ++polyId)
    {
      ...
      vtkIdType numCellPts = static_cast<vtkIdType>(polygonOffsets[polyId + 1] - polygonOffsets[polyId]);
    ...
    for (hsize_t boundaryIndex = 0; boundaryIndex < numBoundaryNames; ++boundaryIndex)
    {
      ...
      vtkPolyData* boundarySurface = vtkPolyData::SafeDownCast(multiSurfaces->GetBlock(boundaryIndex));
    ...
    for (int i = 0; i < this->CellDataArraySelection->GetNumberOfArrays(); ++i)
    {
      ...
      if (success)
      {
        ...
        for (int boundaryIndex = 0; boundaryIndex < multiSurfaces->GetNumberOfBlocks(); ++boundaryIndex)
        {
          vtkPolyData* boundarySurface = vtkPolyData::SafeDownCast(multiSurfaces->GetBlock(boundaryIndex));
          ...
          for (vtkIdType id = 0; id < numSurfacePolys; ++id)
          {
            ...
            if (boundaryIdToIndex.find(polyToBoundary[id]) == boundaryIdToIndex.end()) {
              vtkErrorMacro("polyToBoundary[id] is not found within boundaryIdToIndex" << polyToBoundary[id]);
              return 0;
            }
