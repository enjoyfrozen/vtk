...
public:
  ...
  /**
   * Returns the name of the file currently opened by the reader.
   */
  ...
  /**
   * Get the grids array selection tables used to configure which grids
   * are loaded by the reader.
   */
  vtkGetObjectMacro(GridSelection, vtkDataArraySelection);

  /**
   * Get the number of grids available in the input.
   */
  ...
  /**
   * Get the short name of the grid with the given index in
   * the input.
   */
  const char* GetShortGridArrayName(int index);

  /**
   * Get the VTK object type corresponding to a given grid index.
   * This is VTK_POLY_DATA for a point cloud, VTK_IMAGE_DATA for an image volume, VTK_DATA_SET when
   * unsupported and -1 when out of range.
   */
  int GetGridArrayType(int index);

  ///@{
  /**
   * Get/Set whether the grid with the given name is to
   * be read.
   */
  int GetGridArrayStatus(const char* name);
  void SetGridArrayStatus(const char* name, int status);
  ///@}
...
protected:
  ...
  vtkSmartPointer<vtkDataArraySelection> GridSelection =
...
private:
  ...
  vtkOpenVDBReaderInternals* Internals;
};
