...
int TestNetCDFUGRIDReader(int argc, char* argv[])
{
  ...
  check(std::equal(cellIds, cellIds + 4, expectedQuadIds.begin()),
    "Wrong point ids for first cell");
  ...
  check(std::equal(hData.begin(), hData.begin() + 3, expectedPointData.begin()),
    "Wrong point data");
  check(std::isnan(hData[3]), "Fill value not correctly replaced by NaN");
  check(std::equal(hData.begin() + 4, hData.end(), expectedPointData.begin() + 4),
    "Wrong point data");
  ...
  check(std::equal(areaData.begin(), areaData.end(), expectedCellData.begin()),
    "Wrong point data");
