...
void SaveAttribute(int ncid, int attributeType, int varid, vtkDataArray* a)
{
  ...
  switch (type)
  {
    ...
    case VTK_SIGNED_CHAR:
      status =
        nc_put_var(ncid, varid, (unsigned char*)vtkSignedCharArray::SafeDownCast(a)->GetPointer(0));
      break;
  ...
  {
    std::ostringstream ostr;
    ostr << "Error nc_put_var type(" << type << ") " << a->GetName() << ": " << nc_strerror(status);
    throw std::runtime_error(ostr.str());
  }
}
...
void vtkNetCDFCFWriter::WriteData()
{
  try
  {
    ...
    {
      vtkDataArray* a = attributes->GetArray(i);
      SaveAttribute(ncid, this->AttributeType, attributeid[i], a);
    }
