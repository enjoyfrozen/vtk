...
extern "C"
{
  ...
  void vtkXMLWriterC_SetExtent(vtkXMLWriterC* self, int extent[6])
  {
    ...
    if (vtkImageData* imData = vtkImageData::SafeDownCast(self->DataObject))
    {
      imData->SetExtent(extent);
    }
    else if (vtkStructuredGrid* sGrid = vtkStructuredGrid::SafeDownCast(self->DataObject))
    {
      sGrid->SetExtent(extent);
    }
    else if (vtkRectilinearGrid* rGrid = vtkRectilinearGrid::SafeDownCast(self->DataObject))
    {
  ...
  void vtkXMLWriterC_SetPoints(vtkXMLWriterC* self, int dataType, void* data, vtkIdType numPoints)
  {
    ...
    if (vtkPointSet* dataObject = vtkPointSet::SafeDownCast(self->DataObject))
    {
      ...
      if (vtkSmartPointer<vtkDataArray> array =
            vtkXMLWriterC_NewDataArray("SetPoints", nullptr, dataType, data, numPoints, 3))
      {
        ...
        {
          vtkGenericWarningMacro("vtkXMLWriterC_SetPoints failed to create a vtkPoints object.");
        }
      }
    }
    else if (self->DataObject)
    ...
    vtkXMLWriterC* self, int axis, int dataType, void* data, vtkIdType numCoordinates)
  {
    ...
    if (vtkRectilinearGrid* dataObject = vtkRectilinearGrid::SafeDownCast(self->DataObject))
    {
      // Check the axis number.
      if (axis < 0 || axis > 2)
      {
        vtkGenericWarningMacro("vtkXMLWriterC_SetCoordinates called with invalid axis "
          << axis << ".  Use 0 for X, 1 for Y, and 2 for Z.");
      }

      // Create the vtkDataArray that will reference the coordinates.
      if (vtkSmartPointer<vtkDataArray> array = vtkXMLWriterC_NewDataArray(
    ...
    vtkXMLWriterC* self, int cellType, vtkIdType ncells, vtkIdType* cells, vtkIdType cellsSize)
  {
    ...
    if (vtkPolyData* dataObject = vtkPolyData::SafeDownCast(self->DataObject))
    {
      // Create a cell array to reference the cells.
      if (vtkSmartPointer<vtkCellArray> cellArray =
            vtkXMLWriterC_NewCellArray("SetCellsWithType", ncells, cells, cellsSize))
      {
    ...
    else if (vtkUnstructuredGrid* uGrid = vtkUnstructuredGrid::SafeDownCast(self->DataObject))
    {
      // Create a cell array to reference the cells.
      if (vtkSmartPointer<vtkCellArray> cellArray =
            vtkXMLWriterC_NewCellArray("SetCellsWithType", ncells, cells, cellsSize))
    ...
    vtkXMLWriterC* self, int* cellTypes, vtkIdType ncells, vtkIdType* cells, vtkIdType cellsSize)
  {
    ...
    if (vtkUnstructuredGrid* dataObject = vtkUnstructuredGrid::SafeDownCast(self->DataObject))
    {
      // Create a cell array to reference the cells.
      if (vtkSmartPointer<vtkCellArray> cellArray =
            vtkXMLWriterC_NewCellArray("SetCellsWithType", ncells, cells, cellsSize))
  ...
  int vtkXMLWriterC_Write(vtkXMLWriterC* self)
  {
    ...
    {
      vtkGenericWarningMacro("vtkXMLWriterC_Write called before vtkXMLWriterC_SetDataObjectType.");
      return 0;
    }
  }
  ...
  void vtkXMLWriterC_Start(vtkXMLWriterC* self)
  {
    ...
    else if (self->Writer)
    {
      ...
      {
        vtkGenericWarningMacro("vtkXMLWriterC_Start called with no time steps.");
      }
      else if (self->Writer->GetFileName() == nullptr)
      {
        vtkGenericWarningMacro("vtkXMLWriterC_Start called before vtkXMLWriterC_SetFileName.");
      }
      else
      {
    ...
    {
      vtkGenericWarningMacro("vtkXMLWriterC_Start called before vtkXMLWriterC_SetDataObjectType.");
    }
  }
  ...
  void vtkXMLWriterC_WriteNextTimeStep(vtkXMLWriterC* self, double timeValue)
  {
    ...
    if (!self->Writing)
    {
      vtkGenericWarningMacro("vtkXMLWriterC_WriteNextTimeStep called before vtkXMLWriterC_Start.");
    }
    else if (self->Writer)
    {
    ...
    {
      vtkGenericWarningMacro("vtkXMLWriterC_Stop called before vtkXMLWriterC_SetDataObjectType.");
    }
  }
  ...
  void vtkXMLWriterC_Stop(vtkXMLWriterC* self)
  {
    ...
    if (!self->Writing)
    {
      vtkGenericWarningMacro("vtkXMLWriterC_Stop called before vtkXMLWriterC_Start.");
    }
    else if (self->Writer)
    {
  ...
  const char* name, int dataType, void* data, vtkIdType numTuples, int numComponents)
{
  ...
  vtkSmartPointer<vtkDataArray> array = vtkDataArray::CreateDataArray(dataType);
  if (array)
  {
    array->Delete();
  }
  ...
  {
    vtkGenericWarningMacro(
      "vtkXMLWriterC_" << method << " could not allocate array of type " << dataType << ".");
    return nullptr;
  }
  ...
  const char* method, vtkIdType ncells, vtkIdType* cells, vtkIdType cellsSize)
{
  ...
  if (!array)
  {
    vtkGenericWarningMacro("vtkXMLWriterC_" << method << " failed to allocate a vtkIdTypeArray.");
    return nullptr;
  }
  ...
  if (!cellArray)
  {
    vtkGenericWarningMacro("vtkXMLWriterC_" << method << " failed to allocate a vtkCellArray.");
    return nullptr;
  }
  ...
  int isPoints)
{
  ...
  {
    if (vtkSmartPointer<vtkDataArray> array =
          vtkXMLWriterC_NewDataArray(method, name, dataType, data, numTuples, numComponents))
    {
