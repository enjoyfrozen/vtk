...
int TestXMLHyperTreeGridIO2(int argc, char* argv[])
{
  ...
  std::cout << "Writing TestXMLHyperTreeGridIO2_Appendedv2.htg" << std::endl;
  fname = tdir + std::string("/TestXMLHyperTreeGridIO2_Appendedv2.htg");
  writer->SetDataSetMajorVersion(2);
  writer->SetFileName(fname.c_str());
  writer->Write();

  std::cout << "Reading TestXMLHyperTreeGridIO2_Appendedv2.htg" << std::endl;
  reader->SetFileName(fname.c_str());
  reader->Update();
  htgRead = vtkHyperTreeGrid::SafeDownCast(reader->GetOutputDataObject(0));
  if (!AreHTGSame(htgWrite, htgRead))
  {
    std::cerr << "Appended Write and Read version 2 failed" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "Writing TestXMLHyperTreeGridIO2_Binaryv2.htg" << std::endl;
  fname = tdir + std::string("/TestXMLHyperTreeGridIO2_Binaryv2.htg");
  writer->SetDataSetMajorVersion(2);
  writer->SetFileName(fname.c_str());
  writer->Write();

  std::cout << "Reading TestXMLHyperTreeGridIO2_Binaryv2.htg" << std::endl;
  reader->SetFileName(fname.c_str());
  reader->Update();
  htgRead = vtkHyperTreeGrid::SafeDownCast(reader->GetOutputDataObject(0));
  if (!AreHTGSame(htgWrite, htgRead))
  {
    std::cerr << "Binary Write and Read version 2 failed" << std::endl;
    return EXIT_FAILURE;
  }
