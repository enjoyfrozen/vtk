...
//----------------------------------------------------------------------------
void vtkXMLHyperTreeGridReader::SetIndicesBoundingBox(
...
//----------------------------------------------------------------------------
void vtkXMLHyperTreeGridReader::ClearAndAddSelectedHT(
        unsigned int idg,
...
//----------------------------------------------------------------------------
void vtkXMLHyperTreeGridReader::AddSelectedHT(
        unsigned int idg,
        unsigned int fixedLevel)
{
  assert("pre: too_late" && not this->FixedHTs);
  assert("pre: not_clear_and_add_selected " &&
         this->SelectedHTs == IDS_SELECTED);
  this->IdsSelected[idg] = fixedLevel;
}

//----------------------------------------------------------------------------
void vtkXMLHyperTreeGridReader::CalculateHTs(
        ...
        unsigned int treeIndx) const
{
  ...
  switch (this->SelectedHTs)
  {
    ...
    case vtkXMLHyperTreeGridReader::INDICES_BOUNDING_BOX:
      ...
      return this->IndicesBoundingBox[0] >= i &&
             i <= this->IndicesBoundingBox[1] &&
             this->IndicesBoundingBox[2] >= j &&
             j <= this->IndicesBoundingBox[3] &&
             this->IndicesBoundingBox[4] >= k &&
             k <= this->IndicesBoundingBox[5];
...
void vtkXMLHyperTreeGridReader::ReadTrees_1(vtkXMLDataElement* elem)
{
  ...
  for (int treeIndxInFile = 0; treeIndxInFile < numberOfTrees; ++treeIndxInFile)
  {
    ...
    {
      vtkErrorMacro(
              "Cannot convert vtkDataArray of type " << desc_d->GetDataType()
                                                     << " to vtkBitArray.");
      desc_d->Delete();
      return;
    }
    ...
    treeCursor->GetTree()->InitializeForReader(
            numberOfLevels,
            fixedNbVertices,
            nbByLvl->GetValue(limitedLevel - 1),
            desc,
            mask,
            output->GetMask());
    nbByLvl_a->Delete();
    ...
    if (ePointData)
    {
      char* nameLevel = nullptr;
      char* nameScalar = nullptr;
      ...
      for (int j = 0; j < ePointData->GetNumberOfNestedElements(); ++j)
      {
        ...
        outArray->InsertTuple(
          outArray->GetNumberOfTuples() + fixedNbVertices -1,
          0,
          outArray);
        ...
        this->ReadArrayValues(eNested,
                              ...
                              outArray,
                              0,
                              fixedNbVertices * numberOfComponents,
                              POINT_DATA);
      }
    }
