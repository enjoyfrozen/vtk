...
void ReadHeader(
  istream& stream, vtkArrayExtents& extents, vtkArrayExtents::SizeT& non_null_size, vtkArray* array)
{
  if (!array)
...
vtkArray* vtkArrayReader::Read(istream& stream)
{
  try
  {
    ...
    if (header_magic == "vtk-sparse-array")
    {
      ...
      {
        return (read_binary ? ReadSparseArrayBinary<vtkIdType>(stream)
                            : ReadSparseArrayAscii<vtkIdType>(stream));
      }
    ...
    else if (header_magic == "vtk-dense-array")
    {
      if (header_type == "integer")
      {
        return (read_binary ? ReadDenseArrayBinary<vtkIdType>(stream)
                            : ReadDenseArrayAscii<vtkIdType>(stream));
      }
      else if (header_type == "double")
      {
        return (
