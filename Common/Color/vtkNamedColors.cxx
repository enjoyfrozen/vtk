...
{
public:
  vtkNamedColorsDataStore() { this->Init(); }
...
void vtkNamedColors::PrintSelf(ostream& os, vtkIndent indent)
{
       ...
       p != this->Colors->GetColorMap()->end(); ++p)
  {
    ...
    if (p->second.GetSize() == 4)
    {
      ...
      os << setw(3) << r << "," << setw(3) << g << "," << setw(3) << b << "," << setw(3) << a;
    }
    os << ")" << endl;
  }
}
...
{
  std::stringstream ss;
  ss << "#" << std::hex << std::setfill('0') << std::setw(2) << (int)(rgb.GetRed()) << std::setw(2)
     << (int)(rgb.GetGreen()) << std::setw(2) << (int)(rgb.GetBlue());
