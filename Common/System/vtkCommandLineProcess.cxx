...
std::vector<std::string> ParseCommand(std::string command)
{
  ...
  // 2/exec"`
  std::regex programRegex = std::regex(R"~(^\.?(\/?(\w+(\\ )?)+)+|(^"(\.?(\/?(\w+ ?)+)+))")~");
  std::smatch match;
  if (std::regex_search(command, match, programRegex))
  {
    auto m = match[0];
    res.emplace_back(command.substr(0, m.length()));
    command = command.substr(m.length(), command.length());
  }

  // Split by space for arguments.
