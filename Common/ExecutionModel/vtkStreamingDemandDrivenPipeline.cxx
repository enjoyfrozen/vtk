...
vtkInformationKeyMacro(
  vtkStreamingDemandDrivenPipeline, REQUEST_TIME_DEPENDENT_INFORMATION, Request);
...
vtkInformationKeyRestrictedMacro(
  vtkStreamingDemandDrivenPipeline, COMBINED_UPDATE_EXTENT, IntegerVector, 6);
  ...
  vtkInformation* request, vtkInformationVector** inInfoVec, vtkInformationVector* outInfoVec)
{
  ...
  if (this->Superclass::ExecuteInformation(request, inInfoVec, outInfoVec))
  {
    for (int i = 0; i < this->GetAlgorithm()->GetNumberOfOutputPorts(); ++i)
    {
      ...
      vtkSDDPSetUpdateExtentToWholeExtent(outInfoVec->GetInformationObject(i));
    }
    return 1;
  }
  else
  ...
  for (int i = 0; i < numInfo; ++i)
  {
    ...
    if (numPieces > 1)
    {
      ...
      if (outInfo->Has(vtkAlgorithm::CAN_PRODUCE_SUB_EXTENT()))
      {
        ...
        et->Delete();
        outInfo->Set(vtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), execExt, 6);
      }
    }
  }
}
    ...
    if (numPieces > 1)
    {
      ...
      if (outInfo->Has(vtkDataObject::ALL_PIECES_EXTENT()))
      {
        dobj->GetInformation()->Set(
          vtkDataObject::ALL_PIECES_EXTENT(), outInfo->Get(vtkDataObject::ALL_PIECES_EXTENT()), 6);
      }
      ...
      if (outInfo->Has(vtkAlgorithm::CAN_PRODUCE_SUB_EXTENT()))
      {
        ...
        if (outInfo->Has(vtkDataObject::ALL_PIECES_EXTENT()))
        {
          outInfo->Set(UPDATE_EXTENT(), outInfo->Get(vtkDataObject::ALL_PIECES_EXTENT()), 6);
        }
      }
  ...
  vtkInformation* request, vtkInformationVector** inInfoVec, vtkInformationVector* outInfoVec)
{
  ...
  for (int i = 0; i < outInfoVec->GetNumberOfInformationObjects(); ++i)
  {
    ...
    if (data && !outInfo->Get(DATA_NOT_GENERATED()))
    {
      ...
      if (!dataInfo->Has(vtkDataObject::DATA_TIME_STEP()) && outInfo->Has(TIME_RANGE()))
      {
        // It does not.
        ...
        if (input && input->GetInformation()->Has(vtkDataObject::DATA_TIME_STEP()))
        {
          dataInfo->CopyEntry(input->GetInformation(), vtkDataObject::DATA_TIME_STEP(), 1);
        }
  ...
  int outputPort, vtkInformationVector** inInfoVec, vtkInformationVector* outInfoVec)
{
  ...
  if (outInfo->Has(UPDATE_EXTENT()) && dataInfo->Has(vtkDataObject::DATA_EXTENT_TYPE()) &&
