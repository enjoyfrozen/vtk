  ...
  int idx, int port, int connection, const char* name)
{
  this->SetInputArrayToProcess(
    idx, port, connection, vtkDataObject::FIELD_ASSOCIATION_CELLS, name);
}
  ...
  int idx, int connection, vtkInformationVector** inputVector, int& association)
{
  ...
  if (!inArrayInfo)
  {
    vtkErrorMacro("Attempt to get an input attribute for an index (" << idx << ") that has not been specified");
    return nullptr;
  }
