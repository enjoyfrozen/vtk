...
public:
  ...
  /**
   * Returns 1 if the boxes intersect else returns 0 given a tolerance.
   */
  int Intersects(const vtkBoundingBox& bbox, double tolerance = 0.0) const;
...
inline vtkTypeBool vtkBoundingBox::ContainsPoint(double px, double py, double pz, double tol) const
{
  return px >= this->MinPnt[0] - tol && px <= this->MaxPnt[0] + tol &&
    py >= this->MinPnt[1] - tol && py <= this->MaxPnt[1] + tol && pz >= this->MinPnt[2] - tol &&
    pz <= this->MaxPnt[2] + tol;
}
