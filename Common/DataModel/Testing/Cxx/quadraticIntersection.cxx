...
void RandomCircle(vtkSmartPointer<vtkMinimalStandardRandomSequence> sequence, double radius,
...
void RandomSphere(vtkSmartPointer<vtkMinimalStandardRandomSequence> sequence, double radius,
  double* offset, double* value)
{
  double theta = 2. * vtkMath::Pi() * sequence->GetValue();
  sequence->Next();
...
void IntersectWithCell(unsigned nTest, vtkSmartPointer<vtkMinimalStandardRandomSequence> sequence,
...
int TestIntersectWithLine(int argc, char* argv[])
{
  ...
  for (int i = 0; i < bqPyra->GetNumberOfPoints(); ++i)
  {
  ...
  bqPyra->GetPoints()->SetPoint(18, 0.4, 0.4, 0.2);

  IntersectWithCell(nTest, sequence, true, radius, center, bqPyra, renderWindow);
