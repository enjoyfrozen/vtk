...
{
  vtkSmartPointer<vtkLagrangeTetra> t =
    vtkSmartPointer<vtkLagrangeTetra>::New();
...
int TestInterpolationFunction(vtkSmartPointer<vtkLagrangeTetra> cell,
                              double eps = VTK_EPSILON)
{
...
void InterpolateDerivsNumeric(vtkSmartPointer<vtkLagrangeTetra> cell,
                              double pcoords[3],
                              double* derivs,
                              double eps = VTK_EPSILON)
{
...
int TestInterpolationDerivs(vtkSmartPointer<vtkLagrangeTetra> cell,
                            double eps = VTK_EPSILON)
{
  int numPts = cell->GetNumberOfPoints();
  ...
  for(int i=0;i<numPts;++i)
    ...
    for(int j=0;j<dim*numPts;j++)
      ...
      if (fabs(derivs[j]-derivs_n[j]) >
        ...
        {
          std::cout<<j<<" is different from numeric! "<<derivs[j]<<" "<<derivs_n[j]<<" "<<fabs(derivs[j]-derivs_n[j])<<std::endl;
        ++r;
        }
      }
...
vtkIdType IntersectWithCell(unsigned nTest,
    ...
    if (t->GetPoints()->GetNumberOfPoints() != 15)
      ...
      for (vtkIdType i=0;i<t->GetPoints()->GetNumberOfPoints();i++)
        ...
        vtkIdType bindex[4] = {static_cast<vtkIdType>(t->GetPoints()->GetPoint(i)[0]*order + .5),
                               ...
                               static_cast<vtkIdType>(t->GetPoints()->GetPoint(i)[2]*order + .5),
                               0};
        ...
        if (i != idx)
          ...
          std::cout<<"bindex: "<<bindex[0]<<" "<<bindex[1]<<" "<<bindex[2]<<" "<<bindex[3]<<std::endl;
          std::cout<<"There are "<<t->GetPoints()->GetNumberOfPoints()<<" points"<<std::endl;
          return 1;
          }
        vtkIdType bindex_[4];
        t->ToBarycentricIndex(i,bindex_);
        if (bindex[0] != bindex_[0] || bindex[1] != bindex_[1] ||
            bindex[2] != bindex_[2] || bindex[3] != bindex_[3])
          {
          std::cout<<"barycentric index mismatch for order "<<order<<", index "<<i<<"! ("<<bindex[0]<<" "<<bindex[1]<<" "<<bindex[2]<<" "<<bindex[3]<<") vs "<<bindex_[0]<<" "<<bindex_[1]<<" "<<bindex_[2]<<" "<<bindex_[3]<<")"<<std::endl;
          return 1;
          }
        }
      }
