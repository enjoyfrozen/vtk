...
int TestInterpolationFunction(vtkSmartPointer<vtkLagrangeTriangle> cell,
...
void InterpolateDerivsNumeric(vtkSmartPointer<vtkLagrangeTriangle> tri,
                              double pcoords[3],
                              double* derivs,
                              double eps = VTK_EPSILON)
{
...
int TestInterpolationDerivs(vtkSmartPointer<vtkLagrangeTriangle> cell,
                            double eps = VTK_EPSILON)
{
  int numPts = cell->GetNumberOfPoints();
  ...
  for(int i=0;i<numPts;++i)
    ...
    for(int j=0;j<dim*numPts;j++)
      ...
      if (fabs(derivs[j]-derivs_n[j]) >
        ...
        {
          std::cout<<"Different from numeric! "<<j<<" "<<derivs[j]<<" "<<derivs_n[j]<<" "<<fabs(derivs[j]-derivs_n[j])<<std::endl;
        ++r;
        }
      }
...
int TestLagrangeTriangle(int argc, char* argv[])
{
  ...
  for (vtkIdType order = 1; order <= 7; ++order)
    {
    vtkSmartPointer<vtkLagrangeTriangle> t =
      CreateTriangle(nPointsForOrder[order]);
    ...
    if (t->GetPoints()->GetNumberOfPoints() != 7)
      ...
      for (vtkIdType i=0;i<t->GetPoints()->GetNumberOfPoints();i++)
        ...
        if (bindex[0] != bindex_[0] || bindex[1] != bindex_[1] ||
            bindex[2] != bindex_[2])
          {
          std::cout<<"barycentric index mismatch for order "<<order<<", index "<<i<<"! "<<bindex[0]<<" "<<bindex_[0]<<" "<<bindex[1]<<" "<<bindex_[1]<<" "<<bindex[2]<<" "<<bindex_[2]<<" "<<std::endl;
          return 1;
          }
        }
      }
