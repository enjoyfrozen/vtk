/*=========================================================================
  ...
  Module:    TestCellLocatorsLinearTransform.cxx
...
bool TestCellLocators(vtkUnstructuredGrid* dataset, vtkPointSet* transformedDataset,
  vtkPoints* transformedRandomPoints, vtkAbstractCellLocator* locator,
  const double acceptableAccuracyPercentage)
{
  // build locator with non-rotated dataset
  locator->CacheCellBoundsOn();
  locator->UseExistingSearchStructureOn();
  locator->SupportLinearTransformationOn();
  locator->SetDataSet(dataset);
  locator->BuildLocator();
  // update the locator's dataset
  locator->SetDataSet(transformedDataset);
  locator->BuildLocator();
  ...
  std::vector<vtkIdType> cellIds(static_cast<size_t>(numberOfRandomPointsPoints));
  ...
  for (vtkIdType i = 0; i < numberOfRandomPointsPoints; ++i)
  {
    ...
    cellIds[i] = locator->FindCell(transformedPoint);
  }

  // create a locator with the transformed dataset
  vtkAbstractCellLocator* locator2 = locator->NewInstance();
  ...
  std::cout << locator->GetClassName() << ": Match accuracy: " << matchAccuracyPercentage << "%, "
            << "Acceptable accuracy: " << acceptableAccuracyPercentage << "%" << std::endl;

  return matchAccuracyPercentage < acceptableAccuracyPercentage ? false : true;
}

int TestCellLocatorsLinearTransform(int argc, char* argv[])
{
