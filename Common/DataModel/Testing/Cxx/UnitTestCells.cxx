...
static vtkSmartPointer<vtkBiQuadraticPyramid> MakeBiQuadraticPyramid();
...
int UnitTestCells(int, char*[])
{
  ...
  results["PentagonalPrism"] =
    TestOneCell<vtkPentagonalPrism>(VTK_PENTAGONAL_PRISM, MakePentagonalPrism());
  results["HexagonalPrism"] =
    TestOneCell<vtkHexagonalPrism>(VTK_HEXAGONAL_PRISM, MakeHexagonalPrism());
  results["Polyhedron(Cube)"] = TestOneCell<vtkPolyhedron>(VTK_POLYHEDRON, MakeCube());
  results["Polyhedron(Dodecahedron)"] =
    TestOneCell<vtkPolyhedron>(VTK_POLYHEDRON, MakeDodecahedron());
  ...
  results["QuadraticLinearQuad"] =
    TestOneCell<vtkQuadraticLinearQuad>(VTK_QUADRATIC_LINEAR_QUAD, MakeQuadraticLinearQuad(), 0);
  results["QuadraticLinearWedge"] =
    TestOneCell<vtkQuadraticLinearWedge>(VTK_QUADRATIC_LINEAR_WEDGE, MakeQuadraticLinearWedge(), 0);
  ...
  results["BiQuadraticPyramid"] =
    TestOneCell<vtkBiQuadraticPyramid>(VTK_BIQUADRATIC_PYRAMID, MakeBiQuadraticPyramid(), 0);
...
vtkSmartPointer<vtkBiQuadraticPyramid> MakeBiQuadraticPyramid()
{
  auto aPyramid = vtkSmartPointer<vtkBiQuadraticPyramid>::New();
  ...
  aPyramid->GetPoints()->SetPoint(18, 0.5, 0.5, 13.0 / 54.0);
...
int TestOneCell(VTKCellType cellType, vtkSmartPointer<T> aCell, int linear)
{
    ...
    std::string(aCell->GetClassName()) != "vtkPolyhedron")
  {
    ...
    else if (parametricCoords)
    {
      ...
      for (int p = 0; p < aCell->GetNumberOfPoints(); ++p)
      {
        ...
        int subId = 0;
        double x[3];
        aCell->EvaluateLocation(subId, parametricCoords + 3 * p, x, pweights.data());
  ...
  if (aCell->GetNumberOfPoints() > 0)
  {
    ...
    int pSubId = 0;
    aCell->EvaluateLocation(pSubId, pcenter, center, cweights.data());
  ...
  if (aCell->GetNumberOfPoints() > 0 && strcmp(aCell->GetClassName(), "vtkQuadraticEdge") != 0)
  {
  ...
  if (locations)
  {
    ...
    for (int l = 0; l < aCell->GetNumberOfPoints(); ++l)
    {
      ...
      int subId = 0;
      aCell->EvaluateLocation(subId, locations + 3 * l, point, lweights.data());
      for (int v = 0; v < 3; ++v)
      {
  ...
  if (vlocations)
  {
    for (int i = 0; i < aCell->GetNumberOfPoints(); ++i)
    {
      ...
      aCell->EvaluatePosition(point, closestPoint, subId, pcoords, dist2, weights.data());
      for (int v = 0; v < 3; ++v)
      {
      ...
      {
        std::cout << std::endl
                  << "Point id: " << i << ": " << *(vlocations + 3 * i + 0) << ", "
                  << *(vlocations + 3 * i + 1) << ", " << *(vlocations + 3 * i + 2)
  ...
  if (aCell->GetNumberOfPoints() > 0)
  {
    ...
    aCell->EvaluateLocation(subId, pcenter, &(*tCenter.begin()), weights.data());
  ...
  for (int e = 0; e < aCell->GetNumberOfEdges(); ++e)
  {
    ...
    c->EvaluateLocation(subId, pcenter, &(*eCenter.begin()), weights.data());
  ...
  for (int f = 0; f < aCell->GetNumberOfFaces(); ++f)
  {
    ...
    c->GetParametricCenter(pcenter);
    c->EvaluateLocation(subId, pcenter, &(*fCenter.begin()), weights.data());
  ...
  for (size_t p = 0; p < testPoints.size(); ++p)
  {
    ...
    int inOut = aCell->EvaluatePosition(
      &(*testPoints[p].begin()), closestPoint, subId, pcoords, dist2, weights.data());
