...
static void OctreeNodeGetPointId(
  vtkPoints* vtkNotUsed(points), vtkIdType* vtkNotUsed(pntIdx), const double* vtkNotUsed(coords))
{
...
vtkIncrementalOctreeNode::vtkIncrementalOctreeNode()
{
  ...
  // octree and this function is also always called for any descendant
  ...
  vtkIdList* pntIds, const double newPnt[3], vtkIdType* pntIdx, int maxPts, int ptMode)
{
  ...
  while (duplic == single) // as long as separation has not been achieved
  {
    ...
    for (i = 0; i < 8; i++)
    {
      ...
      ocNode->Children[i]->SetBounds(
        octMin[0], octMax[0], octMin[1], octMax[1], octMin[2], octMax[2]);
    }
  ...
  duplic->UpdateCounterAndDataBoundsRecursively(dupPnt, pntIds->GetNumberOfIds(), 1, this);
  ...
  const double newPnt[3], vtkIdType* pntIdx, int maxPts, int ptMode, int& numberOfNodes)
{
  ...
  else
  {
    ...
    this->Children[target]->UpdateCounterAndDataBoundsRecursively(newPnt, 1, 1, nullptr);
...
int vtkIncrementalOctreeNode::InsertPoint(vtkPoints* points, const double newPnt[3], int maxPts,
  vtkIdType* pntId, int ptMode)
{
  VTK_LEGACY_REPLACED_BODY(vtkIncrementalOctreeNode::InsertPoint(
                             vtkPoints* points, const double newPnt[3], int maxPts,
                             vtkIdType* pntId, int ptMode),
                           "VTK 9.1",
                           vtkIncrementalOctreeNode::InsertPoint(
                             vtkPoints* points,
                             const double newPnt[3], int maxPts,
...
{
  double dumbPnt[3];
  return (checkData == 1 && this->GetNumberOfPoints() == 0)
    ? VTK_DOUBLE_MAX
