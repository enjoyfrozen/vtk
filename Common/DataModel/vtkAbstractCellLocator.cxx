...
{
  if (this->WeightsTime > this->MTime)
  {
    return;
  }
  ...
  bool weightsHasBeenResized = false;
  ...
  if (auto ug = vtkUnstructuredGrid::SafeDownCast(this->DataSet))
  {
    if (vtkCellArray* cells = ug->GetCells())
    {
      if (cells->IsStorage64Bit())
      {
        ...
        this->Weights.resize(std::max<vtkIdType>(worker.GlobalNumberOfPoints, 32));
      }
      else
      {
