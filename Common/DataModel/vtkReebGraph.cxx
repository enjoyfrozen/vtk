...
public:
  ...
  double ComputeCustomMetric(vtkReebGraphSimplificationMetric* simplificationMetric, vtkReebArc* a);

  // Description:
  //  Add a monotonic path between nodes.
  vtkIdType AddPath(int nodeNumber, vtkIdType* nodeOffset, vtkReebLabelTag label);
  ...
  // the Reeb graph.
  void Collapse(vtkIdType startingNode, vtkIdType endingNode, vtkReebLabelTag startingLabel,
  ...
  vtkIdType FindGreater(vtkIdType nodeId, vtkIdType startingNodeId, vtkReebLabelTag label);

  // Description:
  // Find corresponding joining saddle node (persistence-based simplification).
  vtkIdType FindJoinNode(vtkIdType arcId, vtkReebLabelTag label, bool onePathOnly = false);

  // Description:
  // Find smaller arc (persistence-based simplification).
  vtkIdType FindLess(vtkIdType nodeId, vtkIdType startingNodeId, vtkReebLabelTag label);
  ...
  // simplification).
  vtkIdType FindSplitNode(vtkIdType arcId, vtkReebLabelTag label, bool onePathOnly = false);
...
void vtkReebGraph::Implementation::DeepCopy(Implementation* srcG)
{
  ...
  if (srcG->MainArcTable.Buffer)
  {
    ...
    free(this->MainArcTable.Buffer);
    this->MainArcTable.Buffer = (vtkReebArc*)malloc(sizeof(vtkReebArc) * srcG->MainArcTable.Size);
  ...
  if (srcG->MainNodeTable.Buffer)
  {
    ...
    this->MainNodeTable.Buffer =
      (vtkReebNode*)malloc(sizeof(vtkReebNode) * srcG->MainNodeTable.Size);
  ...
  if (srcG->MainLabelTable.Buffer)
  {
    ...
    this->MainLabelTable.Buffer =
      (vtkReebLabel*)malloc(sizeof(vtkReebLabel) * srcG->MainLabelTable.Size);
  ...
  if (srcG->VertexMapSize)
  {
    ...
    this->VertexMap = (vtkIdType*)malloc(sizeof(vtkIdType) * this->VertexMapAllocatedSize);
  ...
  vtkIdType arcId, double simplificationThreshold, vtkReebGraphSimplificationMetric* metric)
{
  ...
  NOT_FOUND:
    ...
    if (Atouch)
      free(Atouch);
    vtkReebPath fake;
    memset(&fake, 0, sizeof(vtkReebPath));
  ...
  vtkReebGraphSimplificationMetric* simplificationMetric, vtkReebArc* a)
{
  ...
  vtkDataArray* vertexInfo =
    vtkArrayDownCast<vtkDataArray>(this->Parent->GetVertexData()->GetAbstractArray("Vertex Ids"));
  if (!vertexInfo)
    return vtkReebGraphGetArcPersistence(this, a);

  vtkVariantArray* edgeInfo =
    vtkArrayDownCast<vtkVariantArray>(this->Parent->GetEdgeData()->GetAbstractArray("Vertex Ids"));
  if (!edgeInfo)
    return vtkReebGraphGetArcPersistence(this, a);
...
int vtkReebGraph::Implementation::CommitSimplification()
{
  ...
  vtkVariantArray* edgeInfo =
    vtkArrayDownCast<vtkVariantArray>(this->Parent->GetEdgeData()->GetAbstractArray("Vertex Ids"));
  vtkDataArray* vertexInfo = this->Parent->GetVertexData()->GetArray("Vertex Ids");
  ...
  for (unsigned int i = 0; i < after.size(); i++)
  {
    ...
    if ((downIt != vMap.end()) && (upIt != vMap.end()))
    {
      ...
      vertexList->SetNumberOfValues(static_cast<vtkIdType>(after[i].second.size()));
      for (unsigned int j = 0; j < after[i].second.size(); j++)
...
void vtkReebGraph::CloseStream()
{
  ...
  for (unsigned int i = 0; i < globalAdjacency.size(); i++)
  {
    ...
    if ((downIt != vMap.end()) && (upIt != vMap.end()))
    {
      vtkVariantArray* edgeProperties = vtkVariantArray::New();
      vtkIdTypeArray* vertexList = vtkIdTypeArray::New();
      vertexList->SetNumberOfValues(static_cast<vtkIdType>(globalAdjacency[i].second.size()));
        ...
        for (int dir = 0; dir <= 1; dir++)
        {
               ...
               A = (!dir) ? (this->GetArc(A)->ArcDwId1) : (this->GetArc(A)->ArcDwId0))
          {
            ...
            else
            {
              this->LoopNumber++;
              this->ArcLoopTable =
                (vtkIdType*)realloc(this->ArcLoopTable, sizeof(vtkIdType) * this->LoopNumber);
...
vtkIdType vtkReebGraph::Implementation::FindDwLabel(vtkIdType nodeId, vtkReebLabelTag label)
{
  for (vtkIdType arcId = this->GetNode(nodeId)->ArcDownId; arcId;
       arcId = this->GetArc(arcId)->ArcDwId1)
  {
    for (vtkIdType labelId = this->GetArc(arcId)->LabelId0; labelId;
...
void vtkReebGraph::Implementation::ResizeMainArcTable(int newSize)
{
  ...
  if ((this->MainArcTable.Size - this->MainArcTable.Number) < newSize)
  {
    ...
    this->MainArcTable.Buffer =
      (vtkReebArc*)realloc(this->MainArcTable.Buffer, sizeof(vtkReebArc) * this->MainArcTable.Size);
...
void vtkReebGraph::Implementation::ResizeMainLabelTable(int newSize)
{
  ...
  if ((this->MainLabelTable.Size - this->MainLabelTable.Number) < newSize)
  {
    ...
    this->MainLabelTable.Buffer = (vtkReebLabel*)realloc(
      this->MainLabelTable.Buffer, sizeof(vtkReebLabel) * this->MainLabelTable.Size);
  ...
  vtkReebLabelTag startingLabel, vtkReebLabelTag endingLabel)
{
  ...
  while (true)
  {
    ...
    if (n0->IsFinalized && vtkReebGraphIsRegular(this, n0))
    {
      if (historyOn)
      {
        vtkReebArc *up = this->GetArc(n0->ArcUpId), *down = this->GetArc(n0->ArcDownId);
  ...
  vtkIdType vertex1Id, double f1, vtkIdType vertex2Id, double f2, vtkIdType vertex3Id, double f3)
{
  ...
  vtkIdType t0[] = { vertex0, vertex1, vertex2 }, t1[] = { vertex0, vertex1, vertex3 },
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    vtkReebLabelTag Label01 =
      ((vtkReebLabelTag)cellIds[i][0]) | (((vtkReebLabelTag)cellIds[i][1]) << 32);
    vtkReebLabelTag Label12 =
      ((vtkReebLabelTag)cellIds[i][1]) | (((vtkReebLabelTag)cellIds[i][2]) << 32);
    vtkReebLabelTag Label02 =
      ((vtkReebLabelTag)cellIds[i][0]) | (((vtkReebLabelTag)cellIds[i][2]) << 32);
  ...
  vtkIdType vertex0Id, double f0, vtkIdType vertex1Id, double f1, vtkIdType vertex2Id, double f2)
{
  ...
  vtkReebLabelTag Label01 = ((vtkReebLabelTag)vertex0) | (((vtkReebLabelTag)vertex1) << 32);
  vtkReebLabelTag Label12 = ((vtkReebLabelTag)vertex1) | (((vtkReebLabelTag)vertex2) << 32);
  vtkReebLabelTag Label02 = ((vtkReebLabelTag)vertex0) | (((vtkReebLabelTag)vertex2) << 32);
  ...
  double scalar3)
{
  ...
  else if (this->VertexMapSize >= this->VertexMapAllocatedSize - 4)
  {
    ...
    this->VertexMap =
      (vtkIdType*)realloc(this->VertexMap, sizeof(vtkIdType) * this->VertexMapAllocatedSize);
...
int vtkReebGraph::Build(vtkPolyData* mesh, vtkIdType scalarFieldId)
{
  ...
  if (!scalarField)
    return vtkReebGraph::ERR_NO_SUCH_FIELD;
...
int vtkReebGraph::Build(vtkPolyData* mesh, const char* scalarFieldName)
{
  ...
  vtkPointData* pointData = mesh->GetPointData();
  vtkDataArray* scalarField = pointData->GetArray(scalarFieldName, scalarFieldId);
