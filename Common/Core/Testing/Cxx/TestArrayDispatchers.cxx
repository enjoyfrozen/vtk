...
int TestDispatch()
{
  ...
  for (vtkDataArray* array : Arrays::allArrays)
  {
    ...
    int rval{ 20 };
    testAssert(
      Dispatcher::Execute(array, paramTester, lval, rval), "Parameter forwarding dispatch failed.");
...
int TestDispatchByValueType()
{
  ...
  for (vtkDataArray* array : Arrays::allArrays)
  {
    ...
    if (isValid)
    {
      ...
      int lval{ 42 };
      int rval{ 20 };
      testAssert(Dispatcher::Execute(array, paramTester, lval, rval),
  ...
  for (vtkDataArray* array1 : Arrays::allArrays)
  {
    ...
    for (vtkDataArray* array2 : Arrays::allArrays)
    {
      ...
      if (array1Valid && array2Valid)
      {
        ...
        int lval{ 42 };
        int rval{ 20 };
        testAssert(Dispatcher::Execute(array1, array2, paramTester, lval, rval),
          "Parameter forwarding dispatch failed.");
        testAssert(paramTester.Success, "Parameter forwarding failed.");
        paramTester.Reset();
    ...
    for (vtkDataArray* array2 : Arrays::allArrays)
    {
      ...
      for (vtkDataArray* array3 : Arrays::allArrays)
      {
        ...
        if (array1Valid && array2Valid && array3Valid)
        {
          ...
          int lval{ 42 };
          int rval{ 20 };
          testAssert(Dispatcher::Execute(array1, array2, array3, paramTester, lval, rval),
            "Parameter forwarding dispatch failed.");
          testAssert(paramTester.Success, "Parameter forwarding failed.");
          paramTester.Reset();
