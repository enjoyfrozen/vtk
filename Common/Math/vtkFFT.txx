  ...
  InputIt begin, InputIt end, Scaling scaling, double fs)
{
  ...
  {
    return 1.0 / std::pow(std::accumulate(begin, end, zero), 2.0);
  }
}

//------------------------------------------------------------------------------
template <typename T, typename TW>
void vtkFFT::PreprocessAndDispatchFft(std::vector<T>& segment, const std::vector<TW>& window,
  bool detrend, bool onesided, vtkFFT::ComplexNumber* result)
{
  const T mean = detrend
    ? std::accumulate(segment.cbegin(), segment.cend(), Zero<T>()) / segment.size()
    : Zero<T>();
  std::transform(segment.cbegin(), segment.cend(), window.cbegin(), segment.begin(),
  ...
  unsigned int* shape)
{
  ...
  vtkSMPTools::For(0, nsegment, [&](std::size_t begin, std::size_t end) {
    ...
    {
      const auto beginSegment = sig.cbegin() + i * segmentOffset;
      const auto endSegment = beginSegment + window.size();
      std::vector<T> segment{ beginSegment, endSegment };
      vtkFFT::PreprocessAndDispatchFft(
        segment, window, detrend, onesided, result.data() + i * nfft);
    }
  });
...
vtkFFT::ComplexNumber* vtkFFT::OverlappingFft(vtkFFT::vtkScalarNumberArray* signal,
  ...
  unsigned int* shape)
{
  const auto sigRange = vtk::DataArrayTupleRange(signal);
  using ConstTupelRefType = decltype(sigRange)::ConstTupleReferenceType;

  const std::size_t segmentOffset = window.size() - noverlap;
  const std::size_t nsegment = (sigRange.size() - noverlap) / segmentOffset;
  const std::size_t nfft = onesided ? window.size() / 2 + 1 : window.size();
  const std::size_t outSize = nsegment * nfft;
  if (shape != nullptr)
  {
  ...
  vtkSMPTools::For(0, nsegment, 1, [&](std::size_t begin, std::size_t end) {
    for (std::size_t i = begin; i < end; ++i)
    {
      const auto beginSig = sigRange.cbegin() + i * segmentOffset;
      const auto endSig = beginSig + window.size();
      if (sigRange.GetTupleSize() == 1)
      {
        std::vector<vtkFFT::ScalarNumber> segment(window.size());
        std::transform(
          beginSig, endSig, segment.begin(), [](ConstTupelRefType tuple) { return tuple[0]; });
        PreprocessAndDispatchFft(segment, window, detrend, onesided, result + i * nfft);
      }
      else //: sigRange.GetTupleSize() == 2
      {
        std::vector<vtkFFT::ComplexNumber> segment(window.size());
        std::transform(beginSig, endSig, segment.begin(), [](ConstTupelRefType tuple) {
          return vtkFFT::ComplexNumber{ tuple[0], tuple[1] };
        });
        PreprocessAndDispatchFft(segment, window, detrend, onesided, result + i * nfft);
      }
    }
  ...
  vtkFFT::Scaling scaling, vtkFFT::SpectralMode mode, unsigned int* shape)
{
  ...
  if (noverlap < 0 || static_cast<size_t>(noverlap) >= window.size())
  {
