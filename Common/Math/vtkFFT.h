...
public:
  ...
  static_assert(sizeof(ComplexNumber) == 2 * sizeof(ScalarNumber), "vtkFFT::ComplexNumber definition is not valid");
  ...
  enum Scaling : int
  {
    Density = 0, ///< Cross Spectral \b Density scaling ( \f(V^2/Hz) )
    Spectrum     ///< Cross \b Spectrum scaling ( \f(V^2) )
  };
   ...
   * @param[in] onesided if true return a one-sided spectrum for real data. If input is copmlex then
   * this option will be ignored.
   * @param[in] scaling can be either Cross Spectral \b Density ( \f(V^2/Hz) ) or Cross \b Spectrum
   * ( \f(V^2) )
   ...
   * ( \f(V^2) )
   *
   * @return a 1D array containing the resulting cross spectral density ( \f(V^2/Hz) )
   *
...
// Contains all complex templated functions. All of these functions will not
// be wrapped by python
#include "vtkFFT.txx"
